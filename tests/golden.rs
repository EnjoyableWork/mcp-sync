...
#[test]
fn golden_m1_journey_runs_init_add_list_dry_run_apply_and_repeat_as_one_safe_flow() {
    ...
    let listed_names = ["\"added\":", "\"alpha\":", "\"beta\":", "\"shared\":"];
    ...
    assert!(
        dry_output.starts_with("Dry run validated 2 targets; no files changed.\n"),
        "dry-run should validate the complete two-target plan"
    );
    assert!(
    ...
    assert!(!claude_backup.exists() && !cursor_backup.exists());

    let apply_output = stdout(&run_success(cli_command(&home, &["sync"])));
    assert!(
        apply_output.starts_with("Sync completed for 2 targets.\n")
    ...
    for target in [&claude_after, &cursor_after] {
        assert!(
    ...
    assert!(
        no_op_output
            ...
            .count()
            == 2,
        "repeat sync should report two target no-ops"
    );
    ...
    for marker in [&imported_marker, &updated_marker, &added_marker] {
        assert!(!marker.exists(), "M1 commands must never execute servers");
    }
    for path in [&canonical_path, &claude_path, &cursor_path] {
...
"#;
...

    {
        ...
        home.write_file(&home.claude_desktop_configuration(), &claude);
        home.write_file(&home.cursor_configuration(), malformed_cursor);
        let diagnostic = stderr(&run_failure(cli_command(&home, &["sync"])));
        assert!(
            diagnostic.contains("cannot plan Cursor sync: invalid Cursor JSON:")
        ...
        assert_file_matches(
            &home.cursor_configuration(),
            malformed_cursor,
            "preflight failure must preserve Cursor",
        );
