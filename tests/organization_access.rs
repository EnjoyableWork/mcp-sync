...
#[test]
fn completed_release_state_and_mcp_043_execution_are_preserved() {
    ...
    assert!(project.contains("`MCP-042` is complete; `MCP-043` is active under its"));
    ...
    assert!(project.contains("| D-22 | Portable canonical environment-name validation | Post-M4 release readiness | Codex | Before `v0.1.1` | In progress |"));
    ...
    assert!(project.contains("| MCP-043 | Reject environment names that cannot retain identity across supported boundaries | Post-M4 release readiness | P0 | Codex | In progress | `MCP-042` |"));
    ...
    assert!(project.contains("Issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46) is the highest-leverage remaining issue"));
