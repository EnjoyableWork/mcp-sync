...
#[test]
fn completed_release_state_and_mcp_043_closure_are_preserved() {
    ...
    assert!(project.contains("M0 through M3, `MCP-001` through `MCP-020`, `MCP-028` through `MCP-037`, and `MCP-039` through `MCP-043` are `Done`"));
    assert!(project.contains("`MCP-037` and D-18 are `Done`"));
    assert!(project.contains("`MCP-037`, `MCP-040`, and `MCP-041` are\ncompleted"));
    assert!(project.contains("`MCP-042` and `MCP-043` are complete; `MCP-044` is"));
    assert!(
        project.contains("active under its exact catalog Goal, while `MCP-045` remains proposed")
    );
    ...
    assert!(project.contains("| D-23 | Deterministic Windows ARM64 health-fixture evidence | Post-M4 release readiness | Codex | Before `v0.1.1` | In progress |"));
    ...
    assert!(project.contains("| MCP-044 | Make Windows ARM64 health-fixture readiness deterministic | Post-M4 release readiness | P1 | Codex | In progress | `MCP-043` |"));
    ...
    assert!(project.contains("| DEC-053 | Reject environment names outside the portable canonical identity intersection | Accepted |"));
    ...
    assert!(project.contains("991e6b2114267fb26c3beb3a319e96de0452c7e2"));
    for exact_main_run in ["31612019267", "31612019238", "31612019237", "31612019507"] {
        assert!(
            project.contains(exact_main_run),
    ...
    for exact_head_run in ["31625719526", "31625718066", "31625719556", "31625719525"] {
        assert!(
            project.contains(exact_head_run),
            "MCP-043 closure must retain exact-head run {exact_head_run}"
        );
    }
    for exact_main_run in ["31628199405", "31628198899", "31628199418", "31628199424"] {
        assert!(
            project.contains(exact_main_run),
            "MCP-043 closure must retain exact-main run {exact_main_run}"
        );
    }
    assert!(project.contains("31513698204/attempts/1"));
    assert!(project.contains("sequential native-Rust process harness"));
    ...
    assert!(!project.contains("| MCP-043 | Reject environment names that cannot retain identity across supported boundaries | Post-M4 release readiness | P0 | Unassigned | Ready |"));
    assert!(!project.contains("| MCP-043 | Reject environment names that cannot retain identity across supported boundaries | Post-M4 release readiness | P0 | Codex | Ready |"));
    assert!(!project.contains("| MCP-044 | Make Windows ARM64 health-fixture readiness deterministic | Post-M4 release readiness | P1 | Unassigned | Proposed |"));
