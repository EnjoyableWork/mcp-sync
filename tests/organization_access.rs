...
fn completed_release_state_and_mcp_043_closure_are_preserved() {
    ...
    assert!(project.contains("ready but not active, while `MCP-045` remains proposed"));
    ...
    assert!(project.contains("| MCP-044 | Make Windows ARM64 health-fixture readiness deterministic | Post-M4 release readiness | P1 | Unassigned | Ready | `MCP-043` |"));
    ...
    assert!(project.contains("Issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46) is fixed and ready for closure on merge"));
