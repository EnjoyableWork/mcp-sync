...
fn completed_release_state_and_mcp_046_closure_are_preserved() {
    ...
    assert!(project.contains("9127775b69f6a710bb8e12a1a10f27409088c68a"));
    for exact_head_run in ["31634225214", "31634222077", "31634225001", "31634225071"] {
        assert!(
            project.contains(exact_head_run),
            "MCP-044 closure must retain exact-head run {exact_head_run}"
        );
    }
    assert!(project.contains("upstream GitHub `503` during pinned Syft acquisition"));
