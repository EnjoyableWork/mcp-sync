...
| Known open defects | Issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46) is closed by merged [PR #66](https://github.com/EnjoyableWork/mcp-sync/pull/66) and exact-main commit [`e96c1c4`](https://github.com/EnjoyableWork/mcp-sync/commit/e96c1c437b5ce9d9f5138b790dd23ad2926fa749), whose 23 CI, CodeQL, release-preflight, and source/install/archive checks passed on attempt one. Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) records the post-`SIDE-008` recurrence of Windows ARM64 health-fixture startup timing without establishing a product defect and is active under exact-Goal `MCP-044`. Issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44) is closed by merged [PR #65](https://github.com/EnjoyableWork/mcp-sync/pull/65) and exact-main commit [`991e6b2`](https://github.com/EnjoyableWork/mcp-sync/commit/991e6b2114267fb26c3beb3a319e96de0452c7e2), whose CI and both native preflights passed on attempt one |
| Current milestone | Post-M4 release readiness — `MCP-042`, `MCP-043`, D-21, and D-22 are `Done`; `MCP-044` and D-23 are `In progress`; `MCP-045` remains `Proposed`; and no side quest is active |
| Overall status | M0 through M3, `MCP-001` through `MCP-020`, `MCP-028` through `MCP-037`, and `MCP-039` through `MCP-043` are `Done`; `MCP-044` is `In progress`; `MCP-045` is `Proposed`; `MCP-021` is `Superseded`; `MCP-022` through `MCP-027` are `Deferred`; `SIDE-007` and `SIDE-008` are `Done`; and funded `SIDE-006` remains proposed |
| Current focus | Complete active `MCP-044` under its exact catalog Goal by replacing scheduler-sensitive PowerShell unit fixtures with a focused sequential native harness, retaining pure lower-layer framing proof, and obtaining repeated no-rerun Windows ARM64 evidence for issue #60. Keep `MCP-045`, every deferred expansion, and funded `SIDE-006` inactive until their predecessors, separate Goal contracts, and publication approval permit activation |
...
first real repeat release. `MCP-042` and `MCP-043` are complete; `MCP-044` is
active under its exact catalog Goal, while `MCP-045` remains proposed.
...
completed. `MCP-042` and `MCP-043` are complete, `MCP-044` is active under its
exact catalog Goal, `MCP-045` remains proposed, and no side quest is active.
...
1. Complete active `MCP-044` under its exact catalog Goal by removing
   interpreter startup from the parallel unit-test readiness boundary, proving
   the replacement native harness repeatedly on Windows ARM64, and closing
   issue #60 only after the complete native matrix passes without rerun.
2. Keep `MCP-045` proposed and ordered behind `MCP-044`; publication requires
   its own exact Goal, separate explicit approval, and live control checks.
...
environment-name validation, active `MCP-044` deterministic Windows ARM64
health-fixture evidence, and separately
approved `MCP-045` publication of `v0.1.1`. `MCP-042` selected and proved the
...
`MCP-029`, M2, `MCP-030` through `MCP-037`, `MCP-039` through `MCP-043`, M3,
bounded M4 expansion, `SIDE-007`, and `SIDE-008` are complete; only `MCP-044`
is active under its exact catalog Goal, and no side quest is active. The
...
activated and completed only `MCP-042`; the subsequent exact Goal activated
and completed only `MCP-043`. The current exact Goal activates only `MCP-044`,
leaving `MCP-045` proposed.
`DEC-031`, `DEC-036`, and
...
| RISK-24 | Scheduler-dependent tests hide regressions behind retries or generous timing | High | High | `DEC-052` makes deterministic synchronization and no-retry evidence repository policy. The historical CI audit distinguished repeatable implementation failures from identical-source variance in process health fixtures, including issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) and `MCP-042`'s macOS x64 failure where a reparented inherited-stdout holder crossed between ancestry snapshots. Active `MCP-044` isolates issue #60 to generated PowerShell startup inside a parallel unit binary, removes interpreter startup from that boundary, retains pure framing proof, and uses a sequential native process harness with an explicit ready/release transition; supplemental Windows ARM64 repetition remains fail-fast within the existing protected CI context. Product time limits and outer watchdogs remain bounded | Identical source both passes and fails; a race or timeout disappears only on rerun; correctness depends on a sleep, polling frequency, runner speed, or broader timeout; automatic retry, quarantine, assertion weakening, or unjustified serialization is proposed; or a native boundary lacks a deterministic lower-layer proof | Open — policy and the deterministic lower-layer correction are implemented locally; `MCP-044` still requires complete no-rerun hosted evidence |
