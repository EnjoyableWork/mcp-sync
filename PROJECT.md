...
#62's accepted source/GNU/Linux release lifecycle repeatable. `DEC-050`
continues the ordered main story through issues #44, #46, and #60 and the first
real repeat release. `MCP-042` through `MCP-046` are complete. `MCP-046`
removed the required P0 corrective boundary after replacing the long-lived
shared-tap writer, receiving fresh owner confirmation for deletion, and
revalidating both affected projects. No main-story ticket or side quest is
active.
...
| Post-M4 corrective release maintenance — tap authority isolation | `MCP-046` (done; depends on completed `MCP-045`) |
...
completed. `MCP-042` through `MCP-046` are complete. No main-story ticket or
side quest is active.
...
- Validation is field-aware: server names and commands reject every Unicode
  control character; arguments and environment values reject NUL; and
  environment names must be non-empty and contain neither NUL nor `=`. Other
  literal environment-name characters remain valid, including whitespace and
  newlines, because the supported process boundaries retain their identity.
...
- Unmanaged Cursor, Windsurf, VS Code, Codex, and Kiro entries remain
  unrepresentable in canonical JSON v1. This includes otherwise-local entries
  whose environment names are empty or contain NUL or `=`. Their sorted names
  are reported as skipped per client, while a same-name local definition is an
  actionable conflict rather than an implicit conversion, coercion, or
  overwrite. Claude Desktop has no unmanaged-local classification and fails
  structurally on the same invalid definition.
...
- Environment names use the same canonical rule before lock acquisition or
  filesystem access: an empty `--env` name and any canonical name containing
  NUL or `=` fail without normalization or disclosure.
...
- Thirteen resolver tests, exact Windows path fixtures in all five adapters, and
  ...
  target placement. A dedicated sequential native-Rust fixture harness exercises
  bounded health success, timeout, malformed output, shutdown refusal,
  redaction, and direct-child cleanup without interpreter startup; pure unit
  tests retain deterministic protocol and response-framing coverage.
...
| D-20 | Repeatable protected source/GNU/Linux release lifecycle after `0.1.0` | Post-M4 release maintenance | Codex | 2026-08-11 | Done | Issue [#62](https://github.com/EnjoyableWork/mcp-sync/issues/62) is fixed by protected [PR #63](https://github.com/EnjoyableWork/mcp-sync/pull/63), implementation head [`39cae90`](https://github.com/EnjoyableWork/mcp-sync/commit/39cae90a20cbd898636b9ed9a7ecce89770b3299), and exact-main merge commit [`9354919`](https://github.com/EnjoyableWork/mcp-sync/commit/93549198af5cad21651d9391f4d4c23a995e7656). The completed implementation adds the shared [stable-version policy](scripts/validate-release-version.sh), credential-free request validation plus exact-tag publish/no-write rehearsal split in the [source/GNU/Linux producer](.github/workflows/source-linux-release.yml), retry-safe exact registry equality in the protected [Cargo publisher](.github/workflows/cargo-publish.yml), the tested [Homebrew formula transition policy](scripts/validate-homebrew-formula-update.sh), generalized [historical Homebrew handoff](https://github.com/EnjoyableWork/mcp-sync/blob/93549198af5cad21651d9391f4d4c23a995e7656/.github/workflows/source-linux-release-publish-homebrew.yml) and [channel verification](.github/workflows/source-linux-release-channels.yml), one global release-writer concurrency group, focused shell and Rust regressions, and current [source](docs/source-linux-release.md) plus [funded](docs/release.md) runbooks. The complete local suite lists 365 tests: 364 pass and the abrupt-process child fixture is intentionally ignored outside its parent regression. Formatting, warning-free Clippy, `cargo deny`, Actionlint, ShellCheck, repository/workflow policy suites, upstream action verification, all six target compilation checks, deterministic double Cargo packaging and source-channel generation, packaged install/recovery, Cargo publication dry-run, native archive smoke, SPDX 2.3 validation, documentation structure, and diff checks also pass. Exact-main [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31542055279), [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31542054634), complete six-platform [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31542055233), and complete [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31542055251) pass. The exact-main [nonpublishing rehearsal](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31542683304) verifies the immutable seven-asset `v0.1.0` release, attestations, complete checksum and target SPDX SBOM evidence, byte-identical unyanked Cargo package, exact tap formula, mismatch rejection, and stable-version policy while every publish, protected-environment, build, attestation, and OIDC path remains skipped. A final public audit finds only the unchanged immutable `v0.1.0` release, unyanked crates.io `0.1.0`, original tap formula, and pre-existing operational recovery tag; no version, tag, release, crate, formula, WinGet package, macOS or Windows artifact, signed-native artifact, reusable token, successor, or side quest was created or activated. |
| D-21 | Bounded cross-platform health-process containment | Post-M4 release readiness | Codex | Before `v0.1.1` | Done | Issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44) is fixed by merged [PR #65](https://github.com/EnjoyableWork/mcp-sync/pull/65). The [containment boundary](src/process_containment.rs) uses a suspended non-breakaway Windows Job Object, monitored Unix process group, Linux subreaper and PID descriptors, and exact macOS process plus inherited-pipe identities; [health integration](src/health.rs) retains bounded cleanup, direct-child reap, structural redaction, and drop recovery. The deterministic [built-binary regression](tests/health_process_containment.rs) forces a root, intermediary, and reparented double-`setsid` leaf through success, protocol failure, response timeout, shutdown refusal, setup failure, returned cleanup failure, and drop; the macOS case erases ancestry history so exact pipe discovery is the only passing path. The first draft [macOS x64 failure](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31602718822/job/94133885654) proved a 5-millisecond ancestry poll was not a correctness mechanism; implementation commit [`1c96c91`](https://github.com/EnjoyableWork/mcp-sync/commit/1c96c9168b6b8d0f741e512f396ffe68972d095e) removed that timing dependency without changing the five-second response, 1-MiB message, or 500-millisecond graceful-shutdown limits. All 24 hosted checks passed on attempt one at both the implementation and closeout heads. The exact implementation head passes [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31609243077), [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31609238773), [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31609243085), and [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31609243622). Exact-main merge commit [`991e6b2`](https://github.com/EnjoyableWork/mcp-sync/commit/991e6b2114267fb26c3beb3a319e96de0452c7e2) passes attempt-one [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31612019267), [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31612019238), [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31612019237), and [main control verification](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31612019507). |
| D-22 | Portable canonical environment-name validation | Post-M4 release readiness | Codex | Before `v0.1.1` | Done | Issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46), `DEC-050`, `DEC-053`, and completed `MCP-043` define one canonical v1 invariant: every environment name is non-empty and contains neither NUL nor `=`, with no normalization or schema migration. One shared validator in [`config.rs`](src/config.rs) is the authority: `add` reaches it before lock acquisition or filesystem access, and canonical reads reach it before list, sync planning, or process launch. Native translation uses that same rule while preserving existing ownership contracts: Claude Desktop returns a structural invalid-canonical error, while Cursor, Windsurf, VS Code, Codex, and Kiro classify an otherwise-local entry with an invalid name as canonical-unrepresentable, preserve it structurally under the adapter's existing round-trip contract, report it as skipped during `init`, and refuse a colliding desired server during sync. No invalid definition is imported, synchronized, or executed, and no existing file changes on rejection. Returned validation errors identify only client/server context, deterministic key position, and the violated rule; unmanaged-entry reports retain their existing client/server-only redaction. Neither path exposes an environment value or invalid key text. Focused model and adapter tests, dedicated canonical and six-client fixtures under [`tests/fixtures`](tests/fixtures), and the combined built-binary regression in [`tests/environment_names.rs`](tests/environment_names.rs) prove every required fail, preserve, collision, redaction, no-mutation, and no-execution outcome; ten consecutive clean-target executions of the new built-binary suite pass. Formatting, dependency policy, deterministic packaging, repository policy checks, all six strict-Clippy compilation surfaces, complete native ARM64 and Rosetta x64 macOS suites, and the complete unprivileged native ARM64 GNU/Linux synthetic-home gate pass. Protected [PR #66](https://github.com/EnjoyableWork/mcp-sync/pull/66) implementation head [`9472a7a`](https://github.com/EnjoyableWork/mcp-sync/commit/9472a7ab946f424df8c157ab35fe5eab9f1a6bb5) passes all 24 hosted checks on attempt one: native x64/ARM64 [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31625719526), [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31625718066), six-target [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31625719556), and [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31625719525). It merged as exact-main commit [`e96c1c4`](https://github.com/EnjoyableWork/mcp-sync/commit/e96c1c437b5ce9d9f5138b790dd23ad2926fa749), whose attempt-one [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31628199405), [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31628198899), [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31628199418), and [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31628199424) pass. No dependency, parser, unsafe code, schema version, version bump, release artifact, or side quest was added; immutable `v0.1.0` is unchanged, and merged PR #66 closed issue #46. |
| D-23 | Deterministic Windows ARM64 health-fixture evidence | Post-M4 release readiness | Codex | Before `v0.1.1` | Done | Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60), completed `MCP-044`, merged [PR #67](https://github.com/EnjoyableWork/mcp-sync/pull/67), and exact implementation head [`9127775`](https://github.com/EnjoyableWork/mcp-sync/commit/9127775b69f6a710bb8e12a1a10f27409088c68a) preserve and correct the post-`SIDE-008` recurrence. Failed [attempt 1](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31513698204/attempts/1) isolates PowerShell interpreter startup and scheduler contention from protocol and cleanup behavior. The correction retains pure response-framing tests and uses one sequential native-Rust process harness with an explicit ready/release state transition under one outer watchdog; the real five-second response, 500-millisecond shutdown, redaction, direct-child, and complete process-tree assertions remain. Ten local repetitions and exact-head [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225214), [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634222077), [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225001), and [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225071) pass; the release-preflight result followed a preserved upstream GitHub `503` during pinned Syft acquisition and is not health-fix acceptance evidence. PR #67 merged as exact-main commit [`31dba46`](https://github.com/EnjoyableWork/mcp-sync/commit/31dba465194ad6e01bf17d997851e8220ad4af24), closing issue #60. Its exact-main [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31639653701), [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31639653650), and [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31639653748) passed; the [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31639653760) preserved a second upstream GitHub `503` at the same action-owned Syft release lookup. Merged corrective [PR #68](https://github.com/EnjoyableWork/mcp-sync/pull/68) replaces that external staging boundary with pinned, checksum-verified repository scripts; all four exact-main gates pass at [`641da01`](https://github.com/EnjoyableWork/mcp-sync/commit/641da01f20c03ebbac96829d300302e76d843ccd). No product timeout, health assertion, version, release artifact, or side-quest scope changed. |
| D-24 | First verified repeat release | Post-M4 release readiness | Codex | `v0.1.1` | Done | Protected [PR #69](https://github.com/EnjoyableWork/mcp-sync/pull/69) prepares the exact manifest, lockfile, and [release notes](docs/releases/v0.1.1.md); [PR #70](https://github.com/EnjoyableWork/mcp-sync/pull/70) corrects the crates.io Trusted Publisher readback field without weakening its one-record identity. Exact release commit [`123aeff`](https://github.com/EnjoyableWork/mcp-sync/commit/123aeffa51bffb208bfaf7eb72b03559b8467043) passes [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31655091904), [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31655091366), [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31655091961), and [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31655091927). The owner-only control audit confirms the sole exact `EnjoyableWork/mcp-sync` / `cargo-publish.yml` / `release` Trusted Publisher, trusted-publishing-only enforcement, zero Cargo API tokens, exact selected-action policy, and protected release controls before the [authorization run](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31655431617) creates one signed annotated [`v0.1.1`](https://github.com/EnjoyableWork/mcp-sync/tree/v0.1.1) tag at that commit. The [source/GNU/Linux publisher](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31655576060) creates the immutable exact-seven-asset [GitHub Release](https://github.com/EnjoyableWork/mcp-sync/releases/tag/v0.1.1) with complete SHA-256, target SPDX 2.3 SBOM, release-attestation, and build-provenance evidence. The [Cargo publisher](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31655833306) uses OIDC to publish unyanked [`enjoyable-mcp-sync` `0.1.1`](https://crates.io/crates/enjoyable-mcp-sync/0.1.1), proves its `.crate` byte-identical at SHA-256 `ea44931c93eaf89126f5dee1978f3485822d2a36f6acb8c0cc1d5f2def925d49`, and passes all six native Cargo install and recovery jobs without token fallback. The first Homebrew handoff [failed closed](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31656009056) before clone or tap mutation because its protected private key no longer matched a tap deploy key; explicit owner-approved rotation restored exactly one tap-scoped verified write-key pairing and removed temporary local key material. The constrained [retry](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31657298846) created only formula commit [`6af7280`](https://github.com/EnjoyableWork/homebrew-tap/commit/6af7280dce30e08662e4e43767142e6f0d37b898), and the final [13-job read-only channel matrix](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31657404968) passes metadata, two GNU/Linux archive, six Cargo, and four source-building Homebrew native install and recovery journeys. Immutable `v0.1.0`, funded signed-native scope, macOS and Windows project binaries, and WinGet remain unchanged and inactive. |

| D-25 | Tap-owned short-lived Homebrew publication | Post-M4 corrective release maintenance | Codex | 2026-08-13 | Done | Coordinated source [PR #72](https://github.com/EnjoyableWork/mcp-sync/pull/72) and tap [PR #4](https://github.com/EnjoyableWork/homebrew-tap/pull/4) remove the source writer and add one manually dispatched tap-owned workflow. Source exact head [`33b5628`](https://github.com/EnjoyableWork/mcp-sync/commit/33b5628245679f16770d1c8094d9eaf5f87c10c1) and exact main [`24b9598`](https://github.com/EnjoyableWork/mcp-sync/commit/24b9598791733590617410f113620365647be3c1) each pass first-attempt CI, CodeQL, release preflight, and source/install/archive preflight; tap exact head [`bb7e3c4`](https://github.com/EnjoyableWork/homebrew-tap/commit/bb7e3c4f08c80ee8475d0b8925630664036a2b50) and exact main [`bc9ca03`](https://github.com/EnjoyableWork/homebrew-tap/commit/bc9ca0348e693005f202f27b00cb7146f16b54b2) each pass first-attempt policy gates. The protected [`v0.1.1` rehearsal](https://github.com/EnjoyableWork/homebrew-tap/actions/runs/31727044858) independently revalidates the immutable seven-asset release, checksums, both SPDX SBOMs, exact-tag attestations, Cargo bytes, prior formula, greatest stable version, dispatch ref, tap base, permissions, and exact allowed path; its no-write job succeeds while the token-bearing publish job is skipped. After fresh owner confirmation, the source `release` environment secret and tap read/write deploy key were permanently deleted; fresh live readback reports no source environment secrets and no tap deploy keys. The exact-main immutable-distribution verifier passes, the previously blocked `mcp-doctor` organization verifier reports `PASS` at source `04a9a7d52bad67e0b2d1749f20f5ba8f41d802e5`, and the fresh first-attempt [13-job channel matrix](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31729266734) passes metadata plus every represented GitHub, Cargo, and Homebrew native install/restore journey. `Formula/mcp-sync.rb`, `Formula/mcp-doctor.rb`, immutable `v0.1.0` and `v0.1.1`, runtime behavior, and every excluded distribution channel remain unchanged. |
...
| MCP-042 | Contain and clean up the complete health-check process tree | Post-M4 release readiness | P0 | Codex | Done | `MCP-041` | `DEC-050` through `DEC-052` and D-21 classify and close issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44). Implementation commit [`1c96c91`](https://github.com/EnjoyableWork/mcp-sync/commit/1c96c9168b6b8d0f741e512f396ffe68972d095e) adds platform-native containment and removes macOS ancestry-poll timing from cleanup correctness through exact inherited-pipe identity plus a forced-state regression. All 24 hosted checks passed at both implementation and closeout heads on attempt one. Merged [PR #65](https://github.com/EnjoyableWork/mcp-sync/pull/65) closed the issue as exact-main commit [`991e6b2`](https://github.com/EnjoyableWork/mcp-sync/commit/991e6b2114267fb26c3beb3a319e96de0452c7e2), whose [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31612019267), [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31612019238), [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31612019237), and [main control verifier](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31612019507) pass on attempt one. |
| MCP-043 | Reject environment names that cannot retain identity across supported boundaries | Post-M4 release readiness | P0 | Codex | Done | `MCP-042` | `DEC-053` and D-22 are implemented by merged [PR #66](https://github.com/EnjoyableWork/mcp-sync/pull/66), exact implementation head [`9472a7a`](https://github.com/EnjoyableWork/mcp-sync/commit/9472a7ab946f424df8c157ab35fe5eab9f1a6bb5), and exact-main commit [`e96c1c4`](https://github.com/EnjoyableWork/mcp-sync/commit/e96c1c437b5ce9d9f5138b790dd23ad2926fa749). One shared `config.rs` validator rejects empty, NUL-bearing, or `=`-bearing names without normalization before canonical create/replace, sync planning, listing, or process start. `AddRequest::validate` preserves pre-lock/pre-filesystem rejection; canonical parsing protects list, sync, restore validation, and health; Claude Desktop fails structurally; and Cursor, Windsurf, VS Code, Codex, and Kiro preserve invalid local entries as unmanaged, report them during `init`, and reject desired collisions. Focused model, six-adapter, fixture, redaction, valid-edge, and built-binary `init`/`sync`/`test` regressions prove no invalid definition is imported, synchronized, executed, dropped, normalized, disclosed, or allowed to mutate state. Every local, dependency, documentation, packaging, cross-target, native macOS, native GNU/Linux, hosted GNU/Linux/Windows, CodeQL, release-preflight, and source/install/archive gate recorded in D-22 passes without retry. No version or artifact changed; merged PR #66 closed issue #46, and successor `MCP-044` is complete. |
| MCP-044 | Make Windows ARM64 health-fixture readiness deterministic | Post-M4 release readiness | P1 | Codex | Done | `MCP-043` | Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) and D-23 are resolved by merged [PR #67](https://github.com/EnjoyableWork/mcp-sync/pull/67), exact implementation head [`9127775`](https://github.com/EnjoyableWork/mcp-sync/commit/9127775b69f6a710bb8e12a1a10f27409088c68a), and exact-main merge [`31dba46`](https://github.com/EnjoyableWork/mcp-sync/commit/31dba465194ad6e01bf17d997851e8220ad4af24). Preserved recurrence evidence isolates PowerShell interpreter startup and scheduler contention inside the parallel unit binary rather than product protocol or cleanup behavior. Pure response-framing tests plus one sequential native-Rust harness with explicit ready/release state remove that timing dependency while retaining the five-second response, 500-millisecond shutdown, redaction, direct-child, and complete process-tree boundaries. Ten local repetitions and exact-head [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225214), [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634222077), [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225001), and [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225071) pass without a health retry. Exact-main CI, CodeQL, and release preflight also pass; its source/install/archive preflight preserves a second separately classified upstream GitHub `503` during action-owned Syft acquisition. Merged corrective [PR #68](https://github.com/EnjoyableWork/mcp-sync/pull/68) removes that dependency, and all four exact-main gates pass at [`641da01`](https://github.com/EnjoyableWork/mcp-sync/commit/641da01f20c03ebbac96829d300302e76d843ccd). Issue #60 is closed; no product limit was weakened, no version or artifact changed in MCP-044, and no side quest was activated. |
| MCP-045 | Publish and verify `v0.1.1` as the first repeat release | Post-M4 release readiness | P1 | Codex | Done | `MCP-044` | D-24 records the protected preparation, exact-main gates and live control readback, signed annotated exact-main tag, immutable seven-asset GitHub-first publication, tokenless byte-identical Cargo publication, fail-closed tap-key drift and explicitly approved scoped recovery, monotonic Homebrew update, and complete 13-job native installed-channel matrix. The current-release distribution, repository supply-chain, Cargo-control, and public-project verifiers now bind `v0.1.1` while retaining the immutable `v0.1.0` first-publication baseline and artifact-scoped assurance statement. Release notes provide explicit Homebrew, Cargo, and verified GNU/Linux archive upgrade paths and preserve every current limitation. No tag or immutable asset moved, no reusable token, project-issued macOS or Windows binary, WinGet package, paid signing, successor, or side quest was created or activated. |

| MCP-046 | Replace the shared-tap deploy key with tap-owned short-lived publication | Post-M4 corrective release maintenance | P0 | Codex | Done | `MCP-045` | D-25 records the coordinated protected source and tap changes, deterministic authority regressions, first-attempt exact-head and exact-main gates, protected no-write/no-op `v0.1.1` rehearsal, fresh action-time owner confirmation, permanent deletion of both credential halves, zero live source-secret and tap-key inventories, exact immutable-distribution verification, the restored `mcp-doctor` organization-control pass, and the complete first-attempt 13-job native channel matrix. The only Homebrew write job is tap-owned, receives only its job-scoped `GITHUB_TOKEN`, stages exactly `Formula/mcp-sync.rb`, and rejects every sibling or extra path; the source retains no cross-repository Homebrew authority. No version, tag, release, crate, formula content, signed artifact, WinGet package, runtime behavior, successor, or funded `SIDE-006` scope changed or activated. |
...
| MCP-043 | Complete MCP-043: fix GitHub issue #46 by enforcing one portable canonical environment-name contract that rejects empty names and names containing NUL or `=` before canonical state is created, replaced, synchronized, listed, or used to start a process, and prevents those names from entering canonical state through native import. Follow AGENTS.md, PROJECT.md, DEC-050, and DEC-053; preserve unrelated worktree changes, canonical v1 ordering and serialization, validation-before-lock and validation-before-filesystem behavior, each client adapter's existing ownership and unmanaged-entry contract, mutation locking and recovery, structural redaction, synthetic-home isolation, all six targets, and immutable v0.1.0. Implement one shared environment-name validator in `config.rs` used by canonical model validation and native translation. `AddRequest::validate` and canonical parsing must fail before mutation or process start. Claude Desktop must return a structural invalid-canonical error for an otherwise-local invalid entry; Cursor, Windsurf, VS Code, Codex, and Kiro must preserve such an entry structurally under their existing round-trip contracts, report it as skipped during `init`, and reject any colliding desired canonical server during sync. Never normalize the name, import or execute the invalid definition, or silently drop native data. Returned validation errors may expose only client/server context, deterministic key position, and the violated rule; unmanaged reports retain their existing client/server-only redaction, and neither path may disclose invalid key text or environment values. Add focused model cases for empty, NUL, and `=` names; prove empty CLI `--env` input fails before lock or managed-state access; add canonical and all-six-adapter fixtures plus built-binary `init`, `sync`, and `test` regressions that prove Claude failure, five-target unmanaged preservation/collision behavior, and no unintended creation, change, or execution; and retain valid edge names and existing structural redaction. Do not add schema migration infrastructure, a new parser, dependency, speculative wrapper type, or platform-specific canonical rule; change unrelated validation; conflate issue #44 or #60; bump a version; publish any artifact; activate SIDE-006; or change immutable v0.1.0. Finish only when focused, built-binary, local quality, dependency, documentation, and native Unix/Windows gates pass, durable evidence is recorded, MCP-043 and D-22 are Done, issue #46 is ready for closure, only MCP-044 is Ready, and no side quest is implicitly activated. |
...
| MCP-046 | Complete MCP-046: eliminate `mcp-sync`'s long-lived cross-repository Homebrew authority by replacing the protected private-key secret and repository-wide `homebrew-tap` deploy key with a manually dispatched, tap-owned publication workflow whose write job uses only the tap repository's job-scoped `GITHUB_TOKEN`. Follow AGENTS.md, PROJECT.md, completed MCP-041 and MCP-045, and both release runbooks; preserve unrelated worktree changes, immutable v0.1.0 and v0.1.1 tags and assets, exact Trusted Publishing, the GitHub-first seven-asset boundary, checksums, SPDX SBOMs, attestations, monotonic formula policy, release serialization, read-only channel verification, and dormant funded SIDE-006. Independently revalidate the exact immutable upstream release, release and Cargo bytes, prior formula, candidate transition, dispatch ref, and allowed path before tap write authority exists; permit only `Formula/mcp-sync.rb` to change and reject any sibling-path, extra-file, stale-version, mismatch, credential, ref, or permission drift. Add deterministic source and tap policy tests plus a protected no-write/no-op v0.1.1 rehearsal; never use a PAT, GitHub App credential, deploy key, automatic cross-repository dispatch credential, test/workflow retry, or timing-based proof. Do not bump a version, create or move a tag, publish or replace a release, crate, formula, macOS/Windows artifact, or WinGet package, change runtime behavior, mutate `Formula/mcp-doctor.rb`, or activate SIDE-006. Permanently delete the mcp-sync protected-environment secret and tap deploy key only after the replacement has merged, passed its rehearsal, and received fresh action-time owner confirmation. Finish only when relevant local checks, protected exact-head and exact-main CI, CodeQL, release preflight, and source/install/archive preflight pass on their first attempt; live readback proves zero source Homebrew secrets and zero tap deploy keys; the mcp-sync release verifiers and blocked mcp-doctor organization-control verifier pass; durable evidence is recorded; MCP-046 and D-25 are Done; and no successor or side quest is implicitly activated. |
...
1. Maintain public `v0.1.1` as the current version across the immutable GitHub
   Release, unyanked Cargo package, source-building Homebrew formula, and
   version-bound live verifiers. Treat any cross-channel drift as a release
   incident and never repair it by moving a tag or replacing an immutable
   asset.
2. Preserve immutable `v0.1.0` as the first-publication baseline and `v0.1.1`
   as the first repeat-release proof. Keep the manual Homebrew, Cargo, and
   verified GNU/Linux archive upgrade guidance and the absence of project-issued
   macOS/Windows binaries and WinGet accurate.
3. Keep every main-story ticket and side quest inactive after completed
   `MCP-046`. Start a successor only through an intentional tracker update and
   matching Goal; keep funded signed-native `SIDE-006` proposed and dormant.
4. Maintain completed `MCP-041`'s canonical later-version policy,
   ...
   nonpublishing rehearsal through completed `MCP-046`'s tap-owned publisher.
   Keep the source Homebrew-secret and tap deploy-key inventories at zero, and
   never restore the removed long-lived pairing for a later release.
5. Maintain completed `MCP-036`'s serialization of `init`, `add`, non-dry-run
...
6. Maintain the completed `SIDE-007` README product-page hierarchy, live
...
7. Maintain `SIDE-008`'s single pull-request CI suite, exact protected check
   names, and test-only Windows fixture headroom without weakening production
   health limits or native x64/ARM64 coverage.
8. Maintain the completed `MCP-030` through `MCP-035` controls, public evidence,
...
9. Maintain completed `MCP-039`'s sole exact trusted-publisher identity,
...
10. Treat OSPS Level 1 as the only M3 assurance gate. Best Practices, NIST SSDF,
...
11. Retain the funded six-target signed-native workflow, preflight, metadata,
...
| DEC-050 | Resolve known correctness and evidence defects before the first repeat release | Accepted | 2026-08-11 | The repository classified three defects that must not be carried silently into the first later version. Issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44) proved direct-child termination did not contain a detached inherited-stdio descendant and is closed by completed `MCP-042`. Issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46) proved canonical environment names could validate and synchronize while changing identity at the process boundary; completed `MCP-043` implements the portable fail-before-mutation rule, and merged PR #66 closed the issue. Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) satisfied `RISK-05` and `RISK-24` recurrence triggers; completed `MCP-044` provides deterministic no-retry Windows ARM64 evidence, merged PR #67 closed the issue, and PR #68 removed the separately exposed nondeterministic Syft acquisition boundary. The owner activated `MCP-045` under its exact Goal and explicitly approved publication of `v0.1.1` as the first production proof of the repeat-release path. `MCP-043` tightens canonical v1 validation for names that never had consistent portable identity and does not activate general migration infrastructure. `MCP-044` closes through deterministic state and repeated evidence, never through an unexplained green health retry. Completed `MCP-045` and D-24 prove immutable GitHub-first publication, exact Trusted Publishing, monotonic Homebrew, manual channel-managed upgrades, zero-cost artifacts, and every existing authority boundary in production. The fail-closed tap-key drift and owner-approved like-for-like rotation changed no accepted authority scope. Deferred expansion candidates and funded signed-native `SIDE-006` remain inactive. |
...
| DEC-052 | Treat timing-dependent CI variance as a defect and require deterministic evidence | Accepted | 2026-08-12 | Product time limits and outer test or job watchdogs remain necessary to bound failure, but elapsed time, fixed sleeps, fast polling, runner speed, and retries are not synchronization or acceptance evidence. An asynchronous or process fixture must expose observable readiness or an explicit event/acknowledgement handshake; eventual-state polling succeeds on the state itself under one outer deadline. A failure followed by an identical-source pass remains an unresolved nondeterminism signal and must be classified in the owning ticket or risk before merge, with the failed attempt preserved. Corrections must address the lowest timing-dependent contract and add one deterministic forced-interleaving or state-transition regression; repetition and stress are supplemental only. Increasing a product timeout, weakening assertions, skipping or quarantining a safety test, adding automatic retries, or broadly serializing the suite is prohibited as a green-CI remedy. Focused serialization remains permitted only for a demonstrated exclusive resource. Built-binary and native-host tests remain appropriate for real process, filesystem, packaging, and operating-system contracts after separable behavior is proved at a narrower layer. This decision governed completed `MCP-042` and `MCP-044`; merged PR #67 closed issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) without changing a product limit. Its preserved exact-main source-preflight `503` exposed the same nondeterministic action-owned Syft acquisition boundary seen during review; merged PR #68 replaces it with repository-owned pinned and checksum-verified acquisition plus deterministic policy tests, and all four exact-main gate families pass without erasing either failure. |
| DEC-053 | Reject environment names outside the portable canonical identity intersection | Accepted | 2026-08-12 | Canonical v1 environment names must be non-empty and contain neither NUL nor `=`. NUL cannot cross process APIs, an empty name is not portable, and `=` is the `key=value` process-environment delimiter that issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46) proves can silently change key identity on Unix; accepting any of them contradicts one client-independent canonical model. `MCP-043` therefore adds one shared environment-name validator in `config.rs`, tightens `CanonicalConfig` without normalizing names or changing schema version 1, and reuses the same predicate during native translation. `AddRequest::validate` rejects before operation-lock acquisition or filesystem access; canonical parsing rejects before list, sync planning, or health process start. Native behavior follows existing ownership boundaries: Claude Desktop has no unmanaged-local classification and returns a structural invalid-canonical error; Cursor, Windsurf, VS Code, Codex, and Kiro classify an otherwise-local invalid entry as canonical-unrepresentable, preserve it structurally under their existing round-trip contracts, report it as skipped during `init`, and refuse a colliding desired canonical server. Thus no invalid definition is imported, synchronized, or executed, while unrelated native data remains available. Returned validation errors identify only client/server context, deterministic key position, and whether the name is empty or contains a forbidden delimiter; unmanaged reports retain their existing client/server-only redaction. Neither path includes invalid key text or any environment value. This is a validation correction for data that never had stable portable meaning, not a schema migration. It adds no parser, dependency, platform-specific branch, normalization, general migration machinery, speculative wrapper type, or prohibition on other names that supported process boundaries can retain. |
| DEC-054 | Move source-built Homebrew publication into the tap with job-scoped authority | Accepted | 2026-08-13 | `MCP-045` proved the immutable GitHub-first release and exact Cargo/Homebrew handoff, but its source-owned private key mapped to a repository-wide write deploy key on the shared tap and therefore exceeded the single-formula ownership boundary. Completed `MCP-046` replaces that reusable cross-repository authority with one manually dispatched `EnjoyableWork/homebrew-tap` workflow running from exact tap `main`. Its unprivileged job independently verifies the signed annotated upstream tag, immutable exact-seven-asset release, release digests, complete checksums, both SPDX SBOMs, exact-tag attestations, trusted-publishing-only crates.io identity, byte-identical Cargo package, prior formula's own immutable release, greatest stable registry version, and strictly monotonic candidate before any write permission exists. The protected no-write path is fixed to an exact `v0.1.1` no-op. Only the tap-local publish job receives `contents: write` through its short-lived repository-scoped `GITHUB_TOKEN`; it revalidates the retained handoff and exact tap base, stages only `Formula/mcp-sync.rb`, rejects any sibling, extra, stale, mismatched, symbolic, malformed, credential, ref, permission, or non-fast-forward drift, and shares one non-cancelling queue with every tap formula writer. The source repository retains no tap mutation workflow or automatic dispatch credential. Both coordinated changes merged and the protected rehearsal passed before the owner gave fresh action-time confirmation; the old protected secret and tap deploy key were then permanently deleted, zero live inventories and both affected projects' verifiers passed, and no version, tag, release, crate, formula byte, macOS/Windows artifact, WinGet package, runtime behavior, or funded `SIDE-006` scope changed. |
...
completed `MCP-042` process containment, completed `MCP-043` portable
environment-name validation, completed `MCP-044` deterministic Windows ARM64
health-fixture evidence, and completed `MCP-045` publication of `v0.1.1`.
`MCP-042` selected and proved the
supported-platform containment mechanism through exact-head no-retry evidence.
`DEC-053` fixes `MCP-043`'s portable name set, shared validation seam,
fail-closed native behavior, redacted diagnostic contract, and no-migration
boundary; no implementation decision remains open.
`DEC-054` fixes `MCP-046`'s manual tap-owned dispatch, independent upstream
verification, protected no-write rehearsal, single-formula staging, tap-wide
serialization, job-scoped write token, and deletion-after-confirmation
boundary; implementation, protected evidence, owner-confirmed credential
removal, zero live inventories, and both affected verifiers pass.
`MCP-029`, M2, `MCP-030` through `MCP-037`, `MCP-039` through `MCP-046`, M3,
bounded M4 expansion, `SIDE-007`, and `SIDE-008` are complete. No main-story
ticket or side quest is active. The
...
activated and completed only `MCP-042`; subsequent exact Goals activated and
completed only `MCP-043`, then `MCP-044`, and finally `MCP-045` under its exact
zero-cost publication boundary. Completing the first repeat release activated
no successor or side quest.
`DEC-031`, `DEC-036`, and
...
| RISK-05 | Health checks hang or leave child processes running | High | High | The only execution boundary still enforces a five-second response limit, 1-MiB message limit, 500-millisecond clean-shutdown allowance, direct-child force termination and reap, and a drop backstop. Issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44) proved the released direct-child boundary could leak a session-escaping inherited-stdout descendant. Completed `MCP-042` and `DEC-051` add a suspended Windows Job Object, monitored Unix process group, Linux subreaper/PID descriptors, exact macOS start identities and inherited-pipe discovery, deadline-polled reap, retained reader ownership, and cleanup on success, failure, setup, and drop. Merged PR #65's [first macOS x64 failure](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31602718822/job/94133885654) proved a rapid intermediary fork, exit, and reparent could cross between ancestry snapshots and retain stdout until reader cleanup timed out. Implementation commit [`1c96c91`](https://github.com/EnjoyableWork/mcp-sync/commit/1c96c9168b6b8d0f741e512f396ffe68972d095e) records the exact stdout writer and a pre-spawn identity baseline, scans only post-spawn exact identities for that writer during cleanup, and adds a forced-state case that erases monitor history after explicit intermediary-exit readiness. Linux process inspection continues to treat volatile `ENOENT`, `ESRCH`, and permission races as disappearance while retaining unrelated monitor failures. Four command outcomes plus setup-failure, returned-cleanup-failure drop-retry, ordinary direct-drop, reader-ownership, and macOS pipe-only cases prove bounded cleanup and structural redaction without changing a product limit. All 24 exact-head checks, including the formerly failing [macOS x64 job](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31609243085/job/94156030891), pass on their first attempt. Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) separately recorded two response-producing Windows ARM64 PowerShell fixtures that missed their startup allowances after completed `SIDE-008`; completed `MCP-044`, `DEC-050`, and `DEC-052` replace that boundary with deterministic native readiness and attempt-one complete x64/ARM64 CI evidence | A contained process or reader survives any return path; a platform primitive permits escape; cleanup exceeds its bound; diagnostics disclose process or configuration details; exact source needs a retry; the Windows ARM64 timing failure recurs; or a product limit or assertion is weakened to make tests pass | Mitigated — `MCP-042` and `MCP-044` are complete, issues #44 and #60 are fixed, and deterministic native evidence passes without a health retry |
...
| RISK-10 | The product or package is confused with unrelated projects using the `mcp-sync` name | High | Medium | Distinct package [`enjoyable-mcp-sync`](https://crates.io/crates/enjoyable-mcp-sync/0.1.1) is published from the immutable repository release, while install commands remain publisher-qualified and the installed executable remains `mcp-sync`; provenance links and exact-byte verification bind the identities | Users install the unrelated `mcp-sync` crate or cannot connect `enjoyable-mcp-sync` to this repository | Mitigated for registry identity — monitor install feedback |
...
| RISK-16 | A single organization owner becomes unavailable or an over-broad long-lived credential becomes a recovery dependency | High | Low | The 2026-08-09 aggregate audit and complete non-disclosing verifier prove organization-required secure-method 2FA, base access `none`, disabled member repository creation, read-only workflow-token defaults, no organization or repository Actions secrets, one protected-environment secret, and one verified tap-only write deploy key. The `v0.1.1` handoff exposed that the protected private half no longer had a public counterpart; the workflow failed before clone or mutation, and explicit owner-approved rotation restored exactly one scoped pairing, verified it non-disclosingly, and removed temporary local material before retry. The sole owner explicitly accepts the residual risk, privately confirmed the installed-application review and successful recovery exercise, and must add a genuinely trusted second owner when one exists | The sole owner loses access, a maintainer cannot recover administration, a future member receives implicit private-repository access or creates an unreviewed repository, an installed application retains unnecessary scope, the key halves drift again, or one long-lived credential becomes necessary for routine automation | Mitigated with accepted single-owner residual risk — reverify on every trigger and add a trusted second owner when available |
| RISK-17 | A release tag or publication credential is changed or misused before immutable release controls take effect | Critical | Low | The active `v*` ruleset restricts tag creation, update, and deletion; both protected environments require review and constrain deployment to exact `main` or `v*` refs; release immutability is enabled; and `DEC-031` carries those controls through the zero-cost releases and later `SIDE-006`. Protected authorization bound `v0.1.0` and `v0.1.1` separately to their exact release commits. For `v0.1.1`, the tag push published nothing, the manually selected source/GNU/Linux producer created the exact immutable release first, Cargo used only short-lived OIDC, and Homebrew read its scoped credential only after upstream verification. The first Homebrew run failed before clone or mutation on key drift; the approved like-for-like rotation and constrained retry created only the monotonic formula commit. Both immutable releases, current crate and formula bytes, and the final read-only native matrix are independently verified | Release automation creates, moves, deletes, or publishes a protected tag outside its exact authorized purpose; recovery accepts a stale commit or another ref; a zero-cost tag invokes paid signing; an approval proceeds after failed controls; or a downstream credential can mutate broader state | Mitigated — two protected immutable releases and constrained recovery are verified; monitor future releases |
| RISK-18 | Source-based macOS and Windows installation creates more setup friction | Medium | High | Current `v0.1.1` offers native GNU/Linux archives, Cargo installation across supported platforms, and source-built Homebrew on macOS/Linux; release notes state prerequisites, explicit upgrade paths, and limitations, and all thirteen represented native install and recovery jobs pass. The funded native path remains available through explicit funding and activation of `SIDE-006`; installation or adoption evidence from either zero-cost release is not a prerequisite for `SIDE-006` | A supported installation path fails, its prerequisites or limitations become inaccurate, or the project explicitly funds and activates signed-native distribution | Open — monitor installed-channel correctness and keep limitations current; no adoption-evidence gate |
...
| RISK-21 | A future Cargo version relies on a reusable token or an over-broad trusted-publisher identity | Critical | Low | `DEC-047` and completed `MCP-039` enforce one dedicated workflow, exact repository/workflow/environment identity, protected-environment review, publish-job-only OIDC, a reviewed SHA-pinned official authentication action, trusted-only crates.io enforcement, immutable-GitHub-first exact-byte verification, no stored token, and no fallback. The first-publication token remains revoked, the aggregate API-token inventory is zero, and no successor token is authorized. The sole `v*` environment rule and exact live verifier pass; protected [run `31655833306`](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31655833306) then used that short-lived path to publish `0.1.1`, preserved `trustpub_only: true`, and proved release/registry byte identity plus all six native Cargo installs | A crates.io token is created, stored, or requested; the publisher omits the `release` environment or names a broader repository/workflow identity; OIDC reaches untrusted code; the trusted-only setting is disabled; authentication failure triggers token fallback; local, release, and registry `.crate` bytes are not equal; or a later version bypasses any protected publication gate | Mitigated — tokenless exact-identity path implemented and production-verified; recheck before every later Cargo version |
| RISK-22 | A later stable release deadlocks, republishes inconsistent bytes, or cannot recover after partial channel publication | High | Medium | Issue [#62](https://github.com/EnjoyableWork/mcp-sync/issues/62) records the fixed-`v0.1.0` triggers, GitHub/crates.io circular dependency, and reject-on-existing Cargo retry gap. Accepted `DEC-049`, completed `MCP-041`, and D-20 implement canonical monotonic stable versions, credential-free request rejection, manual single-producer selection, immutable GitHub-first exact assets, retry-safe registry equality with OIDC/publish skip, monotonic prior-release-backed Homebrew updates, generalized read-only channel verification, one global release serialization boundary, and a nonpublishing `v0.1.0` rehearsal. D-24 production-proves that lifecycle with `v0.1.1`: exact-main authorization, one inert tag push, GitHub-first publication, byte-identical Cargo, fail-closed Homebrew credential drift, constrained monotonic recovery, and a green 13-job final matrix. Both immutable releases remain unchanged | A later tag cannot reach an immutable release before Cargo publication; a stale, prerelease, mismatched, or older version passes; a partial Cargo upload cannot safely resume; registry, release, or downstream bytes differ; a generalized workflow accidentally invokes funded signing or mutates an immutable release; or rehearsal changes public state | Mitigated — repeatable lifecycle is implemented and production-verified; recheck before every stable release |
| RISK-23 | A canonical environment name changes identity or becomes invalid at a supported process boundary | High | Medium | Completed `MCP-043`, `DEC-053`, and D-22 enforce the portable non-empty/no-NUL/no-`=` invariant through one shared `config.rs` validator. The traced consumers cover validation-before-lock `add`, canonical reads, all six native adapters before `init` creation or sync planning, restore validation, and health before process launch. Existing ownership variation is explicit: Claude fails structurally, while Cursor, Windsurf, VS Code, Codex, and Kiro preserve invalid local entries as unmanaged and reject collisions. Focused model, adapter, fixture, redaction, valid-edge, and built-binary regressions plus native macOS, GNU/Linux, and Windows x64/ARM64 evidence prove the boundary without normalization, mutation, execution, dropping, or disclosure. Protected PR #66 exact implementation head `9472a7a` passes all 24 hosted checks on attempt one, and exact-main commit `e96c1c4` passes all 23 checks on attempt one | `add`, canonical parsing or construction, native import, sync, restore validation, list, or `test` accepts a name that cannot retain exact identity on macOS, GNU/Linux, or Windows; an invalid name is normalized, imported, executed, or dropped; an unmanaged native entry is overwritten; a command mutates before rejection; or a diagnostic exposes invalid key text or an environment value | Mitigated — `MCP-043` is complete, merged PR #66 closed issue #46, and native regressions remain required |
| RISK-24 | Scheduler-dependent tests hide regressions behind retries or generous timing | High | High | `DEC-052` makes deterministic synchronization and no-retry evidence repository policy. The historical CI audit distinguished repeatable implementation failures from identical-source variance in process health fixtures, including issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) and `MCP-042`'s macOS x64 failure where a reparented inherited-stdout holder crossed between ancestry snapshots. Completed `MCP-044` isolates issue #60 to generated PowerShell startup inside a parallel unit binary, removes interpreter startup from that boundary, retains pure framing proof, and uses a sequential native process harness with an explicit ready/release transition; ten supplemental Windows ARM64 repetitions remain fail-fast within the existing protected CI context. Product time limits and outer watchdogs remain bounded, and the complete native x64/ARM64 matrix passes on attempt one | Identical source both passes and fails; a race or timeout disappears only on rerun; correctness depends on a sleep, polling frequency, runner speed, or broader timeout; automatic retry, quarantine, assertion weakening, or unjustified serialization is proposed; or a native boundary lacks a deterministic lower-layer proof | Mitigated — deterministic lower-layer correction, forced state transition, and complete no-rerun native hosted evidence pass; retain regressions and investigate any recurrence |

| RISK-25 | A long-lived `mcp-sync` deploy key can mutate sibling formulas in the shared Homebrew tap | Critical | Medium | Completed `MCP-046`, D-25, and `DEC-054` move formula publication into a manually dispatched tap-owned workflow, grant `contents: write` only to its tap-local job-scoped `GITHUB_TOKEN`, independently validate immutable upstream and monotonic transition evidence, permit exactly `Formula/mcp-sync.rb`, and reject every sibling or extra path. Both long-lived credential halves were removed only after coordinated protected merges, first-attempt gates, a protected no-write rehearsal, and fresh owner confirmation. Live readback reports zero source Homebrew secrets and zero tap deploy keys; immutable-distribution, 13-job channel, and sibling organization-control verifiers pass. Existing immutable `v0.1.0` and `v0.1.1` artifacts remain unchanged. | A source-repository secret, PAT, App token, deploy key, or other reusable cross-repository writer returns; the tap workflow can alter a sibling path; a later release bypasses the tap-owned job token; or either project's affected live verifier fails | Mitigated — tap-owned short-lived authority is implemented and verified, both reusable credential halves are permanently deleted, and the blocked sibling verifier passes; recheck the zero inventories and exact path boundary before every later Homebrew publication |
