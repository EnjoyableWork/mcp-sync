...
| Current milestone | M3 — Enterprise assurance and adoption, active |
| Overall status | M2 and `MCP-030` are `Done`; `MCP-031` is `In progress` under its exact Goal with Codex as owner; `MCP-032` through `MCP-035` remain proposed and dormant; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Complete `MCP-031` by publishing the safe vulnerability-reporting contract, activating every available zero-cost repository security control, recording exact feature limitations and non-sensitive baseline evidence, and advancing only `MCP-032` after the ticket-done gate passes |
| Milestone target | Unscheduled |
| Last reviewed | 2026-08-09 |
| Next review trigger | `MCP-031` activation, completion, or blocker; a `main` ruleset/check-context change; an assurance-framework revision; a security or release incident; evidence drift; or explicit funding and activation of `SIDE-006` |
...
1. Complete only active `MCP-031` under its exact Goal while preserving the
   completed `MCP-030` controls. Do not expose a security finding or contact
   credential, and do not add dependency-update automation owned by
   `MCP-033`.
2. Complete `MCP-031` through `MCP-035` in order. Do not activate a successor
...
`MCP-030`'s approval, merge-method, bypass, and signed-commit choices, and
`DEC-040` fixes the `MCP-031` disclosure and zero-cost feature boundary. `MCP-029`,
M2, and `MCP-030` are complete; `MCP-031` is the only active product ticket
under its exact Goal. Organization continuity and recovery choices remain scoped
to later `MCP-034`; no other M3 ticket is implicitly active. `DEC-031`, `DEC-036`, and
...
| RISK-14 | Contributors publicly disclose a vulnerability, credential, or unsafe diagnostic because reporting routes are unclear or unavailable | High | Medium | Private vulnerability reporting, provider-pattern secret scanning, push protection, dependency alerts and security updates, and CodeQL are live and generically baseline-clean. The prepared root security policy defines the private route, safe public fallback, redacted evidence, security contacts, response targets, and coordinated disclosure; GitHub recognition awaits its protected merge. Supplemental non-provider patterns and validity checks are unavailable on the current free plan and documented exactly. Structured issue routing remains `MCP-032` scope | A security report appears in a public issue, a pushed secret is accepted, the private route or security policy becomes unavailable, an open finding appears, or GitHub feature entitlement changes | Mitigation active — complete `MCP-031` recognition, then `MCP-032` issue routing |
| RISK-15 | Mutable Actions references or unmaintained dependencies compromise or silently weaken CI | High | Medium | CI is least-privilege, checkout credentials are not persisted, `cargo deny` runs, and its action is SHA-pinned. Dependabot security updates and alerts are enabled and baseline-clean. Checkout remains tag-pinned, repository SHA enforcement is off, all actions are allowed, and no scheduled version-update configuration exists | An action tag moves unexpectedly, an actionable advisory remains unpatched, or dependency updates stop arriving | Open — `MCP-033` hardening and scheduled-update mitigation |
