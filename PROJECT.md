...
| Current milestone | Post-M3 corrective maintenance — `MCP-036` is the deliberately selected next main-story ticket |
| Overall status | M0 through M3, `MCP-001` through `MCP-035`, `SIDE-007`, and `SIDE-008` are `Done`; `MCP-036` is `In progress` as the sole P0 user-configuration safety correction, no side quest is active, and funded `SIDE-006` remains proposed |
| Current focus | Prevent issue [#45](https://github.com/EnjoyableWork/mcp-sync/issues/45) by serializing every mutating operation per canonical configuration root without weakening validation, rollback, redaction, dry-run, platform, or immutable-release guarantees |
| Milestone target | `MCP-036` ticket-done gate; no product release is authorized or required |
...
The main story is complete through M3. `MCP-036` is the deliberately selected
P0 corrective continuation because issue #45 invalidates a user-configuration
safety invariant; it is not optional M4 expansion:
...
explicit adoption outcome rather than optional work. `SIDE-006` remains the
funding-dependent signed-native distribution stretch and does not gate M3 or
the completed `SIDE-007`. The deferred post-M3 items are expansion candidates,
not executable work. Before one starts, classify it as the next ordered
main-story arc or as a genuinely independent `SIDE-NNN` ticket.
...
| D-16 | Cross-process serialization for every mutating configuration operation | Post-M3 corrective maintenance | Codex | `MCP-036` ticket-done gate | In progress | Open issue [#45](https://github.com/EnjoyableWork/mcp-sync/issues/45) deterministically proves that overlapping sync processes can leave a mixed target generation after one reports success. `MCP-036`, `DEC-045`, and `RISK-20` define the fail-fast per-root lock, command boundary, concurrency regressions, and cross-platform evidence required to restore the transaction guarantee without changing immutable `v0.1.0`. |
...
| MCP-036 | Serialize every mutating operation per canonical configuration root | Post-M3 corrective maintenance | P0 | Codex | In progress | `MCP-035` | Open issue [#45](https://github.com/EnjoyableWork/mcp-sync/issues/45) is fixed only when one fail-fast cross-process lock covers pre-mutation planning through apply and rollback for `init`, `add`, non-dry-run `restore`, and non-dry-run `sync`; contention exits non-zero with an actionable structurally redacted diagnostic and no managed-state read or write; invalid `add` input still fails before filesystem access; read-only and dry-run commands remain non-mutating; the original two-sync stale-generation interleaving, all mutating command paths, release-on-exit, independent roots, coherent backups, and final five-target convergence have focused deterministic regressions; macOS, GNU/Linux, and Windows native surfaces plus the full quality and dependency gates pass; durable evidence is recorded; immutable `v0.1.0` is unchanged; and issues #44, #46, and #47 remain separate. |
...
1. Complete `MCP-036` under its exact active Goal: serialize `init`, `add`,
   non-dry-run `restore`, and non-dry-run `sync` per canonical configuration
   root from before managed-state reads through apply and rollback, then prove
   issue #45 cannot recur without activating a release or adjacent defect.
2. Maintain the completed `SIDE-007` README product-page hierarchy, live
...
3. Maintain `SIDE-008`'s single pull-request CI suite, exact protected check
...
7. Monitor the public GitHub, Cargo, and source-building Homebrew channels and
...
8. Treat OSPS Level 1 as the only M3 assurance gate. Best Practices, NIST SSDF,
...
9. Retain the funded six-target signed-native workflow, preflight, metadata,
...
to issue #45.
`MCP-029`, M2, `MCP-030` through `MCP-035`, M3, `SIDE-007`, and `SIDE-008` are
complete; `MCP-036` is the sole `In progress` corrective main-story ticket. The
completed side quests changed only the README product-page
presentation and CI reliability; neither activated a later main-story arc nor
altered release, assurance, or runtime health scope.
`MCP-034`'s
...
satisfy the final M3 gate. No side quest or successor to `MCP-036` is
implicitly active.
`DEC-031`, `DEC-036`, and
...
| RISK-20 | Overlapping mutating processes leave a mixed generation or invalidate another successful transaction | Critical | Medium | Issue [#45](https://github.com/EnjoyableWork/mcp-sync/issues/45) deterministically demonstrates that sync B can report success before sync A's later rollback restores one target to stale bytes. Exact-byte guards protect each write from uncoordinated external changes but cannot express transaction ownership across cooperating `mcp-sync` processes. `DEC-045` fixes the required shared boundary and `MCP-036` is `In progress` with no release dependency | Two same-root mutating invocations overlap; a command reads managed state before owning the lock; contention waits, leaks details, or mutates state; process exit leaves an unusable stale lock; different roots block each other; or any native platform lacks equivalent proof | Open — blocks the concurrent-mutation safety claim until `MCP-036` passes |
