...
| Current focus | Merge the `MCP-029` explicit-crates.io-client correction and exact protected Homebrew recovery path, repeat fresh exact-main CI and both preflights, authorize the one operational recovery tag without moving `v0.1.0` or widening the `release` environment, publish the byte-identical source formula, and complete the read-only native channel matrix without activating paid signing or deferred channels |
| Milestone target | 2026-08-08 |
| Last reviewed | 2026-08-08 |
| Next review trigger | Merge of the `MCP-029` Homebrew recovery correction; publication and verification of the exact tap formula; completion of the final native channel matrix; or intentional activation of a side quest |
...
| D-12 | Zero-cost market-validation release and installed recovery exercise | M2 | Codex | 2026-08-08 | In progress | Exact release commit [`0836590`](https://github.com/EnjoyableWork/mcp-sync/commit/08365900497bf6fde3749695078794e0f3ecf790) passes [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31283240167), [source and GNU/Linux preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31283240166), and retained [six-target preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31283240132); its protected [authorization](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31283455962) succeeds after operator verification. Annotated tag `v0.1.0` resolves to that commit, and the [source and GNU/Linux publisher](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31284055636) publishes the [immutable seven-asset GitHub Release](https://github.com/EnjoyableWork/mcp-sync/releases/tag/v0.1.0) with verified release/build attestations, checksum manifest, and only GNU/Linux x64/ARM64 native archives. [`enjoyable-mcp-sync` `0.1.0`](https://crates.io/crates/enjoyable-mcp-sync/0.1.0) is public, not yanked, and registry-download byte equality with the immutable release asset passes; the one-use `publish-new` token is revoked server-side and removed locally. The protected [Homebrew publisher](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31284313941) fails before reading its deploy key or mutating the tap because its crates.io download omitted the explicit project user agent and received `403`; the tap formula remains absent. `DEC-037` fixes a one-time protected operational recovery tag at a separately approved exact-main correction commit, with all three gates, the unchanged tag-only environment, immutable `v0.1.0` provenance, and byte equality required before the existing tap-scoped key can create only `Formula/mcp-sync.rb`. The ticket remains open for that corrective merge, protected recovery publication, four native Homebrew and six native Cargo installs, two GitHub archive smokes, recovery journeys, durable tap evidence, and the final channel matrix without project-issued macOS or Windows binaries or WinGet |
...
2. Merge the explicit-user-agent correction for every crates.io request plus
   the exact `v-mcp-029-homebrew-recovery-1` fail-closed publisher path. Require
   fresh exact-main CI and both preflights, rerun the operator repository-control
   verifier, and obtain separate owner authority before creating the annotated
   operational recovery tag. Do not move `v0.1.0`, replace an immutable asset,
   widen the tag-only `release` environment, or create another product release.
3. Follow the [source and GNU/Linux release runbook](docs/source-linux-release.md)
   to publish only the absent byte-identical `v0.1.0` formula with the existing
   repository-scoped tap deploy key, then run the read-only published-channel
   workflow across two GitHub archive, six Cargo, and four Homebrew native
   install and recovery paths. Keep the revoked first-publication credential
   absent from local and hosted automation.
...
| DEC-037 | Recover post-publication automation without changing immutable `v0.1.0` | Accepted | 2026-08-08 | The first protected Homebrew dispatch failed before credential use or tap mutation because one crates.io download used curl's unidentified default client and received `403`; the immutable GitHub Release and crates.io bytes remain correct. Rerunning the frozen tag workflow would repeat the failure, while moving `v0.1.0`, replacing assets, manually bypassing the tap-scoped key, or widening the protected environment would weaken accepted controls. After the explicit-client correction merges and exact-main CI plus both preflights pass, the separately authorized annotated operational tag `v-mcp-029-homebrew-recovery-1` may point only to that exact current `main` commit. It triggers no release publisher itself and is not a product version. A manual Homebrew dispatch from that exact tag remains behind the tag-only `release` environment, rechecks the public tag ruleset and all three recovery-commit gates, then checks out and publishes only the absent byte-identical formula from immutable `v0.1.0`. The operational tag is retained without a GitHub Release or Cargo version and is never moved or deleted; future registry requests always identify the project client. |

### Open decisions

`OPEN-07` is resolved by `DEC-028`, `DEC-036` resolves the release-funding
sequence, and `DEC-037` fixes the one-time immutable-release recovery path. No
open product decision currently blocks `MCP-029`; `DEC-031`, `DEC-036`, and
`DEC-037` fix its release boundary. Protected publication access remains an
...
| RISK-17 | A release tag or publication credential is changed or misused before immutable release controls take effect | Critical | Low | The active `v*` ruleset restricts tag creation, update, and deletion; both protected environments require review and constrain deployment to exact `main` or `v*` refs; release immutability is enabled; and `DEC-031` carries those controls through `MCP-028`, `MCP-029`, and later `SIDE-006`. The operator-side verifier binds the administration-scoped controls to exact current `main` without exporting a PAT; the successful protected authorization binds canonical `v0.1.0` to the release commit. Only that tag invokes the source/GNU/Linux publisher, while the funded workflow has no tag trigger and rejects `v0.1.0`. `DEC-037` permits one separately authorized annotated operational recovery tag only at a corrected exact-current-main commit with successful CI plus both preflights and a verified public ruleset; it neither invokes a release publisher nor represents a product version. The Homebrew job remains behind the unchanged tag-only `release` environment, uses one tap-repository deploy key, verifies immutable release and registry bytes, and can create only the absent formula | Release automation creates, moves, deletes, or publishes a protected tag outside its exact authorized purpose; recovery accepts a stale commit or another ref; `v0.1.0` invokes paid signing; an approval proceeds after failed controls; or a downstream credential can mutate broader state | Mitigated for immutable release and constrained recovery — Homebrew publication gate remains open |
