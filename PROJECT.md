...
first real repeat release. `MCP-042` through `MCP-044` are complete; `MCP-045` is
in progress under its exact Goal and explicit publication approval.
...
completed. `MCP-042` through `MCP-044` are complete, `MCP-045` is the sole
active main-story ticket, and no side quest is active.
...
| D-24 | First verified repeat release | Post-M4 release readiness | Codex | `v0.1.1` | In progress | `MCP-045` is active under its exact Goal and explicit publication approval. The release branch prepares `Cargo.toml`, `Cargo.lock`, and `docs/releases/v0.1.1.md` with manual Homebrew, Cargo, and verified GitHub archive upgrade paths. Isolated format, strict Clippy, the complete test suite, dependency policy, workflow and artifact policy exercises, Actionlint, ShellCheck, warning-free compilation for all six target triples, deterministic locked Cargo package and formula generation, native archive/SBOM smoke, and documentation/diff checks pass locally. Preliminary exact-main repository controls and public-channel absence checks pass at [`641da01`](https://github.com/EnjoyableWork/mcp-sync/commit/641da01f20c03ebbac96829d300302e76d843ccd); owner-only crates.io Trusted Publisher readback remains an exact pre-tag gate. Publication and durable channel evidence remain pending, and funded signed-native scope stays inactive. |
...
| MCP-045 | Publish and verify `v0.1.1` as the first repeat release | Post-M4 release readiness | P1 | Codex | In progress | `MCP-044` | The exact Goal and explicit publication approval are active. Prepare the exact version and lockfile plus `docs/releases/v0.1.1.md`, document manual Homebrew, Cargo, and verified GitHub archive upgrade paths, pass every local and exact-main gate, revalidate live repository and owner-only Trusted Publisher controls, authorize one annotated exact-main `v0.1.1` tag, publish the immutable attested seven-asset source/GNU/Linux GitHub Release, publish byte-identical Cargo through tokenless Trusted Publishing, update the source-building Homebrew formula monotonically, pass the read-only native channel matrix, refresh version-bound current-release control verifiers, and record durable evidence. Preliminary exact-main controls and public absence checks pass at [`641da01`](https://github.com/EnjoyableWork/mcp-sync/commit/641da01f20c03ebbac96829d300302e76d843ccd); the owner-only Trusted Publisher readback remains pending and blocks tagging rather than reversible preparation. No macOS or Windows project binary, WinGet package, paid signing, moved tag, replaced asset, or `SIDE-006` activation is permitted. |
...
1. Prepare the exact `0.1.1` manifest, lockfile, release notes, and manual
   channel-specific upgrade guidance on the active `MCP-045` branch. Merge only
   after every local check and protected preparation gate passes.
2. On the resulting exact current `main`, revalidate repository controls and
   obtain the owner-only crates.io Trusted Publisher readback. Only then run
   protected authorization and create one annotated `v0.1.1` tag at that exact
   commit.
3. Publish and verify the immutable seven-asset GitHub Release first, the exact
   Cargo package through OIDC second, and the monotonic Homebrew formula third;
   then pass the complete read-only installed-channel matrix.
4. Refresh the version-bound current-release control verifiers to the public
   `v0.1.1` baseline, record durable release evidence, and close `MCP-045` and
   D-24 through a protected change without activating a successor or side quest.
5. Maintain completed `MCP-041`'s canonical later-version policy,
...
6. Maintain completed `MCP-036`'s serialization of `init`, `add`, non-dry-run
...
health-fixture evidence, and separately approved `MCP-045` publication of
`v0.1.1`. `MCP-042` selected and proved the
...
`MCP-029`, M2, `MCP-030` through `MCP-037`, `MCP-039` through `MCP-044`, M3,
bounded M4 expansion, `SIDE-007`, and `SIDE-008` are complete; `MCP-045` is the
sole active main-story ticket, and no side quest is active. The
...
completed only `MCP-043` and then `MCP-044`. The owner's latest approval
activated only `MCP-045` under its exact Goal and exact zero-cost publication
boundary; no successor or side quest was activated.
`DEC-031`, `DEC-036`, and
