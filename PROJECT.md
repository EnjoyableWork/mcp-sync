...
| Current focus | Merge the `MCP-029` pre-publication terminology correction that names reusable automation by its source and GNU/Linux artifact boundary, obtain fresh exact-main CI and both preflight runs, then execute the zero-cost `v0.1.0` runbook with the owner-confirmed short-lived `publish-new` credential scoped to exact crate pattern `enjoyable-mcp-sync`, without activating paid signing or deferred channels |
| Milestone target | 2026-08-08 |
| Last reviewed | 2026-08-08 |
| Next review trigger | Merge of the `MCP-029` release-terminology correction; completion of exact-main release authorization; publication of `v0.1.0`; or intentional activation of a side quest |
...
2. Merge the pre-publication source/Linux terminology correction; preserve the
   verified crates.io identity, repository-scoped tap deploy key, and
   short-lived credential limited to `publish-new` and exact crate pattern
