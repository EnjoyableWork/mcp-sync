...
| Current focus | Merge the protected `MCP-029` activation change, replace the current over-broad crates.io token with one short-lived `publish-new`-only credential, then execute the zero-cost `v0.1.0` runbook from one exact green `main` commit without activating paid signing or deferred channels |
| Milestone target | 2026-08-08 |
| Last reviewed | 2026-08-08 |
| Next review trigger | Merge of the `MCP-029` activation change; creation of the accepted `publish-new`-only crates.io credential; publication of `v0.1.0`; or intentional activation of a side quest |
...
| --- | --- | --- |
| crates.io | Sign in, verify the publisher account and email, recheck `enjoyable-mcp-sync`, create a short-lived first-publication API token with only the `publish-new` endpoint scope, and store it only in Cargo's credential store on the controlled publisher host. A crate-name scope cannot be selected before the crate exists | No |
...
2. Merge the protected Homebrew publisher; preserve the verified crates.io
   ...
   broader token with a short-lived `publish-new`-only credential without
   exposing either credential. Revoke publication tokens after the exact crate
