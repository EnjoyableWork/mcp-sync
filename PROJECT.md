...
| Product state | The source-checkout macOS, GNU/Linux, and Windows implementation reconciles six global targets: Claude Desktop, Cursor, Windsurf legacy Cascade, native VS Code default user-profile, global Codex host configuration, and Kiro's selected global-user configuration inherited by Kiro Crew. It exposes one bounded named-server STDIO initialize health boundary and parser-validated restore with one-generation adjacent retention for canonical state plus all six targets. Deterministic fixtures, complete six-client built-binary journeys, native ARM64 and Rosetta x64 macOS suites, native x64/ARM64 GNU/Linux and Windows MSVC CI, six-target returned-failure rollback, seven-file restore, and focused cross-process regressions prove plan/apply, preservation, redaction, ordinary compensation, and the issue #45 lock invariant. Issue [#47](https://github.com/EnjoyableWork/mcp-sync/issues/47) was confirmed against current `main`; the `MCP-040` worktree now replaces the vulnerable backup-first window with a target-first journaled transaction, refuses incomplete state on lock-free reads, and recovers it before locked planning. Native ARM64 and Rosetta x64 macOS execution plus six-target compilation pass, while native GNU/Linux/Windows abrupt-process execution and hosted preflight remain required before the ticket can complete. Controlled macOS current-client evidence covers Claude Desktop, Cursor, Kiro IDE `1.0.288`, and inherited Kiro Crew `0.1.3`; no Linux or Windows target has a current-client smoke claim, and Windsurf, VS Code, and Codex retain no current-client claim. The immutable `v0.1.0` GitHub Release, byte-identical `enjoyable-mcp-sync` `0.1.0` crates.io package, and exact source-building `EnjoyableWork/tap/mcp-sync` formula remain public and unchanged; every later Cargo version is constrained to protected exact-identity crates.io Trusted Publishing with trusted-publishing-only enforcement and no API-token fallback |
| Current milestone | Post-M4 corrective maintenance — issue [#47](https://github.com/EnjoyableWork/mcp-sync/issues/47) is fixed and locally verified in the worktree; `MCP-040` is the sole active P0 main-story ticket and remains In progress pending native GNU/Linux/Windows and hosted evidence, and no side quest is active |
| Overall status | M0 through M3, `MCP-001` through `MCP-020`, `MCP-028` through `MCP-037`, and `MCP-039` are `Done`; `MCP-040` is `In progress`; `MCP-021` is `Superseded`; `MCP-022` through `MCP-027` are `Deferred`; `SIDE-007` and `SIDE-008` are `Done`; funded `SIDE-006` remains proposed; and no later work is activated or treated as a prerequisite |
| Current focus | Submit and verify the bounded `MCP-040` fix through protected review, native GNU/Linux and Windows x64/ARM64 execution, CodeQL, release preflight, and source/install/archive preflight before recording completion or closing issue #47. Do not conflate issue #47 with issues #44 or #46, expand it into whole-sync process-crash atomicity, publish a release, or activate a successor or side quest |
| Milestone target | Met for the bounded `MCP-037` expansion: D-18 records complete six-target import, sync, restore, no-op, drift, redaction, collision, backup, exact-byte, rollback, operation-lock, built-binary, native-platform, current-client, documentation, and ticket-done evidence; profiles, migrations, secret references, and every other M4 candidate remain deferred and inactive |
...
The main story is complete through the post-M3 `MCP-036` correction and
`MCP-039` release-security continuation. The former restored the
user-configuration safety invariant invalidated by issue #45; the latter now
protects every Cargo version after `0.1.0` with tokenless Trusted Publishing.
The owner deliberately activated and completed bounded M4 expansion
`MCP-037` without any `v0.1.0` adoption-evidence prerequisite. No later
main-story ticket has been selected or activated:
...
| Post-M4 corrective maintenance — crash-consistent replacement | `MCP-040` (active) |
...
depends on its credential boundary. `MCP-037` is completed; `MCP-040` is the
sole active main-story ticket and no side quest is active.
...
| MCP-040 | Make existing-file target and backup replacement crash-consistent | Post-M4 corrective maintenance | P0 | Codex | In progress | `MCP-037` | The worktree fixes issue [#47](https://github.com/EnjoyableWork/mcp-sync/issues/47) in the shared existing-file replacement boundary through `DEC-048` and D-19's implementation and local evidence. Every defined pre-commit and post-commit state, both backup histories, all three affected command journeys, lock contention, malformed or externally changed state, safe cleanup, retry, redaction, ordinary compensation, reverse rollback, native ARM64 and Rosetta x64 macOS execution, and all six compilation targets pass. Completion still requires native GNU/Linux and Windows x64/ARM64 execution plus protected hosted CodeQL, CI, release-preflight, and source/install/archive evidence on the submitted exact commit. This ticket does not promise whole-sync atomicity across process death or power loss, change issues #44 or #46, publish a release, or mutate immutable `v0.1.0`. |
...
`MCP-037` and D-18 are `Done`. `MCP-040` was later classified and activated as
the sole P0 corrective successor; no side quest is active.
...
1. Maintain completed `MCP-037`'s Kiro contract revalidation, six-target
   implementation, native matrix, controlled IDE/Crew evidence, all five
   predecessor targets, and every safety boundary. No main-story ticket or
   side quest is active; classify, approve, assign, and activate any future
   work deliberately under its own exact Goal.
...
6. Maintain completed `MCP-039`'s sole exact trusted-publisher identity,
...
for the active ticket. `MCP-029`, M2, `MCP-030` through `MCP-037`, `MCP-039`,
M3, bounded M4 expansion, `SIDE-007`, and `SIDE-008` are complete; `MCP-040`
is the sole active main-story ticket and no side quest is active. The
...
expansion; `MCP-040` is the deliberately classified P0 correction; and no side
quest or later implementation is implicitly active.
`DEC-031`, `DEC-036`, and
...
| --- | --- | --- | --- | --- | --- | --- |
| RISK-01 | A merge or partial failure loses user configuration | Critical | High | Pure plan/apply separation, guarded exact-byte writes, recoverable backups, reversible receipts, reverse-order six-target rollback, non-mutating drift, bounded client ownership, unmanaged-name collision refusal, and ordinary returned-error compensation remain implemented. The `MCP-040` worktree fixes issue [#47](https://github.com/EnjoyableWork/mcp-sync/issues/47) through `DEC-048`'s target-first journal, exact state classification, recovery before locked planning, lock-free refusal, confined cleanup, and redacted failure paths. Twenty-one real process-termination cases plus focused corruption and retry coverage pass on native ARM64 and Rosetta x64 macOS; all six targets compile under strict Clippy | Abrupt process termination loses or ambiguously strands a target, prior backup, transaction stage, or recovery decision; recovery overwrites an external edit; diagnostics expose transaction contents or fingerprints; or the native GNU/Linux/Windows execution differs from the local macOS proof | Open — P0 `MCP-040` In progress; locally mitigated, native hosted evidence required |
...
| RISK-04 | Cross-platform file replacement behaves differently | High | High | No-clobber creation, guarded atomic single-file replacement, one-generation restore, reversible receipts, and reverse-order rollback remain isolated behind filesystem ports. `MCP-040` adds one portable target-first journal protocol without platform-specific unsafe code; native ARM64 and Rosetta x64 macOS process-termination suites, all six strict-Clippy compilation targets, deterministic Cargo packaging, source installation, and native macOS archives pass without weakening exact-byte, file-type, permission, rollback, or cleanup guards | Any native platform cannot publish, inspect, recover, or clean the transaction with equivalent process-termination semantics; a boundary yields a different target/backup generation; or platform behavior requires an unsafe or unreviewed primitive | Open — P0 `MCP-040` In progress; native GNU/Linux/Windows abrupt-process execution and hosted preflight required |
