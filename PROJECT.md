...
real repeat release. `MCP-042` through `MCP-045` are complete. The owner has
now classified `MCP-046` as the required P0 corrective successor because its
long-lived deploy key can write the shared tap outside the `mcp-sync` formula
boundary and blocks the sibling `mcp-doctor` assurance closeout. `MCP-046` is
`In progress` under Codex after its exact goal was deliberately started; no
other main-story ticket or side quest is active.
...
| Post-M4 corrective release maintenance — tap authority isolation | `MCP-046` (in progress under Codex; depends on completed `MCP-045`) |
...
completed. `MCP-042` through `MCP-045` are complete, and `MCP-046` is the sole
active corrective successor under Codex. No other main-story ticket or side
quest is active.
...
| D-25 | Tap-owned short-lived Homebrew publication | Post-M4 corrective release maintenance | Codex | Before the next release | In progress | Replace the protected cross-repository private key and repository-wide tap deploy key with a manually dispatched workflow owned by `EnjoyableWork/homebrew-tap`. Its write job receives only the tap repository's job-scoped `GITHUB_TOKEN`, independently validates the exact immutable `mcp-sync` release, checksums, attestations, Cargo bytes, monotonic version transition, and exact `Formula/mcp-sync.rb` change, and proves every sibling path—including `Formula/mcp-doctor.rb`—remains unchanged. Deterministic no-write, no-op, stale-version, mismatch, extra-path, ref, permission, and credential-absence cases plus protected exact-head and exact-`main` gates must pass. Only after replacement evidence and fresh action-time owner confirmation may the `mcp-sync` protected-environment secret and tap deploy key be permanently deleted; final live inventories and both projects' affected verifiers must pass without publishing or changing an immutable release. |
...
| MCP-046 | Replace the shared-tap deploy key with tap-owned short-lived publication | Post-M4 corrective release maintenance | P0 | Codex | In progress | `MCP-045` | D-25 is complete when coordinated protected `mcp-sync` and `homebrew-tap` changes preserve the GitHub-first, exact-byte, monotonic, serialized release contract while moving the only Homebrew write operation into the tap repository. The source repository retains no Homebrew private key, PAT, App credential, or other cross-repository write authority; the tap retains no `mcp-sync` deploy key; the tap-owned write job uses only its job-scoped `GITHUB_TOKEN`, can stage exactly `Formula/mcp-sync.rb`, and rejects or leaves unchanged every sibling path. A no-write/no-op rehearsal against immutable `v0.1.1`, deterministic positive and negative policy tests, relevant local checks, protected exact-head and exact-`main` CI/CodeQL/preflight gates, live zero-secret and zero-deploy-key readback, and the previously blocked `mcp-doctor` organization-control verifier pass. Permanent secret and key deletion occurs only after the replacement is merged and verified and after fresh action-time owner confirmation. No version, tag, release, crate, formula content, signed artifact, WinGet package, runtime behavior, or funded `SIDE-006` scope changes. |
...
3. Complete `MCP-046` as the sole active corrective task under its exact Goal.
   Do not activate another main-story ticket or side quest, and keep funded
   signed-native `SIDE-006` proposed and dormant.
4. Maintain completed `MCP-041`'s canonical later-version policy,
   ...
   nonpublishing rehearsal while `MCP-046` replaces the current tap-scoped
   deploy-key pairing. Do not remove either credential half before the
   tap-owned replacement passes, and do not use the long-lived pairing for a
   later release after that replacement is verified.
...
boundary; only implementation, protected evidence, and final live credential
removal remain open.
`MCP-029`, M2, `MCP-030` through `MCP-037`, `MCP-039` through `MCP-045`, M3,
bounded M4 expansion, `SIDE-007`, and `SIDE-008` are complete; `MCP-046` is
the sole active main-story ticket, and no side quest is active. The
...
| RISK-25 | A long-lived `mcp-sync` deploy key can mutate sibling formulas in the shared Homebrew tap | Critical | Medium | `MCP-046` moves formula publication into a manually dispatched tap-owned workflow, grants `contents: write` only to its tap-local job-scoped `GITHUB_TOKEN`, independently validates immutable upstream and monotonic transition evidence, permits exactly `Formula/mcp-sync.rb`, rejects every sibling or extra path, and removes both long-lived credential halves only after protected replacement evidence and fresh owner confirmation. Existing immutable `v0.1.0` and `v0.1.1` artifacts remain unchanged. | A source-repository secret, PAT, App token, deploy key, or other reusable cross-repository writer remains; the tap workflow can alter a sibling path; deletion precedes verified replacement; a later release uses the old key; or either project's affected live verifier fails | In progress — `MCP-046` and D-25 are active; this does not invalidate current `v0.1.1` bytes but blocks sibling `mcp-doctor` organization-assurance closure until migration, deletion, and live revalidation pass |
