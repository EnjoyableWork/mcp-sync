...
| Overall status | M0 through M3, `MCP-001` through `MCP-035`, and `SIDE-007` are `Done`; no later main-story milestone or side quest is active, and only funded `SIDE-006` remains proposed |
| Current focus | Maintain the terminal-first README product page, completed M3 controls, BadgeApp `baseline-1` status, public assurance evidence, exact-artifact SLSA statement, and immutable M2 release while collecting adoption feedback; select and activate any later arc deliberately under its own accepted ticket and Goal |
...
changing the product, release, signing, or assurance contract. There are no
active side quests. `DEC-038` promotes and supersedes `SIDE-001` through
...
funding-gated. `SIDE-007` is complete, no side quest is active, and both
remaining objectives stay recorded in the
