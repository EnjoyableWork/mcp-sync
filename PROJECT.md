...
| Current focus | Complete the protected `MCP-029` activation change, confirm the crates.io publisher email is verified, then execute the zero-cost `v0.1.0` runbook from one exact green `main` commit without activating paid signing or deferred channels |
| Milestone target | 2026-08-08 |
| Last reviewed | 2026-08-08 |
| Next review trigger | Merge of the `MCP-029` activation change; confirmation of the verified crates.io email; publication of `v0.1.0`; or intentional activation of a side quest |
...
| --- | --- | --- |
| crates.io | Sign in, verify the publisher account and email, recheck `enjoyable-mcp-sync`, create a first-publication API token, and store it only in Cargo's credential store on the controlled publisher host | No |
...
| D-12 | Zero-cost market-validation release and installed recovery exercise | M2 | Codex | 2026-08-08 | In progress | `MCP-028` is complete through the [zero-cost workflows](.github/workflows/market-release.yml), [deterministic source generator](scripts/generate-market-release-channels.sh), [market runbook](docs/market-release.md), exact-commit [market preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31268357596), retained [six-target preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31268357599), and [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31268357600). `MCP-029` is active under its exact goal; the public organization tap and its protected repository-scoped deploy key are ready, while the crates.io publisher email still requires owner confirmation. The ticket must publish immutable `v0.1.0`, crates.io `enjoyable-mcp-sync` `0.1.0`, and the source-built organization Homebrew formula from one commit and verify their supported-host installation and recovery journeys without project-issued macOS or Windows binaries |
...
2. Merge the protected Homebrew publisher, confirm the crates.io publisher
   email is verified, and preserve the scope-minimized Cargo token and
   repository-scoped tap deploy key without exposing either credential.
