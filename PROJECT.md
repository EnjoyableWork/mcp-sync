...
| Current focus | Verify the signing, protected-publication, registry-identity, and native-runner prerequisites for `MCP-021` before creating a stable tag or package metadata |
| Milestone target | Unscheduled; set after an owner and delivery capacity are known |
| Last reviewed | 2026-08-08 |
| Next review trigger | Completion of the `MCP-021` release-authority preflight; a precise signing, registry, runner, or protection blocker; publication of `v0.1.0`; or intentional activation of a side quest |
...
| D-11 | Accessible release channels and installed recovery exercise | M2 | Codex | Unscheduled | In progress | `MCP-021` release-authority preflight is active. Completion still requires six signed or platform-appropriate binaries, protected `v*` tags and release environment, immutable GitHub Release, SHA-256 manifest, SPDX SBOMs, attestations, Homebrew, WinGet, Cargo, per-target install smoke tests, and restore exercise through the packaged executable |
