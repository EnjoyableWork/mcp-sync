...
| Overall status | M2 and `MCP-030` through `MCP-032` are `Done`; `MCP-033` is `In progress`; `MCP-034` and `MCP-035` remain proposed and dormant; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Complete `MCP-033` dependency-maintenance, CI-isolation, authenticated-distribution, and repository-artifact controls under its exact canonical Goal without changing immutable `v0.1.0` or beginning later assurance work |
| Milestone target | Unscheduled |
| Last reviewed | 2026-08-09 |
| Next review trigger | `MCP-033` completion or blocker; an Actions, dependency-update, workflow-trust, distribution-authentication, repository-artifact, `main` ruleset/check-context, repository-security-control, community-contract, repository-inventory, or licensing change; an assurance-framework revision; a security or release incident; evidence drift; or explicit funding and activation of `SIDE-006` |
...
| D-13 | Evidence-backed enterprise assurance baseline | M3 | Codex | Unscheduled | In progress | `MCP-030` through `MCP-034` implement the contributor, repository, security, supply-chain, and organization controls mapped to all 24 OpenSSF OSPS Baseline `v2026.02.19` Level 1 requirements. `MCP-035` must then publish a complete control-and-evidence crosswalk, a dated and scoped self-attestation, a durable security-assurance contract, and only achieved README trust claims; the milestone does not imply OSPS Level 2 or 3, independent certification, regulatory compliance, funded code signing, or attainment of another candidate assurance target. |
...
| MCP-033 | Automate dependency maintenance and harden CI and distribution supply chains | M3 | P1 | Codex | In progress | `MCP-032` | Weekly grouped Cargo and GitHub Actions updates are configured; every action is pinned to a reviewed full commit SHA with an update hint; allowed-action and SHA policy is narrowed as far as required CI permits; untrusted metadata is validated; fork and other untrusted code receive no privileged credentials or assets; official distribution uses cryptographically authenticated channels; and a documented automated or review gate prevents committed generated executables and unreviewable binary artifacts. CI and dependency policy remain green, and evidence proves `OSPS-BR-01.01`, `OSPS-BR-01.03`, `OSPS-BR-03.02`, `OSPS-QA-05.01`, and `OSPS-QA-05.02` without modifying immutable `v0.1.0`. |
...
1. Keep completed `MCP-030` through `MCP-032` controls active while `MCP-033`
   implements and verifies dependency-maintenance, CI-isolation,
   authenticated-distribution, and tracked-artifact controls under its exact
   canonical Goal.
2. Complete `MCP-033` through `MCP-035` in order. Do not activate a successor
...
`MCP-029`, M2, and `MCP-030` through `MCP-032` are complete;
only `MCP-033` is active under its exact canonical Goal. Organization
continuity and recovery choices remain
scoped to later `MCP-034`; no other M3 ticket is implicitly active.
`DEC-031`, `DEC-036`, and
...
| RISK-15 | Mutable Actions references or unmaintained dependencies compromise or silently weaken CI | High | Medium | Repository Actions now requires full commit SHAs and permits only the ten exact direct or reviewed-transitive action paths in `.github/actions-policy.json`; broad GitHub-owned and verified-creator allowances are off. The `MCP-033` branch pins all 72 committed references, adds weekly grouped Cargo and Actions updates, validates action hints and untrusted workflow boundaries, and makes tracked executable/binary rejection part of the stable dependency check. Dependabot security updates and alerts remain enabled and baseline-clean; protected PR #38 must still merge and pass exact-main verification before this mitigation is closed | An action hint stops resolving to its reviewed SHA, an action appears outside the inventory, fork code receives privilege, a generated or binary artifact is tracked, an actionable advisory remains unpatched, or dependency updates stop arriving | Open — live Actions policy active; protected `MCP-033` merge and exact-main evidence pending |
