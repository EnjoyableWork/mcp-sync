...
| Current milestone | Post-M3 release maintenance is complete — `MCP-039` and D-17 are `Done`; approved M4 successor `MCP-037` is `Ready` for deliberate Goal activation and remains unimplemented |
| Overall status | M0 through M3, `MCP-001` through `MCP-036`, `MCP-039`, `SIDE-007`, and `SIDE-008` are `Done`; no main-story ticket or side quest is active; funded `SIDE-006` remains proposed; and `MCP-037` Kiro is the approved, ready, ordered M4 successor without a `v0.1.0` adoption-evidence prerequisite |
| Current focus | Deliberately hand ready `MCP-037` to its own exact canonical Goal before assigning or implementing it; preserve `MCP-039`'s exact `EnjoyableWork/mcp-sync` / `cargo-publish.yml` / `release` crates.io publisher, trusted-publishing-only setting, zero-token boundary, sole `v*` release-environment policy, immutable release, issue [#45](https://github.com/EnjoyableWork/mcp-sync/issues/45) cross-process mutation boundary, and assurance evidence; make no Kiro support claim before its implementation, native matrix, and current-client evidence exist |
| Milestone target | `MCP-039` and D-17 have satisfied their workflow, trusted-publisher, trusted-only, authorization-only rehearsal, regression, native-preflight, documentation, and ticket-done gates without creating a version, tag, release, crate, formula, WinGet package, or signed-native artifact; `MCP-037` and D-18 are the next bounded target only after deliberate Goal activation |
...
`v0.1.0` adoption-evidence prerequisite. It is `Ready`, but it does not begin
until its exact canonical Goal is deliberately activated and ownership is
assigned:
...
| M4 — Approved Kiro client expansion | `MCP-037` (ready; begins only after deliberate Goal activation) |
...
depends on its credential boundary. `MCP-037` is ready but unassigned and has
not begun.
...
| MCP-037 | Add a global-user Kiro MCP adapter with inherited Kiro Crew coverage | M4 | P1 | Unassigned | Ready | `MCP-039` | `DEC-046` approves Kiro as the ordered M4 successor without requiring `v0.1.0` adoption evidence. The implementation begins only after its exact canonical Goal is deliberately activated, revalidates the current global and `KIRO_HOME` paths, precedence, interpolation, and Kiro Crew inheritance contracts; owns only canonical-compatible local STDIO fields in the selected global file; preserves unowned and unrepresentable data; never accesses higher-precedence, Crew-only, generated-agent, managed, or credential stores; extends deterministic six-target configuration, recovery, rollback, lock, and native-platform evidence; and completes controlled current-client Kiro IDE and inherited Kiro Crew smokes before any support claim. |
...
predecessor gate, so the ticket is now `Ready`; Kiro remains unimplemented and
unsupported. Starting it still requires deliberate activation of its exact
canonical Goal and assignment of ownership.
...
1. Deliberately activate ready `MCP-037` under its exact canonical Goal before
   assigning ownership or beginning implementation. Revalidate Kiro's current
   contracts first, preserve all five existing targets and safety boundaries,
   and make no Kiro support claim before the complete adapter, native-platform,
   and controlled current-client evidence passes.
...
`SIDE-008` are complete; approved successor `MCP-037` is `Ready` but
unassigned, and no main-story ticket or side quest is active. The
...
continuation after `MCP-036`; `MCP-037` is approved and ready but unstarted,
and no side quest or later implementation is implicitly active.
`DEC-031`, `DEC-036`, and
