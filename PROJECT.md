...
| Overall status | M0 through M3, `MCP-001` through `MCP-035`, `SIDE-007`, and `SIDE-008` are `Done`; no later main-story milestone or side quest is active, and funded `SIDE-006` remains proposed |
| Current focus | Maintain the single-suite pull-request CI trigger, test-only Windows fixture startup headroom, terminal-first README, completed M3 assurance, and immutable M2 release while collecting adoption feedback; select any later arc deliberately under its own accepted ticket and Goal |
...
2. Maintain `SIDE-008`'s single pull-request CI suite, exact protected check
   names, and test-only Windows fixture headroom without weakening production
   health limits or native x64/ARM64 coverage.
3. Maintain the completed `MCP-030` through `MCP-035` controls, public evidence,
...
4. Keep M4, M5, and every side quest inactive until a later outcome is
   ...
   successor.
5. Preserve immutable `v0.1.0`, its signed annotated release and operational
...
6. Monitor the public GitHub, Cargo, and source-building Homebrew channels and
...
7. Treat OSPS Level 1 as the only M3 assurance gate. Best Practices, NIST SSDF,
...
8. Retain the funded six-target signed-native workflow, preflight, metadata,
...
complete. The completed side quests changed only the README product-page
