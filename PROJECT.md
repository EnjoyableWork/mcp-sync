...
| Known open defects | Active `MCP-043` addresses issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46), which proves empty or `=`-bearing environment names can pass canonical validation and sync despite changing or losing identity at the process boundary. Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) records the post-`SIDE-008` recurrence of Windows ARM64 health-fixture startup timing without establishing a product defect and remains inactive behind `MCP-043`. Issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44) is closed by merged [PR #65](https://github.com/EnjoyableWork/mcp-sync/pull/65) and exact-main commit [`991e6b2`](https://github.com/EnjoyableWork/mcp-sync/commit/991e6b2114267fb26c3beb3a319e96de0452c7e2), whose CI and both native preflights passed on attempt one. `DEC-050` orders #46 before #60 |
| Current milestone | Post-M4 release readiness — `MCP-042` and D-21 are `Done`; highest-leverage ticket `MCP-043` and D-22 are `In progress`; `MCP-044` and `MCP-045` remain `Proposed`; and no side quest is active |
| Overall status | M0 through M3, `MCP-001` through `MCP-020`, `MCP-028` through `MCP-037`, and `MCP-039` through `MCP-042` are `Done`; `MCP-043` is `In progress`; `MCP-044` and `MCP-045` are `Proposed`; `MCP-021` is `Superseded`; `MCP-022` through `MCP-027` are `Deferred`; `SIDE-007` and `SIDE-008` are `Done`; and funded `SIDE-006` remains proposed |
| Current focus | Complete active `MCP-043` under its exact catalog Goal by fixing issue #46 through the shared canonical validation boundary, all six native ownership contracts, and deterministic pre-mutation and pre-execution regressions. Keep `MCP-044`, `MCP-045`, every deferred expansion, and funded `SIDE-006` inactive until their predecessors and separate Goal contracts permit activation |
...
first real repeat release. `MCP-042` is complete; `MCP-043` is active under its
exact catalog Goal, while `MCP-044` and `MCP-045` remain proposed.
...
completed. `MCP-042` is complete, `MCP-043` is in progress,
`MCP-044` and `MCP-045` remain proposed, and no side quest is active.
...
| D-22 | Portable canonical environment-name validation | Post-M4 release readiness | Codex | Before `v0.1.1` | In progress | Issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46), `DEC-050`, `DEC-053`, and `MCP-043` define one canonical v1 invariant: every environment name is non-empty and contains neither NUL nor `=`, with no normalization or schema migration. One shared validator in [`config.rs`](src/config.rs) is the authority: `add` reaches it before lock acquisition or filesystem access, and canonical reads reach it before list, sync planning, or process launch. Native translation uses that same rule while preserving existing ownership contracts: Claude Desktop returns a structural invalid-canonical error, while Cursor, Windsurf, VS Code, Codex, and Kiro classify an otherwise-local entry with an invalid name as canonical-unrepresentable, preserve it structurally under the adapter's existing round-trip contract, report it as skipped during `init`, and refuse a colliding desired server during sync. No invalid definition is imported, synchronized, or executed, and no existing file changes on rejection. Returned validation errors may identify the client, server, deterministic key position, and violated rule; unmanaged-entry reports retain their existing client/server-only redaction. Neither path may expose an environment value or invalid key text. Readiness inspection identified no new dependency, parser, unsafe code, schema version, or migration requirement; the small shared predicate is justified by the real fail-versus-unmanaged adapter variation. Active implementation evidence includes focused model and adapter tests, dedicated canonical and six-client fixtures under [`tests/fixtures`](tests/fixtures), and the combined built-binary regression in [`tests/environment_names.rs`](tests/environment_names.rs). Formatting, dependency policy, deterministic packaging, repository policy checks, all six strict-Clippy compilation surfaces, complete native ARM64 and Rosetta x64 macOS suites, and the complete unprivileged native ARM64 GNU/Linux synthetic-home gate pass on the current worktree. Acceptance still requires durable exact-head hosted GNU/Linux x64/ARM64 and Windows MSVC x64/ARM64 evidence before this deliverable or ticket can move to `Done`. |
...
| MCP-043 | Reject environment names that cannot retain identity across supported boundaries | Post-M4 release readiness | P0 | Codex | In progress | `MCP-042` | Issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46) is the highest-leverage remaining issue: it is a cross-platform product-correctness defect that can silently change a synchronized definition at execution, whereas issue #60 currently affects test evidence and remains the ordered P1 successor. `DEC-053` fixes the implementation contract at a shared environment-name validator in [`config.rs`](src/config.rs): reject empty names and names containing NUL or `=` without normalization before canonical create/replace, sync planning, listing, or process start. `AddRequest::validate` already provides the pre-lock/pre-filesystem entry point, and canonical parsing protects list, sync, and health. All six native adapters must apply the same rule using their existing ownership semantics: Claude Desktop fails the operation structurally; Cursor, Windsurf, VS Code, Codex, and Kiro preserve the entry structurally as unmanaged, make `init` report it as skipped, and reject a canonical collision. Returned validation errors may expose only client/server context, deterministic key position, and the violated rule; unmanaged reports retain their existing client/server-only redaction. Neither path may expose environment values or invalid key text. No schema bump, migration layer, new dependency, parser, or platform-specific rule is required. Acceptance requires the model, CLI, canonical, six-adapter, built-binary `init`/`sync`/`test`, redaction, preservation, no-mutation, no-execution, and native Unix/Windows evidence described in D-22. The ticket is active under the exact catalog Goal. |
...
1. Complete active highest-leverage ticket `MCP-043` under the exact catalog
   Goal. Merged PR #65 closed its predecessor with complete attempt-one
   exact-main evidence; no dependency or design decision remains.
2. Implement `DEC-053` at the shared canonical model boundary, then prove
   pre-lock `add` and canonical reads reject invalid names, Claude Desktop
   fails invalid native local entries structurally, the other five adapters
   preserve them as unmanaged and refuse collisions, and health never starts
   them. Keep `MCP-044` and `MCP-045`
   proposed and ordered; publication additionally requires separate explicit
   approval and live control checks.
...
completed `MCP-042` process containment, active `MCP-043` portable
...
`MCP-029`, M2, `MCP-030` through `MCP-037`, `MCP-039` through `MCP-042`, M3,
bounded M4 expansion, `SIDE-007`, and `SIDE-008` are complete; owned `MCP-043`
is in progress under its exact catalog Goal, and no side quest is active. The
...
activated and completed only `MCP-042`; the subsequent exact Goal activates
only highest-leverage `MCP-043`, while `MCP-044` and `MCP-045` remain proposed.
`DEC-031`, `DEC-036`, and
...
| RISK-23 | A canonical environment name changes identity or becomes invalid at a supported process boundary | High | Medium | Canonical parsing, client rendering, and health execution reject NUL today, but issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46) proves they still accept empty and `=`-bearing names. Such a definition can validate and synchronize across all clients while the operating-system environment loses or changes the key. `DEC-050` selects active P0 `MCP-043` ahead of the P1 CI-evidence issue, and `DEC-053` fixes the portable non-empty/no-NUL/no-`=` invariant through one shared `config.rs` validator. The traced consumers cover validation-before-lock `add`, canonical reads, all six native adapters before `init` creation or sync planning, and health before process launch. Existing ownership variation is explicit: Claude fails structurally, while Cursor, Windsurf, VS Code, Codex, and Kiro preserve invalid local entries as unmanaged and reject collisions | `add`, canonical parsing or construction, native import, sync, list, or `test` accepts a name that cannot retain exact identity on macOS, GNU/Linux, or Windows; an invalid name is normalized, imported, executed, or dropped; an unmanaged native entry is overwritten; a command mutates before rejection; or a diagnostic exposes invalid key text or an environment value | In progress — `MCP-043` is active under its exact catalog Goal; blocks `v0.1.1` |
