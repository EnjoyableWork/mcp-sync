...
| Current focus | Finish `MCP-042` by publishing the locally verified containment change for review, then obtain exact-head CI, CodeQL, release-preflight, source/install/archive, and native hosted macOS/GNU/Linux/Windows x64/ARM64 evidence before closing issue #44. Keep `MCP-043` through `MCP-045`, every deferred expansion, and funded `SIDE-006` inactive until their predecessors and separate Goal contracts permit activation |
...
| D-21 | Bounded cross-platform health-process containment | Post-M4 release readiness | Codex | Before `v0.1.1` | In progress | Issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44), `DEC-050`, `DEC-051`, and `MCP-042` define the active correction. The local [containment boundary](src/process_containment.rs) now uses a suspended non-breakaway Windows Job Object, a monitored Unix process group, Linux child-subreaper and PID-descriptor hardening, exact-start-time macOS identities, exact-root authorization before Unix group signaling, deadline-polled direct-child reap, and independent drop restoration; [health integration](src/health.rs) applies it to success, failure, setup, cleanup, reader timeout, and drop without any blocking `Child::wait`. The harness-free [built-binary regression](tests/health_process_containment.rs) reproduces the five-second issue path with an intermediary and leaf that each create a new session, reparents the leaf after the intermediary publishes its identity and exits, ignores graceful signals, and retains stdout plus a listener. Four command outcomes prove exact root, intermediary, and leaf disappearance, reader and listener release, bounded return, exact diagnostics, and structural redaction after response timeout, malformed protocol, successful direct-child exit, and shutdown refusal; a fifth direct-boundary case proves explicit setup-failure cleanup, a sixth injects a returned transient cleanup error and proves the retained drop retry still removes the complete tree, and a seventh proves the independent ordinary `ContainedChild` drop backstop. Focused units prove ancestry closure, start-identity PID-reuse refusal, reused-root group-signal refusal, native inspection, Linux stat parsing, volatile `/proc` disappearance and permission races including `ESRCH` without swallowing unrelated I/O failures, and retained reader ownership after an injected join timeout. Ten consecutive strengthened built-binary runs pass on native ARM64 macOS and another ten pass in unprivileged native GNU/Linux ARM64; complete native ARM64 and Rosetta x64 macOS suites, the complete unprivileged GNU/Linux ARM64 suite, every existing direct-child regression, warning-free strict Clippy across all six target triples, formatting, `cargo deny`, Actionlint, ShellCheck, repository and upstream workflow policy, deterministic Cargo and source-channel generation, Cargo publication dry-run, packaged install/recovery, and native archive smoke also pass locally without changing the five-second response, 1-MiB message, or 500-millisecond graceful-shutdown limits. The retained six-host [release preflight](.github/workflows/release-preflight.yml) now runs the focused containment executable directly on native macOS, GNU/Linux, and Windows x64/ARM64 before packaging, so hosted closure evidence proves the affected runtime boundary rather than inferring it from archive smoke. The `libc` `0.2.189`, `rustix` `1.1.4`, and `windows-sys` `0.61.2` direct dependencies add no package to the existing lock graph, pass the existing license/advisory/source policy, and increase the optimized ARM64 macOS binary by 29,648 bytes (1.17%) in an isolated same-toolchain comparison with `origin/main`. Done remains gated on reviewed durable implementation history, exact-head hosted CI, CodeQL, release preflight, source/install/archive preflight, and complete native hosted macOS/GNU/Linux/Windows x64/ARM64 runtime evidence. |
...
| MCP-042 | Contain and clean up the complete health-check process tree | Post-M4 release readiness | P0 | Codex | In progress | `MCP-041` | `DEC-050`, `DEC-051`, and D-21 classify issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44) as the first release-readiness correction. The implementation, four command outcomes, explicit setup-failure cleanup, returned-cleanup-failure drop retry, ordinary direct-drop backstop, double-session reparenting escape, reader-ownership regression, and complete native ARM64/Rosetta x64 macOS plus unprivileged GNU/Linux ARM64 suites are locally green; strict Clippy covers all six supported host triples and release packaging/install/archive gates pass without changing product limits, redaction, synchronous CLI, or the sole execution boundary. Done still requires reviewed durable history, exact-head hosted CI, CodeQL, release preflight, source/install/archive preflight, complete native hosted macOS/GNU/Linux/Windows x64/ARM64 gates, and issue closure readiness. |
...
| RISK-05 | Health checks hang or leave child processes running | High | High | The only execution boundary still enforces a five-second response limit, 1-MiB message limit, 500-millisecond clean-shutdown allowance, direct-child force termination and reap, and a drop backstop. Issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44) proves the released direct-child boundary can leak a session-escaping inherited-stdout descendant. Active `MCP-042` and `DEC-051` now add a local suspended Windows Job Object, monitored Unix process group, Linux subreaper/PID descriptors, exact macOS identities, deadline-polled reap, retained reader ownership, and cleanup on success, failure, setup, and drop. Linux process inspection treats volatile `ENOENT`, `ESRCH`, and permission races as disappearance while retaining unrelated monitor failures. Four command outcomes plus explicit setup-failure, returned-cleanup-failure drop-retry, and ordinary direct-drop cases use an identity-publishing intermediary and a reparented leaf that each create another session; the leaf resists graceful signals, retains stdout and a listener, and the cases prove exact root, intermediary, and leaf disappearance, reader/resource release, bounded return, and structural redaction. Ten consecutive strengthened runs on each of native ARM64 macOS and unprivileged native GNU/Linux ARM64 plus complete ARM64/Rosetta x64 macOS and GNU/Linux ARM64 suites pass without changing a limit or diagnostic boundary; complete native hosted evidence remains required before this mitigation is achieved. Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) additionally records two responsive Windows ARM64 fixture startup timeouts after completed `SIDE-008`, satisfying the documented recurrence trigger even though an unchanged rerun and later suites passed; `DEC-050` assigns deterministic no-retry Windows evidence to proposed `MCP-044` after containment closes | A contained process or reader survives any return path; a platform primitive permits escape; cleanup exceeds its bound; diagnostics disclose process or configuration details; the Windows ARM64 timing failure recurs; or a product limit is weakened to make tests pass | Open — complete local `MCP-042` macOS/GNU/Linux containment and all-target compilation proof pass; native hosted closure evidence is pending, and issue #60 remains queued for `MCP-044` |
