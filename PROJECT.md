...
| Overall status | `MCP-020` is `Done`; `MCP-021` is `Superseded` by `DEC-036`; `MCP-028` is the sole `In progress` main-story ticket under Codex ownership; `MCP-029` is `Proposed`; and `SIDE-001` through `SIDE-006` are proposed but inactive |
| Current focus | Preserve the merged signed-release foundation while `MCP-028` separates and proves a zero-cost market-validation path for an immutable GitHub Release, Cargo source installation, and source-built Homebrew without publishing unsigned macOS or Windows binaries |
| Milestone target | Unscheduled; set after an owner and delivery capacity are known |
| Last reviewed | 2026-08-08 |
| Next review trigger | Completion of `MCP-028`; availability of the no-cost Cargo and Homebrew publication identities; publication of `v0.1.0`; or intentional activation of a side quest |
...
| MCP-028 | Establish the zero-cost market-validation release path | M2 | P1 | Codex | In progress | `MCP-020` | The protected release flow has a dry-run-proven path that publishes only attested GNU/Linux x64/ARM64 binaries plus source-based Cargo and Homebrew outputs; a normal `v*` market-release tag cannot accidentally invoke the credential-gated signed workflow; the retained six-target signed pipeline remains statically and preflight verifiable; release notes and runbooks distinguish the current free boundary from the funded destination; generated checksums, SBOMs, attestations, Cargo package, and Homebrew formula are deterministic; native install/recovery smokes pass for every represented host; and the complete quality gate passes without publishing a tag, release, crate, or formula |
| MCP-029 | Publish and verify the zero-cost `v0.1.0` market-validation release | M2 | P1 | Unassigned | Proposed | `MCP-028` | The exact protected `v0.1.0` commit is published as one immutable GitHub Release with attested GNU/Linux x64/ARM64 artifacts, `enjoyable-mcp-sync` `0.1.0` on crates.io, and `EnjoyableWork/tap/mcp-sync` as a source-building formula; all represented native install and recovery smokes pass; the crate name and public identities are reverified; current release notes state the macOS, Windows, and WinGet boundary without implying unsigned binaries; publication evidence is durable; and the M2 milestone gate passes |
...
1. Execute `MCP-028` as the sole active main-story ticket under its exact
   Goal-mode objective and the `DEC-036` zero-cost boundary.
2. Separate a dry-run-proven market-release path from the retained paid signing
   path before any tag can invoke release automation; publish nothing in
   `MCP-028`.
3. Preserve the active `v*` protections, immutable-release controls,
   provenance, deterministic downstream metadata, native installation and
   recovery smokes, and accurate release notes while limiting project-issued
   `v0.1.0` binaries to GNU/Linux x64/ARM64.
4. Make the remaining no-cost human authorities explicit for `MCP-029`: a
   crates.io account and first-publication token, organization Homebrew tap
   write access, and authorization through the protected GitHub release path.
...
sequence. No open product decision currently blocks `MCP-028`; `DEC-031` and
...
Neither blocks `MCP-028` or `MCP-029`.
...
| RISK-17 | A release tag or publication credential is changed or misused before immutable release controls take effect | Critical | Low | The active `v*` ruleset restricts tag creation, update, and deletion; the protected release environment limits stable-tag deployment; release immutability is enabled; and `DEC-031` carries those controls through `MCP-028`, `MCP-029`, and later `SIDE-006` | Release automation can create, move, delete, or publish a stable tag outside the authorized path, or the free tag can invoke the paid workflow | Open — `MCP-028` separation and `MCP-029` publication gate |
