...
| Overall status | M2, `MCP-030`, and `MCP-031` are `Done`; only `MCP-032` is `Ready`, unassigned, and not active; `MCP-033` through `MCP-035` remain proposed and dormant; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Intentionally activate `MCP-032` under its exact Goal and an assigned owner when work begins; do not begin `MCP-033` or later assurance work early, and preserve the completed repository-protection and security controls |
| Milestone target | Unscheduled |
| Last reviewed | 2026-08-09 |
| Next review trigger | `MCP-032` activation, completion, or blocker; a `main` ruleset/check-context or repository-security-control change; an assurance-framework revision; a security or release incident; evidence drift; or explicit funding and activation of `SIDE-006` |
...
1. Keep completed `MCP-030` and `MCP-031` controls active, and intentionally
   activate only `MCP-032`, the earliest incomplete main-story ticket, by
   assigning an owner and reconciling a thread to its exact canonical Goal.
   `MCP-032` is `Ready`, but no successor ticket or Goal is active yet.
...
M2, `MCP-030`, and `MCP-031` are complete; only `MCP-032` is `Ready`,
unassigned, and not active under a Goal. Organization continuity and recovery
choices remain scoped to later `MCP-034`; no other M3 ticket is implicitly
active. `DEC-031`, `DEC-036`, and
