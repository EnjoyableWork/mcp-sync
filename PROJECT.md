...
| Overall status | M2 and `MCP-030` through `MCP-034` are `Done`; `MCP-035` is the only active main-story ticket and is `In progress` under its canonical Goal; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Publish `MCP-035`'s completed 24-control self-assessment, issued BadgeApp `baseline-1` proof, security-assurance contract, exact-artifact SLSA statement, and README trust surface through the protected path, then verify all public evidence on exact `main` while maintaining the completed `MCP-030` through `MCP-034` controls |
| Milestone target | Unscheduled |
| Last reviewed | 2026-08-09 |
| Next review trigger | `MCP-035` activation, completion, or blocker; an organization-access, recovery, Actions, dependency-update, workflow-trust, distribution-authentication, repository-artifact, `main` ruleset/check-context, repository-security-control, community-contract, repository-inventory, or licensing change; an assurance-framework, issuer-status, badge, or public-proof-link revision; a security or release incident; evidence drift; or explicit funding and activation of `SIDE-006` |
...
| MCP-035 | Self-assess, publish, and showcase the zero-cost enterprise assurance baseline | M3 | P1 | Codex | In progress | `MCP-034` | A public durable assurance document evaluates every one of the 24 OpenSSF OSPS Baseline `v2026.02.19` Level 1 controls, links evidence or exact conditional reasoning, revalidates the four existing-evidence controls `OSPS-DO-01.01`, `OSPS-QA-01.01`, `OSPS-QA-01.02`, and `OSPS-QA-02.01`, and contains no private settings, non-required identity details, reports, or secrets. Treat owner authorization for the badge service's GitHub OAuth and required public account attribution as an explicit human gate; enroll `EnjoyableWork/mcp-sync`, complete its public `baseline-1` assessment against the named checklist version, and only after every control passes publish the dated and scoped self-attestation, obtain the official BadgeApp `baseline-1` self-certification badge, and add a concise README Security and Trust surface whose dynamic shield links to the public assessment. Verify the badge status, link destination, exact scope, terminology, and rendered README on exact `main`; separately verify and scope an artifact-level SLSA v1.0 Build Level 2 statement from the existing GitHub attestations; define annual, framework-change, issuer-status, security-incident, release-pipeline-change, and evidence-drift review triggers; and make no OSPS Level 2 or 3, independent-certification, regulatory, OpenChain, NIST-certification, or paid-signing claim. The owner-authorized [project `14014` assessment](https://www.bestpractices.dev/en/projects/14014/baseline-1) now passes 24/24 and the official dynamic badge is issued; repository publication and exact-`main` verification remain open. |
...
1. Keep completed `MCP-030` through `MCP-034` controls active while completing
   `MCP-035`, the only active main-story ticket, under its exact canonical Goal.
2. Complete `MCP-035` without reusing a retired `SIDE-001` through `SIDE-005`
   objective. Publish the owner-authorized, issued BadgeApp `baseline-1` badge,
   complete crosswalk, exact limitations, and README trust surface through the
   normal protected path, then verify the assessment, badge, link destination,
   rendered README, and supporting evidence on exact `main` before closure.
...
ticket gate without publishing private details. `MCP-035` is the only active
main-story ticket, is owned by Codex under its canonical Goal, and no other M3
ticket is implicitly active.
`DEC-031`, `DEC-036`, and
...
| RISK-19 | A stale, over-broad, unofficial, or ambiguous self-attestation or badge misleads adopters | High | Medium | `DEC-038` limits M3 to the exact OSPS checklist version and Level 1 scope, while `DEC-044` requires issuer-provided proof where available, links it to the public assessment, prohibits a fabricated certification-like substitute, and requires exact-main verification plus framework version, assessment date, scope, limitations, public evidence, issuer status, and revalidation and removal triggers beside every claim. Artifact-scoped SLSA evidence remains separate from project-wide claims, and README trust language remains blocked until `MCP-035` | The framework or issuer status changes, a badge is withdrawn, a mapped control or evidence link fails, a security or release-pipeline change invalidates evidence, or public wording implies a higher level, certification, another framework, all artifacts, or current status that was not assessed | Open — `MCP-035` badge, claim, and maintenance gate |
