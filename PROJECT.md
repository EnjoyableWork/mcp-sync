...
| Current focus | Complete `MCP-034`'s non-disclosing organization-access policy, verifier, private recovery evidence, and live controls; explicit owner approval remains required before changing organization-wide base access or repository creation, and the secure-method, ownership-recovery, and installed-application reviews remain private human gates; do not begin `MCP-035` or publish assurance claims early |
...
| D-13 | Evidence-backed enterprise assurance baseline | M3 | Codex | Unscheduled | In progress | `MCP-030` through `MCP-033` have completed the contributor, repository, security, community, licensing, supply-chain, and artifact controls in their accepted OpenSSF OSPS Baseline `v2026.02.19` Level 1 mappings. `MCP-034` is implementing the remaining organization-access, strong-MFA, least-privilege, automation, and private-recovery controls without publishing identities or recovery details; its live organization changes, secure-method confirmation, ownership choice, application-scope review, and recovery exercise remain approval-gated. `MCP-035` must then publish the complete 24-control crosswalk, a dated and scoped self-attestation, a durable security-assurance contract, and only achieved README trust claims; the milestone does not imply OSPS Level 2 or 3, independent certification, regulatory compliance, funded code signing, or attainment of another candidate assurance target. |
...
licensing choices, and `DEC-042` fixes `MCP-033`'s dependency, action,
...
non-disclosing verification work may proceed, but changing organization-wide
base access or repository creation requires explicit owner approval. The
secure-method-only 2FA confirmation, two-owner versus accepted single-owner
recovery choice, private recovery exercise, and private least-privilege review
of installed applications remain unresolved human gates. `MCP-035` stays
proposed and no other M3 ticket is implicitly active.
`DEC-031`, `DEC-036`, and
...
| RISK-16 | A single organization owner becomes unavailable or an over-broad long-lived credential becomes a recovery dependency | High | Low | The 2026-08-09 aggregate audit verifies organization-wide 2FA, read-only workflow-token defaults, no organization or repository Actions secrets, one protected-environment secret, and one used, verified, tap-only write deploy key. It also confirms one owner, no teams, base repository access `read`, member repository creation enabled, and installed-application access awaiting private least-privilege review. `MCP-034` now carries a non-disclosing target policy and fail-closed verifier, while live changes, secure-method confirmation, the recovery choice and exercise, and application-scope review await explicit private owner action | The sole owner loses access, a maintainer cannot recover administration, a future member receives implicit private-repository access or creates an unreviewed repository, an installed application retains unnecessary scope, or one long-lived credential becomes necessary for routine automation | Open — `MCP-034` mitigation in progress |
