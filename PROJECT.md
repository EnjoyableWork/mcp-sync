...
| Current milestone | M4 — the approved bounded Kiro client expansion is complete; `MCP-037` and D-18 are `Done`, the broader milestone remains inactive and partially delivered, and no later arc or side quest is active |
| Overall status | M0 through M3, `MCP-001` through `MCP-020`, `MCP-028` through `MCP-037`, and `MCP-039` are `Done`; `MCP-021` is `Superseded`; `MCP-022` through `MCP-027` are `Deferred`; `SIDE-007` and `SIDE-008` are `Done`; no main-story ticket or side quest is active; funded `SIDE-006` remains proposed; Kiro selection required no `v0.1.0` adoption evidence; and no later work is activated or treated as a prerequisite |
| Current focus | Maintain the completed six-target product, Kiro IDE and inherited Crew contract, configuration-safety boundaries, exact protected checks, immutable release evidence, and tokenless Cargo publication path. Start no later main-story arc or side quest until it is deliberately classified, approved, assigned, and activated under its own exact Goal |
...
| M4 — Approved Kiro client expansion | `MCP-037` (done; no successor selected or active) |
...
depends on its credential boundary. `MCP-037` is completed; no main-story
...
`MCP-037` and D-18 are `Done`. No successor or side quest is selected or active.
...
`MCP-029`, M2, `MCP-030` through `MCP-037`, `MCP-039`, M3, bounded M4
expansion, `SIDE-007`, and `SIDE-008` are complete; no main-story ticket or
side quest is active. The
...
expansion, and no side quest or later implementation is implicitly active.
`DEC-031`, `DEC-036`, and
