...
| Overall status | M2 and `MCP-030` through `MCP-033` are `Done`; `MCP-034` is `In progress` under its exact canonical Goal, while `MCP-035` remains proposed and dormant; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Complete `MCP-034`'s non-disclosing private evidence and live verification after the explicitly authorized default-no-access and member-repository-creation controls were applied; exact secure-method-only confirmation and a successful-result confirmation for the private recovery exercise remain human gates; do not begin `MCP-035` or publish assurance claims early |
| Milestone target | Unscheduled |
| Last reviewed | 2026-08-09 |
| Next review trigger | `MCP-034` activation, completion, or blocker; an organization-access, recovery, Actions, dependency-update, workflow-trust, distribution-authentication, repository-artifact, `main` ruleset/check-context, repository-security-control, community-contract, repository-inventory, or licensing change; an assurance-framework revision; a security or release incident; evidence drift; or explicit funding and activation of `SIDE-006` |
...
| D-13 | Evidence-backed enterprise assurance baseline | M3 | Codex | Unscheduled | In progress | `MCP-030` through `MCP-033` have completed the contributor, repository, security, community, licensing, supply-chain, and artifact controls in their accepted OpenSSF OSPS Baseline `v2026.02.19` Level 1 mappings. Under `MCP-034`, explicit owner approval changed organization base access from `read` to `none` and disabled member repository creation, the sole owner accepted the residual-risk recovery exception, and the private application and recovery reviews were reported complete without publishing identities or recovery details. Exact secure-method-only and successful-recovery confirmations plus the complete non-disclosing verifier still gate closure. `MCP-035` must then publish the complete 24-control crosswalk, a dated and scoped self-attestation, a durable security-assurance contract, and only achieved README trust claims; the milestone does not imply OSPS Level 2 or 3, independent certification, regulatory compliance, funded code signing, or attainment of another candidate assurance target. |
...
| MCP-034 | Establish organization access, credential, and ownership recovery policy | M3 | P1 | Codex | In progress | `MCP-033` | The [non-sensitive organization access and recovery contract](docs/organization-access-and-recovery.md), aggregate-only [private-evidence template generator](scripts/prepare-organization-access-evidence.sh), read-only [operator verifier](scripts/verify-organization-access-controls.sh), synthetic [acceptance and rejection exercise](scripts/test-organization-access-policy.sh), and [focused regressions](tests/organization_access.rs) encode strong MFA, default-no-access manual grants, deliberate repository creation, real rather than nominal teams and owners, private recovery evidence, and bounded automation without printing identities or sensitive material. The initial aggregate audit verified required 2FA, read-only workflow-token defaults, no organization or repository Actions secrets, one protected tap-only deploy key, one owner, no teams, and the two live access gaps. After explicit approval, authenticated update and read-back set base repository access to `none` and disabled member repository creation. The owner accepted the single-owner recovery exception and privately confirmed the application review and same-day recovery exercise; exact secure-method-only and successful-exercise confirmations, private aggregate evidence, and the complete verifier remain open. No OSPS claim is published. |
...
1. Keep completed `MCP-030` through `MCP-033` controls active. `MCP-034` is the
   only `Ready` successor, remains unassigned and inactive, and may begin only
   after its exact canonical Goal is intentionally activated.
2. Complete `MCP-034` and `MCP-035` in order. Do not activate a successor
...
choice.
`MCP-029`, M2, and `MCP-030` through `MCP-033` are complete. `MCP-034` is
`In progress` under its exact canonical Goal. Its repository-local policy and
non-disclosing verification work is implemented. The explicitly authorized
organization update set base access to `none` and disabled member repository
creation; the owner accepted the single-owner exception and reported the
private application review and recovery exercise complete. Exact
secure-method-only and successful-exercise confirmations, private aggregate
evidence, and a passing complete verifier remain unresolved gates. `MCP-035`
stays proposed and no other M3 ticket is implicitly active.
`DEC-031`, `DEC-036`, and
...
| RISK-16 | A single organization owner becomes unavailable or an over-broad long-lived credential becomes a recovery dependency | High | Low | The 2026-08-09 aggregate audit verifies organization-wide 2FA, read-only workflow-token defaults, no organization or repository Actions secrets, one protected-environment secret, and one used, verified, tap-only write deploy key. Explicitly authorized live changes set base repository access to `none` and disabled member repository creation. The sole owner accepted the residual-risk single-owner exception and reported the private installed-application review and same-day recovery exercise complete; the non-disclosing policy and fail-closed verifier require exact secure-method and successful-exercise confirmation before closure | The sole owner loses access, a maintainer cannot recover administration, a future member receives implicit private-repository access or creates an unreviewed repository, an installed application retains unnecessary scope, or one long-lived credential becomes necessary for routine automation | Open — `MCP-034` mitigation in progress; final private confirmations and live verifier pending |
