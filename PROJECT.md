...
| Current focus | Merge the protected `MCP-029` activation change, obtain exact-main CI and both preflight runs, then execute the zero-cost `v0.1.0` runbook with the owner-confirmed short-lived `publish-new` credential scoped to exact crate pattern `enjoyable-mcp-sync`, without activating paid signing or deferred channels |
| Milestone target | 2026-08-08 |
| Last reviewed | 2026-08-08 |
| Next review trigger | Merge of the `MCP-029` activation change; completion of exact-main release authorization; publication of `v0.1.0`; or intentional activation of a side quest |
...
| D-12 | Zero-cost market-validation release and installed recovery exercise | M2 | Codex | 2026-08-08 | In progress | `MCP-028` is complete through the [zero-cost workflows](.github/workflows/market-release.yml), [deterministic source generator](scripts/generate-market-release-channels.sh), and [market runbook](docs/market-release.md). `MCP-029` is active under its exact goal; ready-for-review [PR #28](https://github.com/EnjoyableWork/mcp-sync/pull/28) adds the protected tap-scoped Homebrew publisher. Activation commit [`0b7738e`](https://github.com/EnjoyableWork/mcp-sync/commit/0b7738eef76113f042f727894f45015abe4758e9) passes [market preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31273731058), retained [six-target preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31273731059), [push CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31273728317), and [pull-request CI rerun](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31273731060/attempts/2); the rerun confirms that the original Windows x64 five-second fixture timeout was transient without changing source. The signed-in crates.io profile reports a verified email without exposing it, the registry still reports no `enjoyable-mcp-sync`, and the owner revoked the UI-exposed intermediate credential before installing a fresh seven-day first-publication token limited to `publish-new` and exact crate pattern `enjoyable-mcp-sync` in Cargo's owner-only credential store. The public organization tap and protected repository-scoped deploy key are ready. The ticket must still merge the activation change, publish immutable `v0.1.0`, crates.io `enjoyable-mcp-sync` `0.1.0`, and the source-built organization Homebrew formula from one commit, and verify their supported-host installation and recovery journeys without project-issued macOS or Windows binaries |
...
2. Merge the protected Homebrew publisher; preserve the verified crates.io
   identity and repository-scoped tap deploy key; and replace the current
   broader token with a short-lived credential limited to `publish-new` and
   exact crate pattern `enjoyable-mcp-sync`, without exposing either
   credential. Revoke publication tokens after the exact crate is public and
   verified.
3. Follow the [market-release runbook](docs/market-release.md) from one exact
   current-`main` commit, recheck every public identity, and exercise the
   release-control and `release` environment reviews before creating the one
   annotated tag or publishing any downstream record.
...
| RISK-17 | A release tag or publication credential is changed or misused before immutable release controls take effect | Critical | Low | The active `v*` ruleset restricts tag creation, update, and deletion; the protected release environment limits stable-tag deployment; release immutability is enabled; and `DEC-031` carries those controls through `MCP-028`, `MCP-029`, and later `SIDE-006`. `MCP-028` proves that only the exact `v0.1.0` market workflow responds to that tag, while the funded workflow has no tag trigger, requires an explicit manual confirmation, and rejects `v0.1.0`; the authorization workflow requires exact-current-main CI plus both preflights without receiving tag-write authority. The public tap accepts one write-enabled deploy key scoped only to that repository, its private half exists only in the protected `release` environment, and the Homebrew publisher can create only an absent byte-identical release formula after the immutable GitHub and crates.io bytes pass | Release automation can create, move, delete, or publish a stable tag outside the authorized path, the free tag can invoke the paid workflow, or a downstream credential can mutate broader repository state | Mitigated for trigger and tap credential scope — `MCP-029` publication gate remains open |
