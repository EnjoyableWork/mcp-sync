...
| Overall status | M2 is `Done`; `MCP-030` is `Done`; only `MCP-031` is `Ready`, unassigned, and not active; `MCP-032` through `MCP-035` remain proposed and dormant; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Intentionally activate `MCP-031` under its exact Goal and an assigned owner when work begins; do not begin `MCP-032` or later assurance work early, and preserve the completed `MCP-030` controls and evidence |
...
1. Keep completed `MCP-030` controls active, and intentionally activate only
   `MCP-031`, the earliest incomplete main-story ticket, by assigning an owner
   and reconciling a thread to its exact canonical Goal. `MCP-031` is `Ready`,
   but no successor ticket or Goal is active yet.
...
M2, and `MCP-030` are complete; only `MCP-031` is `Ready`, unassigned, and not
active under a Goal. Organization continuity and recovery choices remain scoped
