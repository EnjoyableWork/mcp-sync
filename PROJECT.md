...
| Known open defects | Issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46) is fixed and ready for closure on merge by draft [PR #66](https://github.com/EnjoyableWork/mcp-sync/pull/66) and exact implementation head [`9472a7a`](https://github.com/EnjoyableWork/mcp-sync/commit/9472a7ab946f424df8c157ab35fe5eab9f1a6bb5), whose 24 CI, CodeQL, release-preflight, and source/install/archive checks passed on attempt one. Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) records the post-`SIDE-008` recurrence of Windows ARM64 health-fixture startup timing without establishing a product defect and is the sole `Ready` correction as `MCP-044`. Issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44) is closed by merged [PR #65](https://github.com/EnjoyableWork/mcp-sync/pull/65) and exact-main commit [`991e6b2`](https://github.com/EnjoyableWork/mcp-sync/commit/991e6b2114267fb26c3beb3a319e96de0452c7e2), whose CI and both native preflights passed on attempt one |
| Current milestone | Post-M4 release readiness — `MCP-042`, `MCP-043`, D-21, and D-22 are `Done`; only `MCP-044` is `Ready`; `MCP-045` remains `Proposed`; and no side quest is active |
| Overall status | M0 through M3, `MCP-001` through `MCP-020`, `MCP-028` through `MCP-037`, and `MCP-039` through `MCP-043` are `Done`; `MCP-044` is `Ready`; `MCP-045` is `Proposed`; `MCP-021` is `Superseded`; `MCP-022` through `MCP-027` are `Deferred`; `SIDE-007` and `SIDE-008` are `Done`; and funded `SIDE-006` remains proposed |
| Current focus | Review and merge draft [PR #66](https://github.com/EnjoyableWork/mcp-sync/pull/66), which will close issue #46. When intentionally starting the sole ready successor, reconcile the thread Goal to `MCP-044` before moving it to `In progress`. Keep `MCP-045`, every deferred expansion, and funded `SIDE-006` inactive until their predecessors and separate Goal contracts permit activation |
...
ready but not active, while `MCP-045` remains proposed.
...
completed. `MCP-042` and `MCP-043` are complete, `MCP-044` is ready but not
active, `MCP-045` remains proposed, and no side quest is active.
...
1. Review and merge draft [PR #66](https://github.com/EnjoyableWork/mcp-sync/pull/66),
   whose exact implementation head completes `MCP-043` and passes all 24
   hosted checks on attempt one. Merge will close issue #46.
2. Keep sole ready successor `MCP-044` inactive until intentionally starting
   it under its exact catalog Goal. Keep `MCP-045` proposed and ordered;
   publication additionally requires separate explicit approval and live
   control checks.
...
environment-name validation, ready `MCP-044` deterministic Windows ARM64
...
is ready but not active, and no side quest is active. The
...
and completed only `MCP-043`, leaving `MCP-044` ready and `MCP-045` proposed.
`DEC-031`, `DEC-036`, and
...
| RISK-23 | A canonical environment name changes identity or becomes invalid at a supported process boundary | High | Medium | Completed `MCP-043`, `DEC-053`, and D-22 enforce the portable non-empty/no-NUL/no-`=` invariant through one shared `config.rs` validator. The traced consumers cover validation-before-lock `add`, canonical reads, all six native adapters before `init` creation or sync planning, restore validation, and health before process launch. Existing ownership variation is explicit: Claude fails structurally, while Cursor, Windsurf, VS Code, Codex, and Kiro preserve invalid local entries as unmanaged and reject collisions. Focused model, adapter, fixture, redaction, valid-edge, and built-binary regressions plus native macOS, GNU/Linux, and Windows x64/ARM64 evidence prove the boundary without normalization, mutation, execution, dropping, or disclosure. Draft PR #66 exact implementation head `9472a7a` passes all 24 hosted checks on attempt one | `add`, canonical parsing or construction, native import, sync, restore validation, list, or `test` accepts a name that cannot retain exact identity on macOS, GNU/Linux, or Windows; an invalid name is normalized, imported, executed, or dropped; an unmanaged native entry is overwritten; a command mutates before rejection; or a diagnostic exposes invalid key text or an environment value | Mitigated — `MCP-043` is complete and issue #46 is ready for closure on merge; retain regressions and native gates |
| RISK-24 | Scheduler-dependent tests hide regressions behind retries or generous timing | High | High | `DEC-052` makes deterministic synchronization and no-retry evidence repository policy. The historical CI audit distinguished repeatable implementation failures from identical-source variance in process health fixtures, including issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) and `MCP-042`'s macOS x64 failure where a reparented inherited-stdout holder crossed between ancestry snapshots. Product time limits and outer watchdogs remain bounded, while readiness uses observable state or handshakes and every race correction requires one forced-state regression plus supplemental native repetition | Identical source both passes and fails; a race or timeout disappears only on rerun; correctness depends on a sleep, polling frequency, runner speed, or broader timeout; automatic retry, quarantine, assertion weakening, or unjustified serialization is proposed; or a native boundary lacks a deterministic lower-layer proof | Open — policy accepted and `MCP-042` exact-head no-retry proof passes; retain the policy across the repository, while ordered `MCP-044` owns the remaining Windows ARM64 readiness correction |
