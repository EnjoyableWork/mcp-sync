...
| Overall status | M0 through M3, `MCP-001` through `MCP-035`, and `SIDE-007` are `Done`; no later main-story milestone is active, `SIDE-008` is the one independent side quest In progress, and funded `SIDE-006` remains proposed |
| Current focus | Complete `SIDE-008` by eliminating duplicate pull-request CI executions and stabilizing the Windows health-process fixture without changing the product's five-second response boundary; continue maintaining the terminal-first README, completed M3 assurance, and immutable M2 release |
| Milestone target | Completed 2026-08-09 |
| Last reviewed | 2026-08-09 |
| Next review trigger | A README target-state promise, install channel, call to action, navigation anchor, live badge or link, client boundary, or safety contract changes; an organization-access, recovery, Actions, dependency-update, workflow-trust, distribution-authentication, repository-artifact, `main` ruleset/check-context, repository-security-control, community-contract, repository-inventory, or licensing change; an assurance-framework, issuer-status, badge, or public-proof-link revision; a security or release incident; evidence drift; deliberate selection of a later main-story arc; or explicit funding and activation of `SIDE-006` |
...
changing the product, release, signing, or assurance contract. `SIDE-008` is
the one active independent side quest and owns the approved CI-reliability
correction. `DEC-038` promotes and supersedes `SIDE-001` through
...
funding-gated. `SIDE-007` is complete, `SIDE-008` is the one active independent
reliability correction, and all remaining objectives stay recorded in the
...
| SIDE-008 | Eliminate duplicate pull-request CI and stabilize Windows health-process fixture timing | P2 | Codex | In progress | PR #43's [branch-push CI run](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31340410689) failed only because the shutdown-resistant PowerShell fixture did not initialize within its five-second test allowance, while the independently duplicated [pull-request CI run](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31340453263) passed the identical Windows x64 job at the same commit. Done when branch pushes with an open pull request no longer create a second CI suite, `main` and pull requests retain the exact protected check contexts and native x64/ARM64 coverage, test-only Windows fixture startup headroom is distinct from the unchanged product limit, focused regressions and full local gates pass, and one clean non-duplicated PR suite provides durable native evidence. |
...
1. Maintain the completed `SIDE-007` README product-page hierarchy, live
...
2. Maintain the completed `MCP-030` through `MCP-035` controls, public evidence,
...
3. Keep M4, M5, and every side quest inactive until a later outcome is
   deliberately selected, accepted, assigned, and activated under its own
   canonical Goal; completing M3 or `SIDE-007` does not choose a successor.
4. Preserve immutable `v0.1.0`, its signed annotated release and operational
...
5. Monitor the public GitHub, Cargo, and source-building Homebrew channels and
...
6. Treat OSPS Level 1 as the only M3 assurance gate. Best Practices, NIST SSDF,
...
7. Retain the funded six-target signed-native workflow, preflight, metadata,
...
`MCP-029`, M2, `MCP-030` through `MCP-035`, M3, and `SIDE-007` are complete.
The completed side quest changed only the README product-page presentation; it
did not activate a later main-story arc or alter release or assurance scope.
`MCP-034`'s
