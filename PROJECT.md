...
| Overall status | M2 and `MCP-030` through `MCP-032` are `Done`; `MCP-033` is `Ready` but unassigned and inactive; `MCP-034` and `MCP-035` remain proposed and dormant; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Preserve the completed repository, security, community, and licensing controls; intentionally activate `MCP-033` under its exact canonical Goal before beginning dependency-maintenance or supply-chain work, and do not begin later assurance work early |
| Milestone target | Unscheduled |
| Last reviewed | 2026-08-09 |
| Next review trigger | `MCP-033` activation, completion, or blocker; a `main` ruleset/check-context, repository-security-control, community-contract, repository-inventory, or licensing change; an assurance-framework revision; a security or release incident; evidence drift; or explicit funding and activation of `SIDE-006` |
...
1. Keep completed `MCP-030` through `MCP-032` controls active. `MCP-033` is the
   only `Ready` main-story ticket, but remains unassigned and inactive until a
   separate thread intentionally adopts its exact canonical Goal.
...
licensing choices. `MCP-029`, M2, and `MCP-030` through `MCP-032` are complete;
only `MCP-033` is `Ready`, unassigned, and inactive until its exact Goal is
intentionally adopted. Organization continuity and recovery choices remain
