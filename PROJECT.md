...
| Overall status | `MCP-020` and `MCP-028` are `Done`; `MCP-021` is `Superseded` by `DEC-036`; only `MCP-029` is `Ready` in the main story and remains unassigned; and `SIDE-001` through `SIDE-006` are proposed but inactive |
| Current focus | Keep all publication inactive until an owner intentionally starts `MCP-029`, accepts its exact Goal-mode objective, and supplies the no-cost crates.io, organization Homebrew tap, and protected tag authorities required by the market-release runbook |
| Milestone target | Unscheduled; set after an owner and delivery capacity are known |
| Last reviewed | 2026-08-08 |
| Next review trigger | Intentional activation of `MCP-029`; availability of the no-cost Cargo and Homebrew publication identities; publication of `v0.1.0`; or intentional activation of a side quest |
...
| Authority | Human action before `MCP-029` | Paid signing required? |
| --- | --- | --- |
| crates.io | Sign in, verify the publisher account and email, recheck `enjoyable-mcp-sync`, create a first-publication API token, and store it only in the protected release environment | No |
| `EnjoyableWork/homebrew-tap` | Ensure the public tap repository exists and grant the release identity only the write access required to publish `Formula/mcp-sync.rb` | No |
...
| D-12 | Zero-cost market-validation release and installed recovery exercise | M2 | Unassigned | Unscheduled | In progress | `MCP-028` is complete through the [zero-cost workflows](.github/workflows/market-release.yml), [deterministic source generator](scripts/generate-market-release-channels.sh), [market runbook](docs/market-release.md), exact-commit [market preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31268357596), retained [six-target preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31268357599), and [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31268357600). `MCP-029` must still publish immutable `v0.1.0`, crates.io `enjoyable-mcp-sync` `0.1.0`, and the source-built organization Homebrew formula from one commit and verify their supported-host installation and recovery journeys without project-issued macOS or Windows binaries |
...
1. Keep `MCP-029` `Ready` and unassigned until an owner intentionally starts it
   under its exact Goal-mode objective; no release action is active merely
   because `MCP-028` is complete.
2. Before activation, supply the remaining no-cost human authorities: a
   verified crates.io account and scope-minimized first-publication token,
   least-privilege organization Homebrew tap write access, and deliberate
   authorization through the protected GitHub release path.
3. Follow the [market-release runbook](docs/market-release.md) from one exact
   current-`main` commit, recheck every public identity, and create no tag,
   release, crate, formula, or downstream record until the `MCP-029` owner has
   accepted the publication boundary.
