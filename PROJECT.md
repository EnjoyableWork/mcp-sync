...
| Known open defects | Issue [#46](https://github.com/EnjoyableWork/mcp-sync/issues/46) is closed by merged [PR #66](https://github.com/EnjoyableWork/mcp-sync/pull/66) and exact-main commit [`e96c1c4`](https://github.com/EnjoyableWork/mcp-sync/commit/e96c1c437b5ce9d9f5138b790dd23ad2926fa749), whose 23 CI, CodeQL, release-preflight, and source/install/archive checks passed on attempt one. Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) is fixed and ready for closure on merge by draft [PR #67](https://github.com/EnjoyableWork/mcp-sync/pull/67) and exact implementation head [`9127775`](https://github.com/EnjoyableWork/mcp-sync/commit/9127775b69f6a710bb8e12a1a10f27409088c68a), whose complete native CI matrix, CodeQL, and source/install/archive preflight passed on attempt one. Issue [#44](https://github.com/EnjoyableWork/mcp-sync/issues/44) is closed by merged [PR #65](https://github.com/EnjoyableWork/mcp-sync/pull/65) and exact-main commit [`991e6b2`](https://github.com/EnjoyableWork/mcp-sync/commit/991e6b2114267fb26c3beb3a319e96de0452c7e2), whose CI and both native preflights passed on attempt one |
| Current milestone | Post-M4 release readiness — `MCP-042` through `MCP-044` and D-21 through D-23 are `Done`; only `MCP-045` is `Ready`; and no side quest is active |
| Overall status | M0 through M3, `MCP-001` through `MCP-020`, `MCP-028` through `MCP-037`, and `MCP-039` through `MCP-044` are `Done`; `MCP-045` is `Ready`; `MCP-021` is `Superseded`; `MCP-022` through `MCP-027` are `Deferred`; `SIDE-007` and `SIDE-008` are `Done`; and funded `SIDE-006` remains proposed |
| Current focus | Review and merge draft [PR #67](https://github.com/EnjoyableWork/mcp-sync/pull/67), which will close issue #60. Keep sole-ready `MCP-045`, every deferred expansion, and funded `SIDE-006` inactive until a separate exact Goal and explicit publication approval intentionally activate the repeat release |
...
first real repeat release. `MCP-042` through `MCP-044` are complete; only
`MCP-045` is ready, and it is not active.
...
completed. `MCP-042` through `MCP-044` are complete, only `MCP-045` is ready,
and no main-story ticket or side quest is active.
...
| D-23 | Deterministic Windows ARM64 health-fixture evidence | Post-M4 release readiness | Codex | Before `v0.1.1` | Done | Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60), completed `MCP-044`, draft [PR #67](https://github.com/EnjoyableWork/mcp-sync/pull/67), and exact implementation head [`9127775`](https://github.com/EnjoyableWork/mcp-sync/commit/9127775b69f6a710bb8e12a1a10f27409088c68a) preserve and correct the post-`SIDE-008` recurrence. Failed [attempt 1](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31513698204/attempts/1) proves two generated PowerShell fixtures inside the 224-test unit binary did not return output within five- and fifteen-second response allowances while another equivalent PowerShell handshake later exceeded five seconds; the unchanged rerun passed. This isolates interpreter startup and scheduler contention from protocol and cleanup behavior. The correction removes those processes from the parallel unit surface, retains pure response-framing tests, and uses one sequential native-Rust process harness with an explicit ready/release state transition under one outer watchdog rather than a fixture-startup allowance. Windows process-exit observation is native and fails closed on unexpected inspection errors; the real five-second response, 500-millisecond shutdown, redaction, direct-child, and complete process-tree assertions remain. Ten consecutive local native-harness runs, the complete synthetic-home suite, strict warning-as-error compilation for all six target triples, dependency policy, workflow lint, locked package verification, documentation/diff checks, and both live read-only main-control verifiers pass; the ruleset still requires the exact five app-bound contexts. Attempt-one [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225214) passes dependency policy plus native GNU/Linux and Windows x64/ARM64, with the Windows ARM64 job completing all ten fail-fast focused health runs. [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634222077) and [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225001) also pass on attempt one. [Release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225071) is green after its only failed job recovered from an explicit upstream GitHub `503` during pinned Syft acquisition; the same GNU/Linux ARM64 SBOM path passed independently on the original source-preflight attempt, and the external-service recovery is not health-fix acceptance evidence. All 24 current PR checks pass, issue #60 is ready for closure on merge, only `MCP-045` is ready, and no version, release artifact, or side quest changed. |
...
| MCP-044 | Make Windows ARM64 health-fixture readiness deterministic | Post-M4 release readiness | P1 | Codex | Done | `MCP-043` | Issue [#60](https://github.com/EnjoyableWork/mcp-sync/issues/60) and D-23 are resolved by draft [PR #67](https://github.com/EnjoyableWork/mcp-sync/pull/67) and exact implementation head [`9127775`](https://github.com/EnjoyableWork/mcp-sync/commit/9127775b69f6a710bb8e12a1a10f27409088c68a). Preserved recurrence evidence isolates PowerShell interpreter startup and scheduler contention inside the parallel unit binary rather than product protocol or cleanup behavior. Pure response-framing tests plus one sequential native-Rust harness with explicit ready/release state remove that timing dependency while retaining the five-second response, 500-millisecond shutdown, redaction, direct-child, and complete process-tree boundaries. Ten local repetitions and attempt-one [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225214), including all ten fail-fast Windows ARM64 focused runs and the complete native x64/ARM64 matrix, pass without a health retry. Attempt-one [CodeQL](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634222077) and [source/install/archive preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225001) pass; [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31634225071) is green after a separately classified upstream GitHub `503` during pinned Syft acquisition was operationally recovered. That external fetch is not acceptance evidence for the deterministic fixture correction. Issue #60 is ready for closure on merge; no version or artifact changed, and no side quest was activated. |
| MCP-045 | Publish and verify `v0.1.1` as the first repeat release | Post-M4 release readiness | P1 | Unassigned | Ready | `MCP-044` | After separate activation and explicit publication approval, prepare the exact version and lockfile plus `docs/releases/v0.1.1.md`, document Homebrew/Cargo/GitHub upgrade paths, pass every local and exact-main gate, revalidate live repository and Trusted Publisher controls, authorize one annotated exact-main `v0.1.1` tag, publish the immutable attested seven-asset source/GNU/Linux GitHub Release, publish byte-identical Cargo through tokenless Trusted Publishing, update the source-building Homebrew formula monotonically, pass the read-only native channel matrix, and record durable evidence. No macOS or Windows project binary, WinGet package, paid signing, moved tag, replaced asset, or `SIDE-006` activation is permitted. |
...
1. Review and merge draft [PR #67](https://github.com/EnjoyableWork/mcp-sync/pull/67),
   whose completed `MCP-044` correction makes issue #60 ready for closure.
2. Keep sole-ready `MCP-045` inactive; publication requires its own exact Goal,
   separate explicit approval, and live control checks.
...
bounded M4 expansion, `SIDE-007`, and `SIDE-008` are complete; only `MCP-045`
is ready, and no main-story ticket or side quest is active. The
...
completed only `MCP-043` and then `MCP-044`. `MCP-045` is ready but remains
inactive pending its own exact Goal and explicit publication approval.
`DEC-031`, `DEC-036`, and
