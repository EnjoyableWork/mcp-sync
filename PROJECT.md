...
| Overall status | `MCP-020` is `Done`; `MCP-021` is the sole `In progress` main-story ticket under Codex ownership, and `SIDE-001` through `SIDE-005` are proposed but inactive |
| Current focus | The merged fail-closed release foundation and exact `main` preflight are green; finish deterministic downstream metadata and read-only all-channel smoke while the external Apple, Microsoft, Cargo, Homebrew, and WinGet publication authorities are provisioned, and do not create `v0.1.0` early |
| Milestone target | Unscheduled; set after an owner and delivery capacity are known |
| Last reviewed | 2026-08-08 |
| Next review trigger | Review of the downstream channel generator and smoke workflow; availability of both signing authorities; publication of `v0.1.0`; or intentional activation of a side quest |
...
and `MCP-021` must build and smoke-test each advertised OS/CPU
...
| D-11 | Accessible release channels and installed recovery exercise | M2 | Codex | Unscheduled | In progress | The active `v*` ruleset restricts stable-tag creation, update, and deletion to the named administrator bypass; the tag-only `release` environment requires review and accepts only `v*`; repository release immutability is enabled; and merged [PR #24](https://github.com/EnjoyableWork/mcp-sync/pull/24) supplies the [packaging scripts](scripts/package-release.sh), [six-target unsigned preflight](.github/workflows/release-preflight.yml), [signed tag workflow](.github/workflows/release.yml), [release runbook](docs/release.md), and installed restore smoke. Exact `main` [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31264234752) and all six native [preflight jobs](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31264234746) pass. The deterministic downstream generator and read-only all-channel native smoke are in review. Completion still requires both signing authorities, the immutable release and attestations, plus verified Cargo, Homebrew, and WinGet publication and per-target install smoke |
...
1. Execute `MCP-021` as the sole active main-story ticket under its exact
   Goal-mode objective and the fixed `DEC-024` / `DEC-031` trust contract.
2. Verify Apple and Windows signing authority, the protected `v*` tag and
   release environment, all public identities, and native six-target capacity
   before creating a stable tag, release artifact, or package metadata.
3. Build and prove the release path without weakening signing, immutability,
   provenance, per-target smoke, installed restore, or same-version channel
   requirements; a missing authority is a precise blocker rather than an
   unsigned fallback.
