...
| Overall status | M2 is `Done`; `MCP-030` is `In progress` under its exact Goal with Codex as owner; `MCP-031` through `MCP-035` remain proposed and dormant; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Complete `MCP-030` by activating and path-testing the contributor-compatible `main` ruleset, recording durable OSPS evidence without changing stable-tag or release-environment controls, and advancing only `MCP-031` after the ticket-done gate passes |
| Milestone target | Unscheduled |
| Last reviewed | 2026-08-09 |
| Next review trigger | `MCP-030` completion or blocker, a `main` ruleset/check-context change, an assurance-framework revision, a security or release incident, evidence drift, or explicit funding and activation of `SIDE-006` |
...
`DEC-038` establishes the ordered M3 assurance arc, and `DEC-039` resolves
`MCP-030`'s approval, merge-method, bypass, and signed-commit choices. `MCP-029`
and M2 are complete; `MCP-030` is the only active product ticket under its exact
Goal. Organization continuity and recovery choices remain scoped to later
`MCP-034`; no other M3 ticket is implicitly active. `DEC-031`, `DEC-036`, and
