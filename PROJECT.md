...
| Overall status | M2 and `MCP-030` through `MCP-033` are `Done`; only `MCP-034` is `Ready` but unassigned and inactive, while `MCP-035` remains proposed and dormant; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Preserve the completed `MCP-030` through `MCP-033` controls and intentionally activate only `MCP-034` with its exact canonical Goal when organization-access and recovery work is deliberately started; do not begin `MCP-035` or publish assurance claims early |
...
| D-13 | Evidence-backed enterprise assurance baseline | M3 | Codex | Unscheduled | In progress | `MCP-030` through `MCP-033` have completed the contributor, repository, security, community, licensing, supply-chain, and artifact controls in their accepted OpenSSF OSPS Baseline `v2026.02.19` Level 1 mappings; only `MCP-034` is `Ready` for the remaining organization controls. `MCP-035` must then publish the complete 24-control crosswalk, a dated and scoped self-attestation, a durable security-assurance contract, and only achieved README trust claims; the milestone does not imply OSPS Level 2 or 3, independent certification, regulatory compliance, funded code signing, or attainment of another candidate assurance target. |
...
`MCP-029`, M2, and `MCP-030` through `MCP-033` are complete. Only
`MCP-034` is `Ready`; it remains unassigned and inactive until its exact
canonical Goal is intentionally activated. Organization continuity and
recovery choices remain scoped to that ticket, while `MCP-035` stays proposed;
no other M3 ticket is implicitly active.
`DEC-031`, `DEC-036`, and
