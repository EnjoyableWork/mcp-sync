...
| Current milestone | M2 — City, complete |
| Overall status | `MCP-020`, `MCP-028`, and `MCP-029` are `Done`; `MCP-021` is `Superseded` by `DEC-036`; no main-story ticket or side quest is active; and `SIDE-001` through `SIDE-006` remain proposed |
| Current focus | Preserve and monitor the completed M2 implementation and protected zero-cost `v0.1.0` channels, gather market feedback without changing the release boundary, and require intentional classification, ownership, and a matching Goal-mode objective before activating another main-story arc or any side quest |
| Milestone target | 2026-08-08 |
| Last reviewed | 2026-08-08 |
| Next review trigger | A release or channel incident, evidence drift, or intentional classification and activation of the next main-story arc or one side quest |
...
The main story is defined and complete through M2:
...
1. Keep `MCP-029` and M2 `Done`; no main-story ticket or side quest is active,
   and completion does not implicitly activate M3, M4, or `SIDE-001` through
   `SIDE-006`.
2. Preserve immutable `v0.1.0`, its signed annotated release and operational
...
3. Monitor the public GitHub, Cargo, and source-building Homebrew channels and
...
4. Retain the funded six-target signed-native workflow, preflight, metadata,
...
5. Before starting more implementation, intentionally classify the next work
   as an ordered main-story arc or a genuinely independent side quest, make its
   outcome and evidence clear, assign an owner, and reconcile its canonical
   objective in a fresh thread.
...
sequence, and `DEC-037` fixes the completed immutable-release recovery path.
`MCP-029` and M2 are complete, and no product ticket or decision is active.
`DEC-031`, `DEC-036`, and `DEC-037` remain the release boundary; protected
publication access is durable evidence under `RISK-17`, while paid signing
authority under `RISK-12` blocks only dormant `SIDE-006`. The pull-request
approval threshold, allowed merge methods, and signed-commit enforcement
choice remain scoped to `SIDE-001`; organization continuity choices remain
scoped to `SIDE-005`. None is implicitly activated.
...
| RISK-06 | Broad client/platform scope delays the first usable proof | High | High | M1 and the strictly sequenced M2 are complete; the WIP limit and required classification prevent deferred M3, M4, or side-quest scope from silently entering an inactive milestone | Work starts without an accepted next arc or independent side-quest classification | Mitigated — M2 sequence complete; enforce intentional activation |
