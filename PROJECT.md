...
Approved M4 successor `MCP-037` follows in the ordered story without any
`v0.1.0` adoption-evidence prerequisite. Its exact canonical Goal is now
active under Codex, so it is the sole `In progress` main-story ticket:
...
| M4 — Approved Kiro client expansion | `MCP-037` (in progress under its exact canonical Goal) |
...
depends on its credential boundary. `MCP-037` is active as the sole
main-story ticket; no side quest is active.
...
| D-18 | Global-user Kiro adapter with inherited Kiro Crew coverage | M4 | Codex | Current ticket | In progress | Active `MCP-037` adds one bounded global-user Kiro target under its exact canonical Goal, revalidates current default and `KIRO_HOME` path, precedence, interpolation, and Crew inheritance contracts, preserves every unowned or unrepresentable entry, extends the complete six-target import/sync/restore/rollback/lock/native matrix, and requires controlled current-client Kiro IDE and Kiro Crew evidence before any support claim. Its approval has no `v0.1.0` adoption-evidence prerequisite. |
...
| MCP-037 | Add a global-user Kiro MCP adapter with inherited Kiro Crew coverage | M4 | P1 | Codex | In progress | `MCP-039` | `DEC-046` approves Kiro as the ordered M4 successor without requiring `v0.1.0` adoption evidence, and its exact canonical Goal is active. The implementation revalidates the current global and `KIRO_HOME` paths, precedence, interpolation, and Kiro Crew inheritance contracts; owns only canonical-compatible local STDIO fields in the selected global file; preserves unowned and unrepresentable data; never accesses higher-precedence, Crew-only, generated-agent, managed, or credential stores; extends deterministic six-target configuration, recovery, rollback, lock, and native-platform evidence; and completes controlled current-client Kiro IDE and inherited Kiro Crew smokes before any support claim. |
...
requiring `v0.1.0` adoption evidence. Completed `MCP-039` satisfies its
predecessor gate, and the owner deliberately activated the ticket's exact
canonical Goal under Codex on 2026-08-11. The ticket is `In progress`; Kiro
remains unimplemented and unsupported until every acceptance gate below passes.
...
1. Complete active `MCP-037` under its exact canonical Goal. Revalidate Kiro's
   current path, schema, precedence, interpolation, IDE, and Crew contracts
   before implementation; preserve all five existing targets and every safety
   boundary; and make no Kiro support claim before the complete six-target,
   native-platform, and controlled current-client evidence passes.
...
current-client evidence, and the now-active exact Goal remain required. `DEC-047`
...
`MCP-029`, M2, `MCP-030` through `MCP-036`, `MCP-039`, M3, `SIDE-007`, and
`SIDE-008` are complete; approved successor `MCP-037` is `In progress`
under Codex as the sole main-story ticket, and no side quest is active. The
...
continuation after `MCP-036`; `MCP-037` is deliberately active, and no side
