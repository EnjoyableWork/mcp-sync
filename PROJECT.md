...
| Product state | The source-checkout macOS, GNU/Linux, and Windows implementation reconciles global Claude Desktop, Cursor, Windsurf legacy Cascade, native VS Code default user-profile, and global Codex host configuration, exposes one bounded named-server STDIO initialize health boundary, and provides parser-validated restore with one-generation adjacent backup retention for canonical state plus all five targets. Deterministic fixtures and native x64/ARM64 GNU/Linux and Windows MSVC CI prove the pre-restore five-target configuration behavior; the restore journey passes locally on ARM64 and Intel macOS, resolves every supported platform path in tests, and compiles every test surface for all six supported architecture targets. The credential-free source and GNU/Linux release preflight now proves deterministic Cargo and source-built Homebrew outputs, exact-package installation and recovery across all six native hosts, Homebrew installation and recovery across four native Unix hosts, and the two represented GNU/Linux archive/SBOM paths while the retained six-target release preflight remains green. Claude Desktop and Cursor retain macOS current-client sync smoke evidence, no Linux or Windows target has a current-client smoke claim, no distribution channel is published yet, and the repository remains pre-release |
| Current milestone | M2 — City, in progress |
| Overall status | `MCP-020` and `MCP-028` are `Done`; `MCP-021` is `Superseded` by `DEC-036`; `MCP-029` is `In progress` under its exact Goal-mode objective with Codex as owner; and `SIDE-001` through `SIDE-006` are proposed but inactive |
| Current focus | Correct the `MCP-029` release-authorization 403 without storing a repository-administration credential in Actions, merge the operator-verified exact-commit authorization contract, repeat fresh exact-main CI and both preflights, then execute the zero-cost `v0.1.0` runbook with the owner-confirmed short-lived `publish-new` credential scoped to exact crate pattern `enjoyable-mcp-sync`, without activating paid signing or deferred channels |
| Milestone target | 2026-08-08 |
| Last reviewed | 2026-08-08 |
| Next review trigger | Merge of the `MCP-029` release-authorization correction; completion of exact-main release authorization; publication of `v0.1.0`; or intentional activation of a side quest |
...
repository-hardening work, while `SIDE-006` is the funding-dependent signed
native distribution stretch; all remain non-gating unless an accepted decision
...
| M2 | City — dependable daily tool | README client coverage, health checks, cross-platform behavior, restore UX, and a zero-cost market-validation release | Supported matrix passes in CI and one protected tagged release installs and smoke-tests through every channel represented by the M2 release without requiring paid platform signing | In progress |
...
`SIDE-006`. Free does not mean authority-free: `MCP-029` still requires a
no-cost [crates.io account and first-publication token](https://doc.rust-lang.org/cargo/reference/publishing.html),
write access to the organization Homebrew tap, and the already protected GitHub
release path. The [Homebrew formula contract](https://docs.brew.sh/Formula-Cookbook)
...
The remaining human work for the zero-cost release is explicit:

| Authority | Human action for `MCP-029` publication | Paid signing required? |
| --- | --- | --- |
| crates.io | Sign in, verify the publisher account and email, recheck `enjoyable-mcp-sync`, create a short-lived first-publication API token limited to the `publish-new` endpoint and exact crate-name pattern `enjoyable-mcp-sync`, and store it only in Cargo's credential store on the controlled publisher host. crates.io accepts patterns for future crates | No |
| `EnjoyableWork/homebrew-tap` | Ensure the public tap repository exists, grant one repository-scoped deploy key write access, and store that key only as `HOMEBREW_TAP_DEPLOY_KEY` in the protected `release` environment | No |
| `EnjoyableWork/mcp-sync` | Authorize the protected `v0.1.0` tag and release deployment after `MCP-028` evidence passes | No |
| Apple and Windows signing | Take no action for `MCP-028` or `MCP-029`; provision these authorities only after explicit funding and activation of `SIDE-006` | Yes, deferred |
...
| D-12 | Zero-cost market-validation release and installed recovery exercise | M2 | Codex | 2026-08-08 | In progress | `MCP-028` is complete through the [source and GNU/Linux publisher](.github/workflows/source-linux-release.yml), [deterministic source generator](scripts/generate-source-linux-release-channels.sh), and [release runbook](docs/source-linux-release.md). Merged [PR #28](https://github.com/EnjoyableWork/mcp-sync/pull/28) adds the protected tap-scoped Homebrew publisher, and merged [PR #29](https://github.com/EnjoyableWork/mcp-sync/pull/29) gives reusable automation technical source/Linux names. Exact-main commit [`de6f84b`](https://github.com/EnjoyableWork/mcp-sync/commit/de6f84bef390c506a8989b03d68e4123c1f7b86b) passes [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31280981444), [source and GNU/Linux preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31280981440), and retained [six-target preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31280981439). The owner approved the protected [authorization run](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31281291453), which passed the exact commit, version, and three gate checks but then exposed that Actions' `GITHUB_TOKEN` cannot read the administration-scoped immutable-release setting. The in-progress correction keeps that live check on the controlled operator host, records explicit exact-commit confirmation at dispatch, independently verifies the public tag ruleset, and makes every publisher require a successful protected authorization without storing an administration PAT. The signed-in crates.io profile reports a verified email without exposing it, the registry still reports no `enjoyable-mcp-sync`, and the owner revoked the UI-exposed intermediate credential before installing a fresh seven-day first-publication token limited to `publish-new` and exact crate pattern `enjoyable-mcp-sync` in Cargo's owner-only credential store. The public organization tap and protected repository-scoped deploy key are ready. The ticket must still merge the authorization correction, repeat the exact-main gates and protected authorization, publish immutable `v0.1.0`, crates.io `enjoyable-mcp-sync` `0.1.0`, and the source-built organization Homebrew formula from one commit, and verify their supported-host installation and recovery journeys without project-issued macOS or Windows binaries |
...
| MCP-029 | Publish and verify the zero-cost `v0.1.0` market-validation release | M2 | P1 | Codex | In progress | `MCP-028` | The exact protected `v0.1.0` commit is published as one immutable GitHub Release with attested GNU/Linux x64/ARM64 artifacts, `enjoyable-mcp-sync` `0.1.0` on crates.io, and `EnjoyableWork/tap/mcp-sync` as a source-building formula; all represented native install and recovery smokes pass; the crate name and public identities are reverified; current release notes state the macOS, Windows, and WinGet boundary without implying unsigned binaries; publication evidence is durable; and the M2 milestone gate passes |
...
recovery material. All six tickets have dormant objectives in the
...
1. Keep `MCP-029` `In progress` under Codex's exact Goal-mode objective; do not
   activate another main-story ticket or any side quest while it is unfinished.
2. Merge the release-authorization correction that keeps administration-scoped
   repository checks on the controlled operator host, records explicit
   exact-commit confirmation, and requires that authorization in every
   publisher without storing an administration PAT. Preserve the verified
   crates.io identity, repository-scoped tap deploy key, and short-lived
   credential limited to `publish-new` and exact crate pattern
   `enjoyable-mcp-sync`, without exposing either credential. Revoke publication
   tokens after the exact crate is public and verified.
3. Follow the [source and GNU/Linux release runbook](docs/source-linux-release.md)
   from one exact current-`main` commit, recheck every public identity, and
   exercise the `release-control` and `release` environment reviews before
   creating the one annotated tag or publishing any downstream record.
4. Preserve the active `v*` protections, immutable-release controls,
   provenance, deterministic metadata, native installation and recovery
   smokes, accurate release notes, and the retained funded pipeline while
   limiting project-issued `v0.1.0` binaries to GNU/Linux x64/ARM64.
5. Keep `SIDE-001` through `SIDE-006` proposed and non-gating until one is
...
| DEC-014 | Use `enjoyable-mcp-sync` as the Cargo package and `mcp-sync` as its binary | Accepted | 2026-08-06 | The desired package name returned no exact crates.io record while the unrelated `mcp-sync` name is occupied; recheck availability immediately before `MCP-029` publication |
...
| DEC-031 | Protect stable release tags and publication credentials across every release path | Accepted | 2026-08-08 | Release immutability begins only after publication, so `MCP-021` activated a `v*` tag ruleset that restricts creation to the authorized release path and prevents update or deletion outside an explicit emergency bypass. `MCP-028` preserves those controls while separating free and paid automation, `MCP-029` must publish through the protected least-privilege release environment, and `SIDE-006` must use the same standard when activated. GitHub's immutable-release setting requires administration read access unavailable to the standard Actions token, so a controlled operator-side verifier must bind that live setting plus both protected environment policies to the exact current `main` commit immediately before approval; Actions records explicit confirmation, verifies the public ruleset, and requires the successful exact-commit authorization plus final immutable release state. OIDC or trusted publishing remains preferred where supported; an interactive administration PAT is never stored or reused as a release credential. |
...
| RISK-17 | A release tag or publication credential is changed or misused before immutable release controls take effect | Critical | Low | The active `v*` ruleset restricts tag creation, update, and deletion; both protected environments require review and constrain deployment to exact `main` or `v*` refs; release immutability is enabled; and `DEC-031` carries those controls through `MCP-028`, `MCP-029`, and later `SIDE-006`. The operator-side verifier requires those live administration-scoped controls for the exact current `main` commit without exporting a PAT; authorization records explicit confirmation and exact-current-main CI plus both preflights without receiving tag-write authority; each publisher requires that successful exact-commit authorization and independently verifies the public ruleset. `MCP-028` proves that only the exact `v0.1.0` source and GNU/Linux workflow responds to that tag, while the funded workflow has no tag trigger, requires separate signing and repository-control confirmations, and rejects `v0.1.0`. The public tap accepts one write-enabled deploy key scoped only to that repository, its private half exists only in the protected `release` environment, and the Homebrew publisher can create only an absent byte-identical release formula after the immutable GitHub and crates.io bytes pass | Release automation can create, move, delete, or publish a stable tag outside the authorized path, the `v0.1.0` tag can invoke the funded workflow, an approval proceeds after a stale or failed operator-side check, or a downstream credential can mutate broader repository state | Mitigated for trigger, authorization, and tap credential scope — `MCP-029` publication gate remains open |
| RISK-18 | Source-based macOS and Windows installation creates more setup friction and weakens early conversion evidence | Medium | High | `v0.1.0` offers native GNU/Linux archives, Cargo installation across supported platforms, and source-built Homebrew on macOS/Linux; release notes must state prerequisites and limitations, and market feedback must distinguish product-fit objections from installation friction. The funded native path remains ready to activate after demand and budget justify it | Prospective users cannot complete installation, feedback repeatedly cites the toolchain boundary, or target users require WinGet or notarized macOS binaries before evaluation | Open — measure after `MCP-029`; consider funding `SIDE-006` |
