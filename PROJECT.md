...
| Current focus | The fail-closed release foundation, deterministic downstream metadata, read-only all-channel smoke workflow, and exact `main` preflight are merged and green; provision the external Apple, Microsoft, and Cargo publication authorities before authorizing or creating `v0.1.0` |
| Milestone target | Unscheduled; set after an owner and delivery capacity are known |
| Last reviewed | 2026-08-08 |
| Next review trigger | Availability of both signing authorities and first-publication Cargo access; publication of `v0.1.0`; or intentional activation of a side quest |
