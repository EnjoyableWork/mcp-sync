...
| Overall status | M2, `MCP-030`, and `MCP-031` are `Done`; `MCP-032` is `In progress` under Codex; `MCP-033` through `MCP-035` remain proposed and dormant; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Complete `MCP-032`'s public contribution, community, repository-inventory, and licensing contracts plus their GitHub recognition and evidence; do not begin `MCP-033` or later assurance work early, and preserve the completed repository-protection and security controls |
...
1. Keep completed `MCP-030` and `MCP-031` controls active while Codex completes
   `MCP-032`, the only active main-story ticket, under its exact canonical Goal.
   `MCP-033` remains dormant until `MCP-032` is `Done` and intentionally closed.
...
licensing choices. `MCP-029`,
M2, `MCP-030`, and `MCP-031` are complete; only `MCP-032` is `In progress`
under Codex and its exact Goal. Organization continuity and recovery choices
remain scoped to later `MCP-034`; no other M3 ticket is implicitly active.
`DEC-031`, `DEC-036`, and
