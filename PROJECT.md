...
1. Complete active `MCP-039` under its canonical Goal: implement and verify the
   protected OIDC-only Cargo publication foundation, trusted-publisher identity,
   trusted-only setting, authorization rehearsal, and fail-closed evidence
   without publishing a version or creating an API token.
