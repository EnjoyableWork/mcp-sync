...
| Current focus | Merge the protected `MCP-029` activation change, replace the current over-broad crates.io token with one short-lived credential limited to `publish-new` and exact crate pattern `enjoyable-mcp-sync`, then execute the zero-cost `v0.1.0` runbook from one exact green `main` commit without activating paid signing or deferred channels |
| Milestone target | 2026-08-08 |
| Last reviewed | 2026-08-08 |
| Next review trigger | Merge of the `MCP-029` activation change; creation of the accepted exact-crate `publish-new` crates.io credential; publication of `v0.1.0`; or intentional activation of a side quest |
...
| D-12 | Zero-cost market-validation release and installed recovery exercise | M2 | Codex | 2026-08-08 | In progress | `MCP-028` is complete through the [zero-cost workflows](.github/workflows/market-release.yml), [deterministic source generator](scripts/generate-market-release-channels.sh), and [market runbook](docs/market-release.md). `MCP-029` is active under its exact goal; draft [PR #28](https://github.com/EnjoyableWork/mcp-sync/pull/28) adds the protected tap-scoped Homebrew publisher, and its initial exact commit passes [market preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31273125678), retained [six-target preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31273125681), and [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31273125782). The signed-in crates.io profile reports a verified email without exposing it, the public organization tap and protected repository-scoped deploy key are ready, and the registry still reports no `enjoyable-mcp-sync`; the existing short-lived Cargo token is unrestricted by crate and includes additional endpoint scopes, so it is not accepted as the required exact-`enjoyable-mcp-sync`, `publish-new`-only first-publication credential. The ticket must still merge the activation change, publish immutable `v0.1.0`, crates.io `enjoyable-mcp-sync` `0.1.0`, and the source-built organization Homebrew formula from one commit, and verify their supported-host installation and recovery journeys without project-issued macOS or Windows binaries |
