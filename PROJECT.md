...
The main story is complete through the post-M3 `MCP-036` correction,
`MCP-039` release-security continuation, bounded M4 `MCP-037` expansion, and
post-M4 `MCP-040` correction. These respectively restore the issue #45
serialization invariant, protect every Cargo version after `0.1.0` with
tokenless Trusted Publishing, add the approved Kiro target without an adoption
prerequisite, and make issue #47's existing-file replacement recoverable across
abrupt process termination. Confirmed issue #62 now makes `MCP-041` the earliest
incomplete main-story ticket: the accepted source/GNU/Linux release path must
be repeatable before the project can intentionally publish any later version.
...
| Post-M4 release maintenance — repeatable source/GNU/Linux publication | `MCP-041` (in progress; depends on completed `MCP-040`) |
...
depends on its credential boundary. `MCP-037` and `MCP-040` are completed;
`MCP-041` is in progress as the required repeat-release correction, and no side
quest is active.
...
| MCP-041 | Make post-`0.1.0` source and GNU/Linux releases repeatable | Post-M4 release maintenance | P1 | Codex | In progress | `MCP-040` | Issue [#62](https://github.com/EnjoyableWork/mcp-sync/issues/62), `DEC-049`, D-20, and `RISK-22` define the confirmed correction. Done when canonical stable SemVer/tag/package validation accepts only a version newer than `0.1.0` and not older than the published stable inventory; the source/GNU/Linux workflow no longer requires the candidate crate before creating the immutable release; an already-published candidate is accepted only as exact-byte partial-run recovery; the dedicated protected Trusted Publisher remains tokenless and retry-safe; Homebrew handoff, public channel verification, authorization guidance, and release runbooks support later stable versions; focused acceptance/rejection and retry regressions plus a nonpublishing rehearsal pass; all local quality, dependency, workflow, package, archive, documentation, and native hosted preflight gates pass; and no version, tag, release, crate, formula, WinGet package, or signed-native artifact is created. |
...
activates in-progress `MCP-041` as the release-maintenance successor; no side
quest is active.
...
1. Complete `MCP-041` under its exact active Goal: finish local and hosted
   verification of the implemented canonical later-version policy,
...
before implementation. `MCP-029`, M2, `MCP-030` through `MCP-037`, `MCP-039`,
`MCP-040`, M3, bounded M4 expansion, `SIDE-007`, and `SIDE-008` are complete;
`MCP-041` is in progress and no side quest is active. The
...
`MCP-041` is the deliberately classified release-maintenance successor without
implicitly activating a side quest or funded signed-native work.
`DEC-031`, `DEC-036`, and
...
| RISK-22 | A later stable release deadlocks, republishes inconsistent bytes, or cannot recover after partial channel publication | High | High | Issue [#62](https://github.com/EnjoyableWork/mcp-sync/issues/62) records the fixed-`v0.1.0` triggers, GitHub/crates.io circular dependency, and reject-on-existing Cargo retry gap. Accepted `DEC-049`, in-progress `MCP-041`, and D-20 now implement canonical monotonic stable versions, credential-free request rejection, manual single-producer selection, immutable GitHub-first exact assets, retry-safe registry equality with OIDC/publish skip, monotonic prior-release-backed Homebrew updates, generalized read-only channel verification, one global release serialization boundary, and a nonpublishing `v0.1.0` rehearsal without changing the accepted artifact or credential scope. Complete local regressions and release checks pass | A later tag cannot reach an immutable release before Cargo publication; a stale, prerelease, mismatched, or older version passes; a partial Cargo upload cannot safely resume; registry, release, or downstream bytes differ; a generalized workflow accidentally invokes funded signing or mutates `v0.1.0`; or rehearsal requires real publication | Open — local mitigation implemented; close only after hosted native preflights and nonpublishing exact-main rehearsal pass |
