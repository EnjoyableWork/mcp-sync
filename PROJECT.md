...
| Current focus | Implement and prove the six-target preflight plus fail-closed signed release workflow while the external Apple, Microsoft, Cargo, Homebrew, and WinGet publication authorities are provisioned; do not create `v0.1.0` early |
| Milestone target | Unscheduled; set after an owner and delivery capacity are known |
| Last reviewed | 2026-08-08 |
| Next review trigger | Native review of the `MCP-021` release workflows; availability of both signing authorities; publication of `v0.1.0`; or intentional activation of a side quest |
