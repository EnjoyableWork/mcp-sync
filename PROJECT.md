...
| Product state | The source-checkout macOS, GNU/Linux, and Windows implementation reconciles six global targets: Claude Desktop, Cursor, Windsurf legacy Cascade, native VS Code default user-profile, global Codex host configuration, and Kiro's selected global-user configuration inherited by Kiro Crew. It exposes one bounded named-server STDIO initialize health boundary and parser-validated restore with one-generation adjacent retention for canonical state plus all six targets. Deterministic fixtures, complete six-client built-binary journeys, native ARM64 and Rosetta x64 macOS suites, native x64/ARM64 GNU/Linux and Windows MSVC CI, six-target returned-failure rollback, seven-file restore, and focused cross-process regressions prove plan/apply, preservation, redaction, ordinary compensation, and the issue #45 lock invariant. Issue [#47](https://github.com/EnjoyableWork/mcp-sync/issues/47) was confirmed against current `main`; submitted [PR #61](https://github.com/EnjoyableWork/mcp-sync/pull/61) replaces the vulnerable backup-first window with a target-first journaled transaction, refuses incomplete state on lock-free reads, and recovers it before locked planning. Twenty-one abrupt-process cases, complete native GNU/Linux and Windows x64/ARM64 CI, native macOS evidence, CodeQL, release preflight, and source/install/archive preflight pass on exact implementation head `d87272c`; the issue is ready for closure when the fix merges. Controlled macOS current-client evidence covers Claude Desktop, Cursor, Kiro IDE `1.0.288`, and inherited Kiro Crew `0.1.3`; no Linux or Windows target has a current-client smoke claim, and Windsurf, VS Code, and Codex retain no current-client claim. The immutable `v0.1.0` GitHub Release, byte-identical `enjoyable-mcp-sync` `0.1.0` crates.io package, and exact source-building `EnjoyableWork/tap/mcp-sync` formula remain public and unchanged; every later Cargo version is constrained to protected exact-identity crates.io Trusted Publishing with trusted-publishing-only enforcement and no API-token fallback |
| Current milestone | Post-M4 corrective maintenance complete in submitted [PR #61](https://github.com/EnjoyableWork/mcp-sync/pull/61) — `MCP-040` and D-19 are `Done`, issue [#47](https://github.com/EnjoyableWork/mcp-sync/issues/47) is ready for closure on merge, and no main-story ticket or side quest is active |
| Overall status | M0 through M3, `MCP-001` through `MCP-020`, `MCP-028` through `MCP-037`, `MCP-039`, and `MCP-040` are `Done`; `MCP-021` is `Superseded`; `MCP-022` through `MCP-027` are `Deferred`; `SIDE-007` and `SIDE-008` are `Done`; funded `SIDE-006` remains proposed; and no later work is activated or treated as a prerequisite |
| Current focus | Carry submitted [PR #61](https://github.com/EnjoyableWork/mcp-sync/pull/61) through protected review and merge so GitHub closes issue #47. Do not conflate issue #47 with issues #44 or #46, expand it into whole-sync process-crash atomicity, publish a release, or activate a successor or side quest |
...
abrupt process termination. No later main-story ticket has been selected or
activated:
...
depends on its credential boundary. `MCP-037` and `MCP-040` are completed; no
main-story ticket or side quest is active.
...
| MCP-040 | Make existing-file target and backup replacement crash-consistent | Post-M4 corrective maintenance | P0 | Codex | Done | `MCP-037` | `DEC-048` is implemented by D-19, submitted [PR #61](https://github.com/EnjoyableWork/mcp-sync/pull/61), and exact implementation head [`d87272c`](https://github.com/EnjoyableWork/mcp-sync/commit/d87272ca2d10ab74f8a65940e724abb7f381f718). Every defined pre-commit and post-commit state, both backup histories, all three affected command journeys, lock contention, malformed or externally changed state, safe cleanup, retry, redaction, ordinary compensation, reverse rollback, six-target native compilation and execution, CodeQL, CI, release preflight, and source/install/archive preflight pass. The PR carries `Fixes #47`, so the issue is ready for closure on merge. This ticket does not promise whole-sync atomicity across process death or power loss, change issues #44 or #46, publish a release, mutate immutable `v0.1.0`, or activate a successor or side quest. |
...
completed as the P0 corrective successor; no main-story ticket or side quest is
active.
...
for the completed ticket. `MCP-029`, M2, `MCP-030` through `MCP-037`,
`MCP-039`, `MCP-040`, M3, bounded M4 expansion, `SIDE-007`, and `SIDE-008` are
complete; no main-story ticket or side quest is active. The
...
no side quest or later implementation is implicitly active.
`DEC-031`, `DEC-036`, and
