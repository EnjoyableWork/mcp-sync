...
| Overall status | M2 and `MCP-030` through `MCP-034` are `Done`; `MCP-035` is the only `Ready` main-story successor and remains unassigned and inactive; only funded `SIDE-006` remains a proposed side quest |
| Current focus | Intentionally activate `MCP-035` under its exact canonical Goal when ready; until then, keep its public self-assessment and README assurance claims dormant while maintaining the completed `MCP-030` through `MCP-034` controls |
| Milestone target | Unscheduled |
| Last reviewed | 2026-08-09 |
| Next review trigger | `MCP-035` activation, completion, or blocker; an organization-access, recovery, Actions, dependency-update, workflow-trust, distribution-authentication, repository-artifact, `main` ruleset/check-context, repository-security-control, community-contract, repository-inventory, or licensing change; an assurance-framework revision; a security or release incident; evidence drift; or explicit funding and activation of `SIDE-006` |
...
| D-13 | Evidence-backed enterprise assurance baseline | M3 | Codex | Unscheduled | In progress | `MCP-030` through `MCP-034` have completed the contributor, repository, security, community, licensing, supply-chain, artifact, organization-access, strong-MFA, least-privilege, automation, and private-recovery controls in their accepted OpenSSF OSPS Baseline `v2026.02.19` Level 1 mappings. `MCP-034`'s explicitly authorized live changes, accepted single-owner residual risk, private secure-method and successful-recovery confirmations, aggregate evidence, and non-disclosing verifier pass are recorded without publishing identities or recovery details. `MCP-035` is now the only `Ready` successor and must publish the complete 24-control crosswalk, a dated and scoped self-attestation, a durable security-assurance contract, and only achieved README trust claims; the milestone does not imply OSPS Level 2 or 3, independent certification, regulatory compliance, funded code signing, or attainment of another candidate assurance target. |
...
1. Keep completed `MCP-030` through `MCP-034` controls active. `MCP-035` is the
...
2. Complete `MCP-035` without reusing a retired `SIDE-001` through `SIDE-005`
   objective or publishing an assurance badge or claim before its evidence
   gate passes.
...
choice.
`MCP-029`, M2, and `MCP-030` through `MCP-034` are complete. `MCP-034`'s
...
ticket gate without publishing private details. `MCP-035` is the only `Ready`
main-story successor, remains unassigned and inactive, and no other M3 ticket
