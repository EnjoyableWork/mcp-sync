...
| Product state | The source-checkout macOS, GNU/Linux, and Windows implementation reconciles six global targets: Claude Desktop, Cursor, Windsurf legacy Cascade, native VS Code default user-profile, global Codex host configuration, and Kiro's selected global-user configuration inherited by Kiro Crew. It exposes one bounded named-server STDIO initialize health boundary and parser-validated restore with one-generation adjacent retention for canonical state plus all six targets. Deterministic fixtures, complete six-client built-binary journeys, native ARM64 and Rosetta x64 macOS suites, native x64/ARM64 GNU/Linux and Windows MSVC CI, six-target rollback, seven-file restore, and focused cross-process regressions prove plan/apply, preservation, redaction, recovery, and the issue #45 lock invariant. Controlled macOS current-client evidence covers Claude Desktop, Cursor, Kiro IDE `1.0.288`, and inherited Kiro Crew `0.1.3`; no Linux or Windows target has a current-client smoke claim, and Windsurf, VS Code, and Codex retain no current-client claim. The immutable `v0.1.0` GitHub Release, byte-identical `enjoyable-mcp-sync` `0.1.0` crates.io package, and exact source-building `EnjoyableWork/tap/mcp-sync` formula remain public and unchanged; every later Cargo version is constrained to protected exact-identity crates.io Trusted Publishing with trusted-publishing-only enforcement and no API-token fallback |
| Current milestone | M4 — approved Kiro client expansion; `MCP-037` and D-18 are `In progress` under their exact canonical Goal while the six-target implementation, native matrix, and controlled Kiro IDE and Crew gates pass on the implementation branch |
| Overall status | M0 through M3, `MCP-001` through `MCP-036`, `MCP-039`, `SIDE-007`, and `SIDE-008` are `Done`; `MCP-037` is active under Codex as the sole main-story ticket; no side quest is active; funded `SIDE-006` remains proposed; Kiro selection has no `v0.1.0` adoption-evidence prerequisite; and only documentation, protected merge, exact-main verification, ticket closure, and Goal completion remain for this ticket |
| Current focus | Complete active `MCP-037`: retain the passing lossless Kiro adapter, complete six-target native and current-client evidence, and every existing safety and immutable-release boundary; finish documentation and final exact-head checks, merge through protected `main`, reverify exact `main`, record durable closure evidence, mark only `MCP-037` and D-18 `Done`, and complete the current Goal without activating a later arc or side quest |
...
| D-18 | Global-user Kiro adapter with inherited Kiro Crew coverage | M4 | Codex | Current ticket | In progress | [PR #58](https://github.com/EnjoyableWork/mcp-sync/pull/58) implementation head [`31df07b`](https://github.com/EnjoyableWork/mcp-sync/commit/31df07bc9a19877f25326c9444ed312e28323a10) adds the sixth target through one comment-preserving Kiro JSON adapter at default or absolute `KIRO_HOME`-resolved `settings/mcp.json`, owns only canonical-compatible local `command`, `args`, and string `env`, leaves reference-bearing and every other unrepresentable entry unmanaged, preserves comments and all unowned fields, and excludes workspace, agent, Crew-only, generated-agent, organization-managed, remote-service, and credential stores. Focused fixtures plus complete built-binary journeys prove deterministic six-client import, sync, restore, no-op, drift, redaction, collision, backup, exact-byte, sixth-target rollback, seven-file retention, operation-lock, relocated-home, and excluded-store behavior. All 342 tests pass natively on ARM64 and Rosetta x64 macOS; warning-free Clippy compiles all six macOS/GNU/Linux/Windows target triples; format, `cargo deny`, deterministic packaging, and diff checks pass; and exact-head native [CI](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31504935677) plus [release preflight](https://github.com/EnjoyableWork/mcp-sync/actions/runs/31504935703) pass. Controlled current-client smokes on 2026-08-11 prove signed and notarized Kiro IDE `1.0.288` completes `initialize`, `notifications/initialized`, and `tools/list` from the mcp-sync-rendered default global file in a restricted synthetic workspace and then returns the previously absent real target and directory exactly to absence while excluded real Crew/agent metadata remains unchanged. Isolated Kiro Crew `0.1.3` lists and probes the same rendered global entry, copies it exactly into its generated agent with `includeMcpJson: false`, leaves its override absent, preserves the global hash, and reaps every process. Documentation, protected merge, exact-main verification, ticket closure, and Goal completion remain; no `v0.1.0` adoption evidence is required, and no later work is activated or treated as a prerequisite. |
...
| MCP-037 | Add a global-user Kiro MCP adapter with inherited Kiro Crew coverage | M4 | P1 | Codex | In progress | `MCP-039` | `DEC-046` approves Kiro as the ordered M4 successor without requiring `v0.1.0` adoption evidence, and its exact canonical Goal is active. [PR #58](https://github.com/EnjoyableWork/mcp-sync/pull/58) now implements and documents the bounded global-user adapter, all six-target safety and native-platform gates, and the controlled Kiro IDE `1.0.288` plus inherited Kiro Crew `0.1.3` smokes summarized in D-18. Protected merge, exact-main verification, durable ticket closure, and Goal completion remain before this row becomes `Done`; no later arc or side quest is active. |
...
canonical Goal under Codex on 2026-08-11. The ticket remains `In progress`
until protected merge, exact-main verification, durable closure, and Goal
completion. Its implementation, native-platform, current-client, and support-
claim gates now pass on PR #58; no later work is active.
...
1. Complete active `MCP-037` under its exact canonical Goal. Preserve the now-
   passing Kiro contract revalidation, six-target implementation, native matrix,
   controlled IDE/Crew evidence, all five predecessor targets, and every safety
   boundary; finish final documentation and exact-head checks, merge through
   protected `main`, reverify exact `main`, close only `MCP-037` and D-18, and
   complete the Goal without activating a successor.
...
native and current-client evidence now pass on PR #58, while protected
exact-main ticket and Goal closure remain required. `DEC-047`
...
| --- | --- | --- | --- | --- | --- | --- |
| RISK-01 | A merge or partial failure loses user configuration | Critical | Medium | Pure plan/apply separation, guarded exact-byte writes, recoverable backups, reversible receipts, reverse-order six-target rollback, non-mutating drift, bounded client ownership, unmanaged-name collision refusal, interrupted-write compensation, and the complete synthetic failure matrix are implemented. Fixed-target restore validates retained bytes, swaps existing target and backup exactly, recreates missing targets without consuming the backup, and compensates an interrupted creation. A forced Kiro sixth-target failure restores Claude Desktop, Cursor, Windsurf, VS Code, and Codex plus their pre-existing backups exactly. Current Cursor verification is isolated; current Claude verification uses a no-clobber backup and verified exact restore; current Kiro verification returns a previously absent real target exactly to absence while preserving excluded Crew/agent metadata; every represented installed release channel completes its synthetic recovery journey | Any unrecoverable fixture mutation or ambiguous ownership case | Mitigated — sync, restore, current-client, and installed-channel recovery proof; monitor future releases |
