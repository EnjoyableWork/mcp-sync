...
| Product state | Pre-MVP; M0 foundation, the pure reconciliation engine, and fixture-backed global Claude Desktop and Cursor macOS adapters are implemented; no user journey is wired yet |
| Current milestone | M1 — Town MVP |
| Overall status | `MCP-008` complete; only `MCP-009` is ready; neither M1 client has received the controlled support smoke test |
| Current focus | `MCP-009` is the next eligible ticket and has not started |
| Milestone target | Unscheduled; set after an owner and delivery capacity are known |
| Last reviewed | 2026-08-07 |
| Next review trigger | Completion of `MCP-009`, or any change to the MVP boundary |
...
| D-05 | Two-client import and conflict reporting | M1 | Unassigned | Unscheduled | In progress | [Global Claude Desktop adapter and native fixtures](src/claude_desktop.rs), [global Cursor adapter](src/cursor.rs), and [Cursor fixtures](tests/fixtures/cursor); import orchestration remains in `MCP-009` |
...
| MCP-009 | Implement `init` discovery, import, normalization, and conflict reporting | M1 | P0 | Unassigned | Ready | `MCP-008` | `assert_cmd` built-binary journeys for deterministic two-client import and no-mutation conflict behavior through the synthetic home |
...
1. Start only `MCP-009`; assign its owner and activate its canonical goal before
   implementing deterministic two-client import and conflict reporting.
2. After `MCP-009` is `Done`, update its evidence and move only `MCP-010` to
   `Ready`.
3. Continue one row at a time; do not begin `MCP-010` or any later ticket early.
...
| --- | --- | --- | --- | --- | --- | --- |
| RISK-01 | A merge or partial failure loses user configuration | Critical | Medium | Pure plan/apply separation, non-mutating drift, bounded Claude and Cursor field ownership, Cursor remote-collision refusal, and project-file isolation are implemented; backups, atomic replacement, rollback, and failure tests remain sequenced | Any unrecoverable fixture mutation or ambiguous ownership case | Open — M1 gate |
| RISK-02 | Secrets leak through plans, errors, logs, fixtures, or snapshots | High | Medium | Plan plus both native document/render debug surfaces expose structure only; example, generated, malformed-input, remote-header, and native-fixture sentinel tests reject raw process values | Any test or output path observes a secret value | Open — M1 gate |
