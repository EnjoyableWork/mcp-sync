...
| Current milestone | Post-M4 release maintenance — `MCP-041` and D-20 are `Done`; no main-story ticket or side quest is active |
| Overall status | M0 through M3, `MCP-001` through `MCP-020`, `MCP-028` through `MCP-037`, and `MCP-039` through `MCP-041` are `Done`; `MCP-021` is `Superseded`; `MCP-022` through `MCP-027` are `Deferred`; `SIDE-007` and `SIDE-008` are `Done`; and funded `SIDE-006` remains proposed |
| Current focus | No main-story ticket or side quest is active. Maintain the completed repeat-release, Trusted Publishing, configuration-safety, recovery, and native evidence boundaries; require deliberate classification and activation before any later main-story work or side quest, and keep funded `SIDE-006` dormant until explicitly funded and activated |
...
#62's accepted source/GNU/Linux release lifecycle repeatable. No successor is
classified or active.
...
completed, and no main-story ticket or side quest is active.
...
1. Maintain completed `MCP-041`'s canonical later-version policy,
   ...
   the tap-scoped deploy key, and dormant funded `SIDE-006` unchanged; classify
   and activate any successor deliberately before work begins.
...
`MCP-039` through `MCP-041`, M3, bounded M4 expansion, `SIDE-007`, and
`SIDE-008` are complete; no main-story ticket or side quest is active. The
