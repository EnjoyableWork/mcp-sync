...
| Current milestone | Post-M3 release maintenance — `MCP-039` tokenless Cargo publication is `In progress`; approved M4 successor `MCP-037` remains `Proposed` until `MCP-039` is `Done` |
| Overall status | M0 through M3, `MCP-001` through `MCP-036`, `SIDE-007`, and `SIDE-008` are `Done`; `MCP-039` is active under Codex and remains the gate for every Cargo version after `0.1.0`; no side quest is active; funded `SIDE-006` remains proposed; and `MCP-037` Kiro is the approved ordered M4 successor without a `v0.1.0` adoption-evidence prerequisite |
| Current focus | Complete `MCP-039`'s fixed authorization-only rehearsal from exact protected `main` for the live exact `EnjoyableWork/mcp-sync` / `cargo-publish.yml` / `release` crates.io publisher, restore the `release` environment from its temporary exact-`main` bootstrap allowance to the sole `v*` tag policy, then enable and read back trusted-publishing-only enforcement without publishing a version; maintain the issue [#45](https://github.com/EnjoyableWork/mcp-sync/issues/45) cross-process mutation boundary, immutable release, and assurance evidence; keep approved successor `MCP-037` unstarted and make no Kiro support claim before its implementation and current-client evidence exist |
| Milestone target | `MCP-039` and D-17 satisfy their workflow, trusted-publisher, trusted-only, rehearsal, regression, native-preflight, documentation, and ticket-done gates without creating a version, tag, release, crate, formula, WinGet package, or signed-native artifact |
| Last reviewed | 2026-08-11 |
| Next review trigger | A README target-state promise, install channel, call to action, navigation anchor, live badge or link, client boundary, mutation-lock or safety contract changes; an organization-access, recovery, Actions, dependency-update, workflow-trust, crates.io trusted-publisher or trusted-only setting, distribution-authentication, repository-artifact, `main` ruleset/check-context, repository-security-control, community-contract, repository-inventory, or licensing change; an assurance-framework, issuer-status, badge, or public-proof-link revision; a security or release incident; evidence drift; completion or blocking of `MCP-039`; deliberate activation of approved successor `MCP-037`; or explicit funding and activation of `SIDE-006` |
...
The main story is complete through the post-M3 `MCP-036` correction. That P0
continuation restored the user-configuration safety invariant invalidated by
issue #45. `MCP-039` is now the active release-security continuation required
before another Cargo version. Approved M4 successor `MCP-037` follows it in the
ordered story without any `v0.1.0` adoption-evidence prerequisite, but it does
not begin until `MCP-039` is `Done` and intentionally handed off:
...
| Post-M3 release maintenance — tokenless Cargo publication | `MCP-039` (in progress; gates every Cargo version after `0.1.0`) |
| M4 — Approved Kiro client expansion | `MCP-037` (proposed successor; begins only after `MCP-039` is `Done`) |
...
explicit adoption outcome rather than optional work. `MCP-039` is required
release maintenance rather than a side quest because every later Cargo version
depends on its credential boundary; it is active under its canonical Goal.
...
- Existing exact-byte checks remain authoritative for non-cooperating client or
  ...
  boundary, process termination release, independent roots, final five-target
...
| D-17 | Tokenless crates.io publication for every version after `0.1.0` | Post-M3 release maintenance | Codex | Current ticket | In progress | Active `MCP-039` adds one dedicated protected GitHub Actions publisher bound to the `EnjoyableWork/mcp-sync` repository, the exact `cargo-publish.yml` workflow, and the `release` environment; uses short-lived crates.io OIDC authorization only; enables trusted-publishing-only enforcement; preserves immutable-GitHub-first and exact-`.crate` verification; and records non-sensitive configuration, authorization, rejection, and hosted-run evidence before a later Cargo version may publish. `v0.1.0` and its revoked first-publication token remain historical evidence, not a reusable path. No version is published while implementing this foundation. |
| D-18 | Global-user Kiro adapter with inherited Kiro Crew coverage | M4 | Unassigned | After `MCP-039` | Proposed | Approved `MCP-037` will add one bounded global-user Kiro target after `MCP-039` is `Done`, revalidate current default and `KIRO_HOME` path, precedence, interpolation, and Crew inheritance contracts, preserve every unowned or unrepresentable entry, extend the complete six-target import/sync/restore/rollback/lock/native matrix, and require controlled current-client Kiro IDE and Kiro Crew evidence before any support claim. Its approval has no `v0.1.0` adoption-evidence prerequisite. |
...
| MCP-039 | Replace Cargo publication tokens with crates.io Trusted Publishing | Post-M3 release maintenance | P1 | Codex | In progress | `MCP-036` | `DEC-047`'s dedicated `.github/workflows/cargo-publish.yml` exists on protected `main`, accepts only explicit manual version and tag input, enters the existing protected `release` environment only after unprivileged validation, requests `id-token: write` only in its publish job, uses the official crates.io authentication action pinned to a reviewed full commit SHA, and stores no crates.io token. The crates.io publisher is bound exactly to owner `EnjoyableWork`, repository `mcp-sync`, workflow `cargo-publish.yml`, and environment `release`; an authorization-only hosted rehearsal succeeds without publishing; repository policy and focused regressions reject mutable action references, wrong refs, untrusted triggers, missing exact-byte checks, or token fallback; and the crate setting **Require trusted publishing for all new versions** is enabled and read back without creating a token. Every later publication must first verify a protected immutable GitHub Release, reproduce and compare the exact `.crate`, publish through OIDC, download and compare the registry bytes, and run the native installed-channel checks. A failed trusted-publishing path stops publication; it never falls back to an API token. Documentation and durable non-sensitive evidence are current, all quality, dependency, workflow, native preflight, and hosted checks pass, and no tag, release, crate version, formula, or funded signing path is created while implementing this foundation. |
| MCP-037 | Add a global-user Kiro MCP adapter with inherited Kiro Crew coverage | M4 | P1 | Unassigned | Proposed | `MCP-039` | `DEC-046` approves Kiro as the ordered M4 successor without requiring `v0.1.0` adoption evidence. The implementation revalidates the current global and `KIRO_HOME` paths, precedence, interpolation, and Kiro Crew inheritance contracts; owns only canonical-compatible local STDIO fields in the selected global file; preserves unowned and unrepresentable data; never accesses higher-precedence, Crew-only, generated-agent, managed, or credential stores; extends deterministic six-target configuration, recovery, rollback, lock, and native-platform evidence; and completes controlled current-client Kiro IDE and inherited Kiro Crew smokes before any support claim. |
...
### Approved M4 successor

The owner approved `MCP-037` as the ordered M4 successor on 2026-08-10 without
requiring `v0.1.0` adoption evidence. Kiro is not implemented or publicly
supported yet: the ticket remains `Proposed` behind active `MCP-039`, and it
may move to `Ready` only after that predecessor is `Done`. Starting it still
requires deliberate Goal activation and ownership.

#### Kiro acceptance boundary

The approval is based on current official vendor material, not on a support
claim. `MCP-037` must recheck the then-current release, documentation, source
contract, and platform paths; name every Kiro product surface covered by the
adapter; and preserve the accepted global-only ownership and precedence
boundary.

The Kiro adapter must satisfy the normal adapter safety contract: never inspect
...
rendering, reporting, or testing. The ticket must extend deterministic
...
a controlled current-client smoke for every named Kiro product surface in that
...
precedence over the global file. Its plain-JSON `mcpServers` local STDIO shape
has canonical-compatible `command`, `args`, and `env`, so a bounded global
adapter is a strong fit. Current Kiro CLI also supports
[relocating its whole user directory](https://kiro.dev/changelog/cli/2-3/) with
`KIRO_HOME`, so the ticket must revalidate the default and relocated path
behavior for every surface it names rather than assuming that one process
environment controls them all. It must manage only the selected user-level
file; preserve `disabled`, `autoApprove`, `disabledTools`, remote URL, headers,
OAuth, and unknown data; leave higher-precedence layers untouched; and
explicitly treat Kiro's `${VARIABLE}` expansion syntax as semantic data rather
than a literal canonical value unless a separately accepted schema evolution
...
future controlled smokes; installation alone is not acceptance evidence or a
support claim. Crew-only targeting would require a separate later ticket and an
...
1. Complete active `MCP-039` under its canonical Goal: finish the protected
   authorization-only rehearsal from exact protected `main` for the live exact
   trusted-publisher identity, restore the `release` environment to its sole
   `v*` tag policy, enable and read back the trusted-only setting, and record
   the remaining fail-closed evidence without publishing a version or creating
   an API token.
2. Maintain completed `MCP-036`'s serialization of `init`, `add`, non-dry-run
...
3. Maintain the completed `SIDE-007` README product-page hierarchy, live
...
4. Maintain `SIDE-008`'s single pull-request CI suite, exact protected check
   names, and test-only Windows fixture headroom without weakening production
   health limits or native x64/ARM64 coverage.
5. Maintain the completed `MCP-030` through `MCP-035` controls, public evidence,
...
6. Keep approved `MCP-037` `Proposed` until `MCP-039` is `Done`, then hand it
   off deliberately under its own canonical Goal. Its product selection does
   not depend on `v0.1.0` adoption evidence, but a support claim still depends
   on the complete adapter, native-platform, and current-client proof.
7. Preserve immutable `v0.1.0`, its signed annotated release and operational
...
8. Monitor the public GitHub, Cargo, and source-building Homebrew channels as
...
9. Treat OSPS Level 1 as the only M3 assurance gate. Best Practices, NIST SSDF,
...
10. Retain the funded six-target signed-native workflow, preflight, metadata,
...
| DEC-046 | Approve Kiro as the next ordered M4 client expansion with bounded global-user and inherited Crew scope | Accepted | 2026-08-10 | Kiro exposes an official global JSON MCP configuration and a local STDIO shape that fits the existing adapter architecture, while workspace and agent layers retain higher precedence, `KIRO_HOME` can relocate the user directory, and `${VARIABLE}` references require explicit representability handling. The owner approved `MCP-037` as the proposed main-story successor to active `MCP-039` without requiring `v0.1.0` adoption evidence. Kiro Crew does not create a second adapter because it currently inherits the user-owned Kiro global file; `MCP-037` includes controlled Crew inheritance evidence and excludes Crew-only and generated-agent files. Installed Kiro IDE and Kiro Crew provide future macOS smoke access but are not evidence by themselves. Approval fixes product selection and story order, not current support: the ticket remains `Proposed` until `MCP-039` is `Done`, and revalidation, bounded ownership, lossless preservation, cross-platform transaction proof, current-client smokes, and deliberate Goal activation remain mandatory before implementation or a README support claim. |
| DEC-047 | Require crates.io Trusted Publishing for every Cargo version after `0.1.0` | Accepted | 2026-08-10 | The completed first publication is the only token-based exception: its scope-minimized `publish-new` token was revoked and removed, and no replacement token may be created or stored. Active `MCP-039` introduces one dedicated `.github/workflows/cargo-publish.yml` workflow so the same narrow Cargo publisher can follow either the zero-cost or funded immutable GitHub release path without granting either release builder direct registry authority. crates.io must bind the publisher exactly to owner `EnjoyableWork`, repository `mcp-sync`, workflow `cargo-publish.yml`, and environment `release`, then enable **Require trusted publishing for all new versions**. The protected publish job alone receives `id-token: write`, obtains a short-lived registry credential through the official crates.io authentication action pinned to a reviewed full commit SHA, and never stores or falls back to an API token. Unprivileged input and immutable-release validation, exact local/release/registry `.crate` equality, native installed-channel verification, trusted-setting readback, an authorization-only hosted rehearsal, and focused rejection tests gate completion. A failure stops publication and preserves evidence; it never weakens the publisher identity, bypasses the environment, republishes a version, replaces an immutable asset, or changes `v0.1.0`. Activation authorizes implementation of this security boundary but no version, tag, release, crate, formula, WinGet package, or signed-native artifact. |
...
to issue #45. `DEC-046` approves Kiro `MCP-037` as the ordered M4 successor to
`MCP-039` without a `v0.1.0` adoption-evidence prerequisite. Its bounded
global-user and inherited Crew scope is accepted; implementation revalidation,
current-client evidence, and later Goal activation remain required. `DEC-047`
...
decision remains open for that ticket, which is now active under its canonical
Goal. External setting changes remain bounded by its acceptance and
authorization gates.
`MCP-029`, M2, `MCP-030` through `MCP-036`, M3, `SIDE-007`, and `SIDE-008` are
complete; `MCP-039` is `In progress`, approved successor `MCP-037` remains
`Proposed`, and no side quest is active. The
...
satisfy the final M3 gate. `MCP-039` is the active release-maintenance successor
to `MCP-036`; `MCP-037` is approved but unstarted, and no side quest or later
implementation is implicitly active.
`DEC-031`, `DEC-036`, and
...
| RISK-03 | Native client schemas or paths drift | High | Medium | All five implemented global contracts and macOS, GNU/Linux, and Windows paths are fixture-backed. Current Cursor `3.15.6` and current Claude Desktop `1.26832.0` accept and initialize rendered global entries on macOS. GNU/Linux and Windows have deterministic built-binary coverage and successful native x64/ARM64 whole-suite CI; neither has a current-client smoke claim. Windsurf's legacy Cascade, VS Code's native default-profile, and Codex's global TOML contracts retain explicit exclusions and have no current-client smoke claim on any implemented platform. Approved but unimplemented Kiro `MCP-037` must revalidate its then-current global paths, schema, precedence, interpolation, `KIRO_HOME`, Crew inheritance, and product-surface contracts before implementation, without managing Crew-specific files or making a current support claim | A client update invalidates fixture/discovery behavior or changes one of the five implemented contracts, or `MCP-037` cannot establish one bounded, lossless global ownership model with current-client evidence | Mitigated — monitor implemented clients and revalidate Kiro during `MCP-037` |
...
| RISK-21 | A future Cargo version relies on a reusable token or an over-broad trusted-publisher identity | Critical | Medium | `DEC-047` makes active `MCP-039` a hard gate for every version after `0.1.0`: one dedicated workflow, exact repository/workflow/environment identity, protected-environment review, publish-job-only OIDC, a reviewed SHA-pinned official authentication action, trusted-only crates.io enforcement, immutable-GitHub-first exact-byte verification, no stored token, and no fallback. The first-publication token remains revoked and no successor token is authorized | Any later version is prepared before `MCP-039` is Done; a crates.io token is created, stored, or requested; the publisher omits the `release` environment or names a broader repository/workflow identity; OIDC reaches untrusted code; the trusted-only setting is disabled; authentication failure triggers token fallback; or local, release, and registry `.crate` bytes are not equal | In progress — blocks every Cargo version after `0.1.0` until `MCP-039` is implemented and verified |
