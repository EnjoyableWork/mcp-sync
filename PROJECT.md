...
| Current focus | Complete `MCP-039`'s protected authorization-only rehearsal for the live exact `EnjoyableWork/mcp-sync` / `cargo-publish.yml` / `release` crates.io publisher, then enable and read back trusted-publishing-only enforcement without publishing a version; maintain the issue [#45](https://github.com/EnjoyableWork/mcp-sync/issues/45) cross-process mutation boundary, immutable release, and assurance evidence; keep approved successor `MCP-037` unstarted and make no Kiro support claim before its implementation and current-client evidence exist |
...
1. Complete active `MCP-039` under its canonical Goal: finish the protected
   authorization-only rehearsal for the live exact trusted-publisher identity,
   enable and read back the trusted-only setting, and record the remaining
   fail-closed evidence without publishing a version or creating an API token.
