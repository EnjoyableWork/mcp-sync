...
| Current milestone | M3 — Enterprise assurance and adoption, accepted but not active |
| Overall status | M2 is `Done`; `DEC-038` promotes the outcomes of `SIDE-001` through `SIDE-005` into ordered `MCP-030` through `MCP-034`; `MCP-035` closes the evidence and public-claim gate; `MCP-030` is `Ready`, no Goal or owner is active, and only funded `SIDE-006` remains a proposed side quest |
| Current focus | Intentionally activate `MCP-030` with its exact Goal and an owner, complete the ordered M3 repository-control and assurance sequence without changing the immutable `v0.1.0` release boundary, and publish no badge or assurance claim before `MCP-035` verifies its exact scope, version, date, and evidence |
| Milestone target | Unscheduled |
| Last reviewed | 2026-08-08 |
| Next review trigger | `MCP-030` activation or completion, an assurance-framework revision, a security or release incident, evidence drift, or explicit funding and activation of `SIDE-006` |
...
north-star exception: badges and claims may enter the README only after
`MCP-035` verifies their exact framework version, level, scope, assessment date,
and durable public evidence, and they must be removed or corrected when that
...
- **Assurance serves adoption:** implement controls because they protect users,
  ...
  reduces evaluation and adoption friction.
...
| D-13 | Evidence-backed enterprise assurance baseline | M3 | Unassigned | Unscheduled | Proposed | `MCP-030` through `MCP-034` implement the contributor, repository, security, supply-chain, and organization controls mapped to all 24 OpenSSF OSPS Baseline `v2026.02.19` Level 1 requirements. `MCP-035` must then publish a complete control-and-evidence crosswalk, a dated and scoped self-attestation, a durable security-assurance contract, and only achieved README trust claims; the milestone does not imply OSPS Level 2 or 3, independent certification, regulatory compliance, funded code signing, or attainment of another candidate assurance target. |
...
| MCP-030 | Protect the default branch and define a contributor-compatible merge policy | M3 | P1 | Unassigned | Ready | `MCP-029` | An active `main` ruleset requires pull requests, binds strict up-to-date CI checks to the expected GitHub Actions app, requires resolved conversations, blocks deletion and force pushes, and documents a minimal bypass, approval count, allowed merge methods, and signed-commit choice after normal and emergency paths are tested. Durable evidence proves `OSPS-AC-03.01` and `OSPS-AC-03.02` without changing stable-tag or release-environment controls. |
| MCP-031 | Establish vulnerability disclosure, security contacts, and secret prevention | M3 | P1 | Unassigned | Proposed | `MCP-030` | A GitHub-recognized `SECURITY.md` defines supported versions, security contacts, a private reporting route, response and coordinated-disclosure expectations, and safe public-issue routing; private vulnerability reporting plus the available dependency graph, Dependabot security updates, CodeQL, secret scanning, and push protection are enabled where available and baseline-clean. Any unavailable supplemental feature has an exact documented platform limitation, while zero-cost security-contact and secret-prevention controls must still prove `OSPS-BR-07.01` and `OSPS-VM-02.01` without exposing alerts, reports, secrets, identities, or contact credentials. |
| MCP-032 | Publish the public contribution, community, repository, and licensing contract | M3 | P1 | Unassigned | Proposed | `MCP-031` | `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, structured bug and feature forms, and a pull-request template define setup, checks, defect and feature discussions, inbound MIT licensing, the explicit no-CLA and separate DCO decision, redaction, support boundaries, and the completed private security route. Every official project URI uses HTTPS; all in-scope code and distribution repositories are listed; source and released assets carry the MIT license; GitHub recognizes the community files; and evidence proves `OSPS-BR-03.01`, `OSPS-DO-02.01`, `OSPS-GV-02.01`, `OSPS-GV-03.01`, `OSPS-LE-02.01`, `OSPS-LE-02.02`, `OSPS-LE-03.01`, `OSPS-LE-03.02`, and applicable `OSPS-QA-04.01`. `CODEOWNERS` is added only when an independent reviewer exists. |
| MCP-033 | Automate dependency maintenance and harden CI and distribution supply chains | M3 | P1 | Unassigned | Proposed | `MCP-032` | Weekly grouped Cargo and GitHub Actions updates are configured; every action is pinned to a reviewed full commit SHA with an update hint; allowed-action and SHA policy is narrowed as far as required CI permits; untrusted metadata is validated; fork and other untrusted code receive no privileged credentials or assets; official distribution uses cryptographically authenticated channels; and a documented automated or review gate prevents committed generated executables and unreviewable binary artifacts. CI and dependency policy remain green, and evidence proves `OSPS-BR-01.01`, `OSPS-BR-01.03`, `OSPS-BR-03.02`, `OSPS-QA-05.01`, and `OSPS-QA-05.02` without modifying immutable `v0.1.0`. |
| MCP-034 | Establish organization access, credential, and ownership recovery policy | M3 | P1 | Unassigned | Proposed | `MCP-033` | Sensitive repository and organization resources require strong MFA; new collaborators receive manually assigned or least-privilege default access; there are two trusted owners or an explicitly accepted and tested single-owner recovery plan; administrative and maintainer access uses least-privilege teams where useful; repository creation is deliberate; private recovery practices are documented; and automation uses short-lived, scope-minimized credentials where possible. Evidence proves `OSPS-AC-01.01` and `OSPS-AC-02.01` without publishing identities, tokens, factors, recovery material, or private policy details. |
| MCP-035 | Self-assess, publish, and showcase the zero-cost enterprise assurance baseline | M3 | P1 | Unassigned | Proposed | `MCP-034` | A public durable assurance document evaluates every one of the 24 OpenSSF OSPS Baseline `v2026.02.19` Level 1 controls, links evidence or exact conditional reasoning, revalidates the four existing-evidence controls `OSPS-DO-01.01`, `OSPS-QA-01.01`, `OSPS-QA-01.02`, and `OSPS-QA-02.01`, and contains no private settings, identities, reports, or secrets. Only after all controls pass, publish a dated and scoped self-attestation plus a concise README Security and Trust surface and, only where a framework provides one, its official verifiable badge; separately verify and scope an artifact-level SLSA v1.0 Build Level 2 statement from the existing GitHub attestations; define annual, framework-change, security-incident, release-pipeline-change, and evidence-drift review triggers; and make no OSPS Level 2 or 3, independent-certification, regulatory, OpenChain, NIST-certification, or paid-signing claim. |
...
| MCP-035 | Complete MCP-035: perform and publish a point-in-time self-assessment of all 24 OpenSSF OSPS Baseline `v2026.02.19` Level 1 controls, link durable evidence or exact conditional reasoning, revalidate existing user-guide, public-source, public-history, and direct-dependency evidence, publish a security-assurance contract and only then a concise README Security and Trust surface plus any official verifiable badge actually provided for an achieved target, verify and scope an artifact-level SLSA v1.0 Build Level 2 statement from the existing GitHub attestations, and define revalidation triggers. Follow AGENTS.md and PROJECT.md, preserve unrelated worktree changes, disclose no private setting, identity, report, or secret, and make no OSPS Level 2 or 3, independent-certification, regulatory, OpenChain, NIST-certification, or paid-signing claim. Finish only when every Level 1 control passes, the exact version, level, scope, date, limitations, public evidence and claim language are verified, documentation checks and the ticket-done and M3 gates pass, durable evidence is recorded, MCP-035 and M3 are Done, and no later arc or side quest is implicitly activated. |
...
release process, and organization practices within the stated scope. It is not
an independent certification, and M3 makes no Level 2 or Level 3 claim.

| Owning work | Level 1 controls | Count | Evidence boundary |
| --- | --- | --- | --- |
| `MCP-030` | `OSPS-AC-03.01`, `OSPS-AC-03.02` | 2 | Direct-commit prevention and primary-branch deletion protection |
| `MCP-031` | `OSPS-BR-07.01`, `OSPS-VM-02.01` | 2 | Security contacts, private vulnerability reporting, and secret prevention |
...
| `MCP-034` | `OSPS-AC-01.01`, `OSPS-AC-02.01` | 2 | Organization access, authentication, least privilege, and recovery |
...
| **Total** | **Every Level 1 control in the named checklist version** | **24** | `MCP-035` must link durable evidence or exact applicability reasoning for every row before any public claim |
...
| Target | Permitted assertion and scope | Direct fee | Adoption value | Owning work | Status and public-claim gate |
| --- | --- | --- | --- | --- | --- |
| [OpenSSF OSPS Baseline `v2026.02.19` Level 1](https://baseline.openssf.org/versions/2026-02-19-checklist.md) | Dated, scoped, evidence-backed project self-assessment | $0 | Broad minimum open-source security and governance signal | `MCP-030` through `MCP-035` | Accepted M3 gate; no claim or badge before every mapped control passes in `MCP-035` |
| [OpenSSF Best Practices Passing](https://www.bestpractices.dev/en/criteria/0) | Public project self-certification against the badge criteria | $0 | Familiar open-source project-health signal | Candidate gap assessment after M3 | Not active and not claimed; a later accepted ticket must verify every criterion before displaying a badge |
| [SLSA v1.0 Build Level 2](https://slsa.dev/spec/v1.0/levels#build-l2) | Artifact-scoped statement only for release assets with verified provenance | $0 | Supply-chain provenance signal for enterprise artifact review | Existing `MCP-029` evidence; exact-scope audit in `MCP-035` | Evidence exists for the attested `v0.1.0` assets; public wording remains gated on scope verification and must not imply all channels or future releases |
| [NIST SSDF 1.1](https://csrc.nist.gov/pubs/sp/800/218/final) | Self-evaluated practice alignment, never “NIST certified” | $0 | Enterprise procurement vocabulary and control crosswalk | Candidate gap assessment after M3 | Not active and not claimed; a later accepted ticket must define applicable practices, evidence, gaps, and assessment date |
| [OpenChain ISO/IEC 18974](https://openchainproject.org/security-assurance) and [ISO/IEC 5230](https://openchainproject.org/license-compliance) | Narrow organization-process self-certification only where the official conformance path permits it; never a product certification | $0 self-certification resources; implementation expertise may cost | Licensing and vulnerability-management process signal for larger organizations | Funding- and capacity-aware stretch after M3 | Not active and not claimed; requires a separately accepted scope, owner, and evidence plan |

### Immediate focus

1. Intentionally activate only `MCP-030`, the earliest incomplete main-story
   ticket, by assigning an owner and reconciling the thread to its exact
   canonical Goal. M3 is accepted but no ticket or Goal is active yet.
2. Complete `MCP-030` through `MCP-035` in order. Do not activate a successor
   early, reuse a retired `SIDE-001` through `SIDE-005` objective, or publish an
   assurance badge or claim before the `MCP-035` evidence gate passes.
...
5. Treat OSPS Level 1 as the only M3 assurance gate. Best Practices, NIST SSDF,
   ...
   non-gating candidates until separately accepted; SLSA wording remains
   limited to the exact attested release artifacts verified by `MCP-035`.
...
`DEC-038` establishes the ordered M3 assurance arc. `MCP-029` and M2 are
complete. `MCP-030` is the only `Ready` product ticket, but it has no owner and
no active Goal. Its pull-request approval threshold, allowed merge methods,
minimal bypass, and signed-commit enforcement choice must be decided and tested
inside that ticket. Organization continuity and recovery choices remain scoped
to later `MCP-034`; no other M3 ticket is implicitly active. `DEC-031`,
`DEC-036`, and `DEC-037` remain the release boundary; protected publication
access is durable evidence under `RISK-17`, while paid signing authority under
`RISK-12` blocks only dormant `SIDE-006`.
...
| RISK-19 | A stale, over-broad, or ambiguous self-attestation or badge misleads adopters | High | Medium | `DEC-038` limits M3 to the exact OSPS checklist version and Level 1 scope, separates artifact-scoped SLSA evidence from project-wide claims, blocks README trust language until `MCP-035`, and requires the framework version, assessment date, scope, limitations, public evidence, and revalidation triggers beside any claim | The framework changes, a mapped control or evidence link fails, a security or release-pipeline change invalidates evidence, or public wording implies a higher level, certification, another framework, all artifacts, or current status that was not assessed | Open — `MCP-035` claim and maintenance gate |
...
- any README Security and Trust text or badge describes only achieved current
  evidence and does not imply Level 2 or 3, independent certification,
  regulatory compliance, another assurance framework, or paid native signing;
